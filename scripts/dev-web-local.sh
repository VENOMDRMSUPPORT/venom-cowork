...
PROJECT_NAME="venomcowork-web-local"

# Local-dev defaults â€” match the MySQL container in docker-compose.web-local.yml.
# These are only used when not already set in the environment or .env.
: "${DATABASE_URL:=mysql://root:password@127.0.0.1:3306/venomcowork_den}"
...
pnpm --filter @venom-cowork-ee/den-db db:push <<'EOF'
y
EOF
...
(
  ...
  env \
    ...
    NEXT_PUBLIC_VENOMCOWORK_AUTH_CALLBACK_URL="$DEN_WEB_ORIGIN" \
