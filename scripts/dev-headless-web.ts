...
const autoBuildEnabled =
  process.env.VENOMCOWORK_DEV_HEADLESS_WEB_AUTOBUILD == null
    ? true
    : readBool(process.env.VENOMCOWORK_DEV_HEADLESS_WEB_AUTOBUILD);
...
const remoteAccessEnabled = readBool(process.env.VENOMCOWORK_REMOTE_ACCESS);
const host = remoteAccessEnabled ? "0.0.0.0" : "127.0.0.1";
const viteHost = process.env.VITE_HOST ?? process.env.HOST ?? host;
const publicHost = process.env.VENOMCOWORK_PUBLIC_HOST ?? null;
const clientHost = publicHost ?? (host === "0.0.0.0" ? "127.0.0.1" : host);
const workspace = process.env.VENOMCOWORK_WORKSPACE ?? cwd;
const venomcoworkPort = await resolvePort(process.env.VENOMCOWORK_PORT, "127.0.0.1");
const webPort = await resolvePort(process.env.VENOMCOWORK_WEB_PORT, "127.0.0.1");
const venomcoworkToken = process.env.VENOMCOWORK_TOKEN ?? randomUUID();
const venomcoworkHostToken = process.env.VENOMCOWORK_HOST_TOKEN ?? randomUUID();
const venomcoworkServerBin = path.join(
  cwd,
  "apps/server/dist/bin/venomcowork-server",
);
...
const ensureOpenworkServer = async () => {
  try {
    ...
    if (!autoBuildEnabled) {
      logLine(
        `[dev:headless-web] Missing VenomCowork server binary at ${venomcoworkServerBin}`,
      );
      logLine(
        "[dev:headless-web] Auto-build disabled (VENOMCOWORK_DEV_HEADLESS_WEB_AUTOBUILD=0)",
      );
      logLine(
        "[dev:headless-web] Run: pnpm --filter venomcowork-server build:bin",
      );
      logLine(
    ...
    logLine(
      "[dev:headless-web] Auto-building: pnpm --filter venomcowork-server build:bin",
    );
    try {
      await runCommand("pnpm", ["--filter", "venomcowork-server", "build:bin"]);
      await access(venomcoworkServerBin);
    } catch (error) {
      logLine(
...
const opencodeRouterEnabled =
  process.env.VENOMCOWORK_DEV_OPENCODE_ROUTER == null
    ? true
    : readBool(process.env.VENOMCOWORK_DEV_OPENCODE_ROUTER);
const opencodeRouterRequired = readBool(
  process.env.VENOMCOWORK_DEV_OPENCODE_ROUTER_REQUIRED,
);
const viteEnv = {
  ...process.env,
  HOST: viteHost,
  PORT: String(webPort),
  VITE_VENOMCOWORK_URL: process.env.VITE_VENOMCOWORK_URL ?? venomcoworkUrl,
  VITE_VENOMCOWORK_PORT: process.env.VITE_VENOMCOWORK_PORT ?? String(venomcoworkPort),
  VITE_VENOMCOWORK_TOKEN: process.env.VITE_VENOMCOWORK_TOKEN ?? venomcoworkToken,
};
const headlessEnv = {
  ...process.env,
  VENOMCOWORK_WORKSPACE: workspace,
  VENOMCOWORK_HOST: host,
  VENOMCOWORK_REMOTE_ACCESS: remoteAccessEnabled ? "1" : "0",
  VENOMCOWORK_PORT: String(venomcoworkPort),
  VENOMCOWORK_TOKEN: venomcoworkToken,
  VENOMCOWORK_HOST_TOKEN: venomcoworkHostToken,
  VENOMCOWORK_SERVER_BIN: venomcoworkServerBin,
  VENOMCOWORK_SIDECAR_SOURCE: process.env.VENOMCOWORK_SIDECAR_SOURCE ?? "external",
...
logLine(`[dev:headless-web] VenomCowork server: ${venomcoworkUrl}`);
...
logLine(
  `[dev:headless-web] OpenCodeRouter: ${opencodeRouterEnabled ? "on" : "off"} (set VENOMCOWORK_DEV_OPENCODE_ROUTER=0 to disable)`,
);
logLine("[dev:headless-web] VENOMCOWORK_TOKEN: [REDACTED]");
logLine("[dev:headless-web] VENOMCOWORK_HOST_TOKEN: [REDACTED]");
logLine(
