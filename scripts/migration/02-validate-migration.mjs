...
async function main() {
  ...
  const workDir = join(tmpdir(), `venomcowork-migrate-validate-${args.tag}`);
  ...
  presenceChecks.forEach(([label, name]) => {
    console.log(`    ${name ? "âœ“" : "âœ—"} ${label}${name ? ` â€” ${name}` : ""}`);
  });
  if (!electronZip) die("no Electron zip asset on the release.");

  // 2. Download + Apple sig check on the Electron zip.
  console.log("");
  console.log("[validate] downloading Electron zip for signature checkâ€¦");
  ...
  const appPath = join(unzipDir, "VenomCowork.app");
  if (!existsSync(appPath)) {
    console.log(`[validate] âœ— VenomCowork.app not found inside zip (got: ${unzipDir})`);
    die("zip layout unexpected");
  }
  const sig = codesignVerify(appPath);
  console.log(`[validate] ${sig.ok ? "âœ“" : "âœ—"} codesign verify on ${appPath}`);
  ...
  console.log("");
  console.log("[validate] manual checklist â€” acknowledge each step.");
  const steps = [
    "Install a fresh Tauri v0.11.x build on a test machine (or VM).",
    `Launch it, "Check for updates" â†’ migrates to ${args.tag}.`,
    "Restart, see the 'VenomCowork is moving' modal, click 'Install now'.",
    ...
    "Settings â†’ Advanced: both runtimes show 'Connected'.",
    "Reopen: confirm the migration modal does NOT reappear.",
  ];
  for (const step of steps) {
    const ok = await confirm(`  â˜ ${step}`);
