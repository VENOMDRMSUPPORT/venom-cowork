# Tauri â†’ Electron migration runbook
...
| ---- | ---- | ------------------------ |
| 01   | Ready to ship the migration | Tauri users see "VenomCowork is moving" prompt on next update |
| 02   | Right after the workflow finishes | Dogfood validation â€” no user effect |
...
- Pushes to `dev` or `main` refresh the rolling prerelease bucket at
  <https://github.com/venom-cowork/workspace/releases/tag/electron-preview-latest>.
...
- The install handoff requires a pasted Electron artifact URL plus two explicit
  ...
  `VenomCowork.app.migrate-bak` for rollback.
...
3. Ask testers to download the matching platform artifact:
   - macOS Apple Silicon: `venomcowork-mac-arm64-*.dmg` or `.zip`
   - macOS Intel: `venomcowork-mac-x64-*.dmg` or `.zip`
   - Windows: `venomcowork-win-x64-*.exe`
   - Linux: `venomcowork-linux-x64-*.AppImage` or `.tar.gz`
...
## 01 â€” cut-migration-release.mjs

```bash
node scripts/migration/01-cut-migration-release.mjs --version 0.12.0 \
  --mac-url 'https://github.com/venom-cowork/workspace/releases/download/v0.12.0/VenomCowork-darwin-arm64-0.12.0-mac.zip' \
...
3. Creates a release-config fragment at
   ...
   `VITE_VENOMCOWORK_MIGRATION_RELEASE=1` and the per-platform download
...
## 03 â€” post-migration-cleanup.mjs
...
1. Flips `apps/desktop/package.json` defaults:
   - `dev` â†’ `node ./scripts/electron-dev.mjs`
   - `build` â†’ `node ./scripts/electron-build.mjs`
   - `package` â†’ `pnpm run build && pnpm exec electron-builder â€¦`
...
- Users still on Tauri: the migrate prompt is gated on
  `VITE_VENOMCOWORK_MIGRATION_RELEASE=1` at build time. Re-cut the
...
- Users mid-migration: the Rust `migrate_to_electron` command keeps
  the previous `.app` at `VenomCowork.app.migrate-bak`. Instruct users
  to `mv VenomCowork.app.migrate-bak VenomCowork.app` if the Electron
  launch fails.
