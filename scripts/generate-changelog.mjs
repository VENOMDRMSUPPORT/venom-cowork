...
const COMPARE_BASE = "https://github.com/venom-cowork/workspace/compare"
...
/** "2026-02-19T17:49:05Z" â†’ "February 19th" */
...
function resolveTags(features, bugs, deprecated) {
  const tags = []
  if (features > 0) tags.push("ðŸš€ New Features")
  if (bugs > 0) tags.push("ðŸ› Bug Fixes")
  if (deprecated > 0) tags.push("ðŸ—ï¸ Refactoring")
  return tags.length > 0 ? tags : ["ðŸ”§ Misc"]
}
...
function renderVersionBlock(entry) {
  ...
  // Body: major â†’ bullet points, minor â†’ one-liner
 ...
 * "ðŸ”§ Misc" is dropped if any real tag exists.
 */
function renderDayGroup(dayEntries) {
  ...
  const filtered = allTags.filter((t) => t !== "ðŸ”§ Misc")
  const tags = filtered.length > 0 ? filtered : ["ðŸ”§ Misc"]
