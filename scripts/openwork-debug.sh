...
#   kill-orphans    remove orphan venomcowork/opencode processes (ppid == launchd)
...
#   wait-healthy    block until venomcowork-server reports /health = 200
#   reset           stop + wipe Vite dep cache + truncate log sink + start
#   restart         alias for reset
#
# Variant (VENOMCOWORK_DEV_VARIANT):
#   electron   (default) pnpm --filter @venom-cowork/desktop dev:electron
...
#   - ~/Library/Application Support/com.venom.cowork.dev/** (tokens,
#     workspaces registry, prefs). Use `reset-webview` for WebKit state.
#   - /Applications/VenomCowork.app (prod build never targeted).
#
set -euo pipefail
...
DEV_LOG_FILE="${VENOMCOWORK_DEV_LOG_FILE:-$HOME/.venomcowork/debug/venomcowork-dev.log}"
PNPM_DEV_LOG="${VENOMCOWORK_PNPM_DEV_LOG:-/tmp/venomcowork-test/pnpm-dev.log}"
PNPM_DEV_PID_FILE="${VENOMCOWORK_PNPM_DEV_PID:-/tmp/venomcowork-test/pnpm-dev.pid}"
WAIT_HEALTHY_SECS="${VENOMCOWORK_WAIT_HEALTHY_SECS:-90}"
ELECTRON_CDP_PORT="${VENOMCOWORK_ELECTRON_REMOTE_DEBUG_PORT:-9823}"
...
DEV_VARIANT="${VENOMCOWORK_DEV_VARIANT:-electron}"
case "$DEV_VARIANT" in electron|tauri) ;; *)
  printf '[venomcowork-debug] unknown VENOMCOWORK_DEV_VARIANT=%s (expected electron|tauri)\n' "$DEV_VARIANT" >&2
  exit 2
  ;;
esac
...
log() { printf '[venomcowork-debug] %s\n' "$*"; }
...
kill_by_pattern() {
  ...
  # webview (matched by its target/debug path, so prod VenomCowork.app is safe).
  local pattern="$1"
  local pids
...
discover_venomcowork_server_port() {
  ps -Ao command \
    | grep -E "(target/debug|apps/desktop/src-tauri/sidecars)/venomcowork-server" \
    | grep -v grep \
    | grep -oE '\-\-port [0-9]+' \
    | head -1 \
    | awk '{print $2}' \
...
probe_electron_page_cdp() {
  node <<'NODE'
const port = process.env.VENOMCOWORK_ELECTRON_REMOTE_DEBUG_PORT || "9823";
...
snapshot() {
  ...
  local port
  port=$(discover_venomcowork_server_port)
  if [[ -z "$port" ]]; then
    echo "  (no dev venomcowork-server running)"
  else
  ...
  oc_port=$(ps -Ao command \
    | grep -E "(target/debug|apps/desktop/src-tauri/sidecars)/venomcowork-orchestrator" \
    | grep -v grep \
  ...
  if [[ -f "$DEV_LOG_FILE" ]]; then
    ...
    echo "  (no sink file yet â€” run the dev app with VENOMCOWORK_DEV_LOG_FILE set)"
  fi
}
...
diagnose_hang() {
  ...
  echo "=== venomcowork hang diagnosis ==="
  echo "time=$now"
  echo "repo=$REPO_ROOT"
  ...
  if [[ -n "$renderer_pid" ]]; then
    ...
    printf '  %s\n' "$renderer_stats" | sed -E 's#/Users/[^ ]*/#â€¦/#g'
  else
  ...
  if [[ "$browser_json" == *"webSocketDebuggerUrl"* && "$target_json" == *"webSocketDebuggerUrl"* ]]; then
    if page_probe=$(VENOMCOWORK_ELECTRON_REMOTE_DEBUG_PORT="$ELECTRON_CDP_PORT" probe_electron_page_cdp 2>&1); then
  ...
  port=$(discover_venomcowork_server_port)
  if [[ -n "$port" ]]; then
    echo "  venomcowork-server port=$port"
    curl -sS --max-time 2 "http://127.0.0.1:$port/health" || echo "unreachable"
    echo
  else
    echo "  no venomcowork-server port discovered"
  fi
  ...
  if [[ -n "$renderer_pid" && ( "$page_probe" == "failed" || "$high_cpu" == "true" ) ]]; then
    local sample_file="/tmp/venomcowork-renderer-${renderer_pid}-$(date +%Y%m%dT%H%M%S).sample.txt"
    ...
    echo "  skipped (renderer responsive and CPU not high). Set VENOMCOWORK_FORCE_SAMPLE=1 not currently supported."
  else
...
stop() {
  ...
  # 3. Tauri dev webview â€” match full path so the installed /Applications/
  #    prod bundle is never targeted.
  kill_by_pattern "target/debug/VenomCowork-Dev"
  ...
  #    src-tauri/sidecars/ â€” kill both trees, both are idempotent.
...
start() {
  ...
  case "$DEV_VARIANT" in
    electron)
      ...
      env VENOMCOWORK_DEV_LOG_FILE="$DEV_LOG_FILE" \
        nohup pnpm --filter @venom-cowork/desktop dev:electron >"$PNPM_DEV_LOG" 2>&1 &
      pid=$!
      ;;
    tauri)
...
wait_healthy() {
  ...
  while (( SECONDS < deadline )); do
    ...
    if [[ -n "$port" ]]; then
      ...
      if [[ "$code" == "200" ]]; then
        log "venomcowork-server healthy on :$port"
        return 0
      fi
    fi
    sleep 1
  done
  log "venomcowork-server did not become healthy within ${WAIT_HEALTHY_SECS}s" >&2
  return 1
}
...
reset() {
  ...
  case "$DEV_VARIANT" in
    electron)
      log "reset complete (variant=electron) â€” Electron CDP should be up at"
      ...
      ;;
    tauri)
      log "reset complete â€” now reload the Tauri webview (Cmd+Shift+R) to drop"
...
reset_webview_state() {
  ...
  # the venomcowork-workspaces.json registry or server-side tokens.
  local webkit_dir="$HOME/Library/WebKit/com.venom.cowork.dev"
