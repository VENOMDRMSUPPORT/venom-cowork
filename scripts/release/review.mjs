...
addCheck(
  "App/venomcowork-orchestrator versions match",
  versions.app &&
...
addCheck(
  "App/venomcowork-server versions match",
...
} else if (!venomcoworkServerPinned) {
  addWarning(
    `venomcowork-orchestrator venomcowork-server dependency is not pinned (${venomcoworkServerRange}).`,
  );
} else {
  addCheck(
...
const sidecarManifestPath = resolve(
  ...
  "dist",
  "sidecars",
  "venomcowork-orchestrator-sidecars.json",
);
if (existsSync(sidecarManifestPath)) {
  const manifest = readJson(sidecarManifestPath);
  addCheck(
    "Sidecar manifest version matches venomcowork-orchestrator",
  ...
  addWarning(
    "Sidecar manifest missing (run pnpm --filter venomcowork-orchestrator build:sidecars).",
  );
}
