...
const composeProject = "venomcowork-den-local"
...
const appPort = process.env.VENOMCOWORK_APP_PORT?.trim() || process.env.PORT?.trim() || "5173"
const databaseUrl = process.env.DATABASE_URL?.trim() || "mysql://root:password@127.0.0.1:3306/venomcowork_den"
...
async function main() {
  ...
  await run("bash", ["-lc", "pnpm --filter @venom-cowork-ee/den-db build && pnpm --filter @venom-cowork-ee/den-db exec node --import tsx ./node_modules/drizzle-kit/bin.cjs push --config drizzle.config.ts --force"], {
    env: {
      ...process.env,
  ...
  turboChild = spawn(
    ...
    [
      ...
      "dev:local",
      "--output-logs=full",
        "--filter=@venom-cowork-ee/den-api",
        "--filter=@venom-cowork-ee/inference",
        "--filter=@venom-cowork-ee/den-worker-proxy",
        "--filter=@venom-cowork-ee/den-web",
    ],
    {
      cwd: rootDir,
      stdio: "inherit",
      ...
      env: {
        ...process.env,
        VENOMCOWORK_DEV_MODE: process.env.VENOMCOWORK_DEV_MODE?.trim() || "1",
        DATABASE_URL: databaseUrl,
