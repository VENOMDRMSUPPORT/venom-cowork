...
VENOMCOWORK_ROOT="$(cd "$(dirname "$0")/.." && pwd)"

# Detect whether we're inside a factory layout (../../.. has _repos/)
FACTORY_CANDIDATE="$(cd "$VENOMCOWORK_ROOT/../../.." 2>/dev/null && pwd)"
...
if [ -n "$FACTORY_ROOT" ]; then
  for d in "$FACTORY_ROOT"/_repos/*/; do
    [ -d "$d" ] || continue
    [ "$(cd "$d" && pwd)" = "$VENOMCOWORK_ROOT" ] && continue
    SIBLING_REPOS+=("$d")
  done
fi

# â”€â”€ Internal infra: all build/config/CI files that may reference source â”€â”€â”€â”€
# These are files WITHIN venomcowork that knip can't trace but that use source files
...
# File-based routing directories â€” files here are entry points by convention
ROUTING_DIRS=(
  "ee/apps/den-web/app/"
  "ee/apps/landing/app/"
)
...
collect_infra_files() {
  ...
  for glob_pattern in "${INFRA_GLOBS[@]}"; do
    ...
    local matched
    matched=$(find "$VENOMCOWORK_ROOT" -path "$VENOMCOWORK_ROOT/$glob_pattern" 2>/dev/null || true)
    if [ -n "$matched" ]; then
      files="${files}${matched}"$'\n'
    fi
  done
  ...
  local pkg_jsons
  pkg_jsons=$(find "$VENOMCOWORK_ROOT" -name package.json -not -path '*/node_modules/*' -not -path '*/.git/*')
  files="${files}${pkg_jsons}"$'\n'
  # And all tsconfig*.json files (for path aliases / includes)
  local tsconfigs
  tsconfigs=$(find "$VENOMCOWORK_ROOT" -name 'tsconfig*.json' -not -path '*/node_modules/*' -not -path '*/.git/*')
...
# Search sibling repo CI/CD and build scripts for an venomcowork-relative path.
# Only checks infra files (workflows, Dockerfiles, build scripts), NOT source code,
# since no sibling repo has an npm dependency on venomcowork packages.
search_sibling_ci() {
  local pattern="$1"
  local hits=""
...
format_refs() {
  ...
  while IFS= read -r ref; do
    ...
    local short="$ref"
    if [[ "$ref" == "$VENOMCOWORK_ROOT/"* ]]; then
      short="${ref#"$VENOMCOWORK_ROOT/"}"
...
# â”€â”€ Step 1: Run knip â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
cd "$VENOMCOWORK_ROOT"
...
# â”€â”€ Step 2: Build infra file list once â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
if [ -n "$FACTORY_ROOT" ]; then
  ...
  echo -e "${DIM}  No factory layout detected â€” skipping sibling repo checks.${RESET}\n" >&2
fi

# â”€â”€ Step 3: Cross-reference each file â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
for filepath in "${UNUSED_FILES[@]}"; do
  ...
  # â”€â”€ Check 1: Internal infra (CI, Docker, build scripts, configs, tsconfigs, package.jsons) â”€â”€
  ...
  # â”€â”€ Check 2: Convention-based usage â”€â”€
  if [ "$status" = "safe" ]; then
  ...
  # â”€â”€ Check 3: File-based routing dirs â”€â”€
  if [ "$status" = "safe" ]; then
  ...
  # â”€â”€ Check 4: Sibling repo CI/CD references â”€â”€
  # Only search by the venomcowork-relative path (precise) or unique filename.
  # Since no sibling repo has npm deps on venomcowork packages, we only check
...
# â”€â”€ Step 4: Split into two buckets, sort by date â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
safe_entries=()
flagged_entries=()
...
# â”€â”€ Step 5: Display â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
echo -e "${RED}${BOLD} UNUSED â€” safe to remove (${safe_count})${RESET}"
echo -e "${BOLD}â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”${RESET}"
echo -e "${DIM}  No imports in venomcowork source, no references in CI/CD, build${RESET}"
echo -e "${DIM}  scripts, configs, conventions, routing, or sibling repo pipelines.${RESET}"
echo ""

if [ "$safe_count" -eq 0 ]; then
  echo -e "  ${GREEN}None â€” all files have references somewhere.${RESET}"
else
  for entry in "${safe_sorted[@]}"; do
    date="${entry%%|*}"
    filepath="${entry#*|}"
    short_date="${date%%T*}"
    echo -e "  ${RED}âœ—${RESET} ${DIM}${short_date}${RESET}  ./$filepath:1"
  done
fi

echo ""
echo -e "${BOLD}â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”${RESET}"
echo -e "${YELLOW}${BOLD} REVIEW â€” referenced in infra/CI (${flagged_count})${RESET}"
echo -e "${BOLD}â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”${RESET}"
...
if [ "$flagged_count" -eq 0 ]; then
  ...
  for entry in "${flagged_sorted[@]}"; do
    ...
    case "$status" in
      infra)
        echo -e "  ${YELLOW}âš ${RESET} ${DIM}${short_date}${RESET}  ./$filepath:1"
        echo -e "    ${DIM}â†³ ${formatted_refs}${RESET}"
        ;;
      convention)
        echo -e "  ${YELLOW}âš ${RESET} ${DIM}${short_date}${RESET}  ./$filepath:1"
        echo -e "    ${DIM}â†³ ${refs}${RESET}"
        ;;
      routing)
        echo -e "  ${YELLOW}âš ${RESET} ${DIM}${short_date}${RESET}  ./$filepath:1"
        echo -e "    ${DIM}â†³ ${refs}${RESET}"
        ;;
      sibling_ci)
        echo -e "  ${CYAN}âš ${RESET} ${DIM}${short_date}${RESET}  ./$filepath:1"
        echo -e "    ${DIM}â†³ sibling CI: ${formatted_refs}${RESET}"
        ;;
    esac
  done
fi
...
echo ""
echo -e "${BOLD}â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”${RESET}"
echo -e "${BOLD}Summary:${RESET}  ${RED}${safe_count} removable${RESET}  â”‚  ${YELLOW}${flagged_count} need review${RESET}  â”‚  ${#UNUSED_FILES[@]} total"
echo -e "${BOLD}â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”${RESET}"
