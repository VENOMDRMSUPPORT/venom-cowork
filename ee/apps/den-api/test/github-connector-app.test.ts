...
describe("github connector app helpers", () => {
  ...
  test("lists repositories through the GitHub installation token flow", async () => {
    ...
    const repositories = await listGithubInstallationRepositories({
      ...
      fetchFn: async (url, init) => {
        ...
        if (String(url).endsWith("/contents/.claude-plugin/marketplace.json")) {
          if (String(url).includes("venom-cowork/workspace")) {
        ...
        return new Response(JSON.stringify({
          repositories: [
            { default_branch: "main", full_name: "venom-cowork/workspace", id: 42, private: true },
    ...
    expect(requests.map((request) => request.url)).toEqual([
      ...
      "https://api.github.com/repos/venom-cowork/workspace/contents/.claude-plugin/marketplace.json",
    ...
    expect(repositories).toEqual([
      { defaultBranch: "main", fullName: "venom-cowork/workspace", hasPluginManifest: true, id: 42, manifestKind: "marketplace", marketplacePluginCount: 3, private: true },
  ...
  test("builds install URLs and validates signed state tokens", async () => {
    const app = await getGithubAppSummary({
      ...
      fetchFn: async () => new Response(JSON.stringify({
        html_url: "https://github.com/apps/venomcowork-test",
        name: "VenomCowork Test",
        slug: "venomcowork-test",
      }), { status: 200 }),
    })
    ...
    expect(buildGithubAppInstallUrl({ app, state: token })).toBe(`https://github.com/apps/venomcowork-test/installations/new?state=${encodeURIComponent(token)}`)
  ...
  test("validates repository identity and branch existence against GitHub", async () => {
    const result = await validateGithubInstallationTarget({
      ...
      fetchFn: async (url) => {
        ...
        if (String(url).endsWith("/repos/venom-cowork/workspace")) {
          return new Response(JSON.stringify({
            default_branch: "main",
            full_name: "venom-cowork/workspace",
            id: 42,
          }), { status: 200 })
        }

        if (String(url).endsWith("/repos/venom-cowork/workspace/branches/main")) {
      ...
      installationId: 777,
      ref: "refs/heads/main",
      repositoryFullName: "venom-cowork/workspace",
      repositoryId: 42,
    })
