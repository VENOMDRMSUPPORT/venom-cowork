 ...
 * been built to `dist/`):
 *
 *   docker exec -e VENOMCOWORK_DEV_MODE=0 \
 *     venomcowork-den-dev-<id>-den-1 \
...
try {
  ...
  if (process.env.VENOMCOWORK_DEV_MODE === "1" || !process.env.VENOMCOWORK_DEV_MODE) {
    console.log("[smoke] ok dev_mode_noop (no email sent, no throw â€” expected)")
    process.exit(0)
  }
