import { and, eq, inArray } from "@venom-cowork-ee/den-db/drizzle"
import {
  AuthUserTable,
  ...
  TeamMemberTable,
  TeamTable,
} from "@venom-cowork-ee/den-db/schema"
...
const DEMO_OWNER_PASSWORD = process.env.DEN_DEMO_OWNER_PASSWORD?.trim() || "VenomCoworkDemo123!"
...
const demoPlugins: DemoPlugin[] = [
  ...
  {
    description: "Streamline engineering workflows â€” standups, code review, architecture decisions, incident response, and technical documentation.",
    slug: "engineering",
    teamAccess: ["Engineering", "Product"],
  },
  {
    description: "Accelerate design workflows â€” critique, design system management, UX writing, accessibility audits, research synthesis, and dev handoff.",
    slug: "design",
    teamAccess: ["Design", "Product"],
  },
  {
    description: "Optimize business operations â€” vendor management, process documentation, change management, capacity planning, and compliance tracking.",
    slug: "operations",
    teamAccess: ["Operations", "Finance", "Human Resources"],
  },
  {
    description: "Streamline people operations â€” recruiting, onboarding, performance reviews, compensation analysis, and policy guidance.",
...
function assertSafeDevTarget() {
  if (!env.devMode) {
    throw new Error("Refusing to seed demo data unless VENOMCOWORK_DEV_MODE=1.")
  }
  if (env.dbMode !== "mysql") {
...
function githubHeaders() {
  return {
    ...
    "User-Agent": "venomcowork-den-demo-seed",
...
async function resetDemoOrg() {
  const existing = await db.select().from(OrganizationTable).where(eq(OrganizationTable.slug, DEMO_ORG_SLUG)).limit(1)
  if (!existing[0]) {
    log("âŠ˜", "no existing demo org to reset")
    return
  }
  const orgId = existing[0].id
  log("â†»", `resetting demo org ${orgId}â€¦`)
...
async function seedPlugins(input: {
  ...
  for (const plugin of demoPlugins) {
    ...
    seededPlugins++
    log("âœ“", `plugin ${seededPlugins}/${demoPlugins.length}: ${plugin.slug} (${contentObjects.length} objects)`)
  }
...
async function main() {
  ...
  console.log()
  console.log(`  den demo seed Â· ${DEMO_ORG_NAME}`)
  console.log(`  ${"â”€".repeat(40)}`)
  log("â—ˆ", `org slug: ${DEMO_ORG_SLUG}`)
  log("â—ˆ", `database: ${env.databaseUrl?.replace(/:[^@]*@/, ":***@") ?? "unknown"}`)
  log("â—ˆ", `github fetch: ${SHOULD_FETCH_GITHUB ? "enabled" : "disabled"}`)
  if (RESET_MODE) log("â—ˆ", "reset mode: will delete existing demo org first")
  console.log()
  ...
  log("â€¦", "creating owner account")
  const ownerUserId = await ensureSignedInOwnerUser()
  log("âœ“", `owner: ${DEMO_OWNER_EMAIL}`)

  log("â€¦", "creating organization")
  const organizationId = await ensureOrganization(ownerUserId)
  log("âœ“", `org: ${organizationId}`)
  console.log()

  log("â€¦", `seeding ${demoPeople.length} users and teams`)
  const { memberIdsByEmail, teamIdsByName } = await seedPeopleAndTeams(organizationId)
  log("âœ“", `${memberIdsByEmail.size} members Â· ${teamIdsByName.size} teams Â· ${pendingInvites.length} pending invites`)
  console.log()
  ...
  console.log(`  ${"â”€".repeat(40)}`)
  log("âœ“", `done in ${elapsedSeconds}s`)
  log(" ", `${memberIdsByEmail.size} members Â· ${teamIdsByName.size} teams Â· ${seededPlugins} plugins Â· ${seededObjects} config objects`)
  console.log()
  log("â†’", `login: ${DEMO_OWNER_EMAIL} / ${DEMO_OWNER_PASSWORD}`)
  log("â†’", "open: /organization or /dashboard")
  console.log()
}
