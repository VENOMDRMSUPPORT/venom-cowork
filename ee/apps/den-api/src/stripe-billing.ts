...
import { and, eq, isNull, sql } from "@venom-cowork-ee/den-db/drizzle"
import {
  MemberTable,
...
export async function createOrgSubscriptionCheckoutSession(input: {
  ...
  const venomcoworkProduct = input.subscriptionType === SEAT_SUBSCRIPTION_TYPE ? "venomcowork_seats" : "venomcowork_models"
  const metadata = {
    org_id: input.organizationId,
    created_by_org_member_id: input.orgMemberId,
    venomcowork_product: venomcoworkProduct,
  ...
  const subscription = await stripe().subscriptions.create(
    {
      ...
      metadata: {
        ...
        venomcowork_product: "venomcowork_seats",
        subscription_type: SEAT_SUBSCRIPTION_TYPE,
      },
    },
    { idempotencyKey: `venomcowork-seat-subscription-${session.id}` },
  )
