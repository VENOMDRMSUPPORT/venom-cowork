...
import { and, eq, inArray, isNull, sql } from "@venom-cowork-ee/den-db/drizzle"
import {
  InferenceKeyTable,
...
import { createDenTypeId, type DenTypeId } from "@venom-cowork-ee/utils/typeid"
import {
  INFERENCE_RESET_STRATEGY_BY_WINDOW_TYPE,
  INFERENCE_TIER_LIMITS,
  INFERENCE_WINDOW_DURATIONS_MS,
} from "@venom-cowork/types/den/inference"
import type { InferenceOrganizationMetadata, InferenceTier, InferenceWindowType } from "@venom-cowork/types/den/inference"
...
function buildVenomCoworkProviderConfig() {
  return {
    id: VENOMCOWORK_PROVIDER_ID,
    name: "VenomCowork",
    npm: "@openrouter/ai-sdk-provider",
    env: ["VENOMCOWORK_API_KEY"],
    doc: "VenomCowork-managed inference proxy for organization models.",
...
async function deleteVenomCoworkProviders(where: { organizationId: OrgId; memberId?: MemberId }) {
  const providerWhere = where.memberId
    ? and(
        ...
        eq(LlmProviderTable.source, "venomcowork"),
        eq(LlmProviderTable.providerId, VENOMCOWORK_PROVIDER_ID),
      )
    : and(
        eq(LlmProviderTable.organizationId, where.organizationId),
        eq(LlmProviderTable.source, "venomcowork"),
...
async function ensureVenomCoworkLlmProviderForMember(input: { organizationId: OrgId; memberId: MemberId; inferenceKey: string }) {
  const now = new Date()
  ...
  const providerConfig = buildVenomCoworkProviderConfig()
  ...
  await db.transaction(async (tx) => {
    if (providerRows[0]) {
      await tx
        ...
        .set({ name: "VenomCowork Models", providerConfig, apiKey: input.inferenceKey, updatedAt: now })
      ...
      await tx.insert(LlmProviderTable).values({
        ...
        source: "venomcowork",
        providerId: VENOMCOWORK_PROVIDER_ID,
        name: "VenomCowork Models",
        providerConfig,
...
async function ensureMemberInferenceAccess(input: { organizationId: OrgId; memberId: MemberId }) {
  ...
  await ensureVenomCoworkLlmProviderForMember({ ...input, inferenceKey: key })
}
...
export async function syncInferenceAfterMemberChange(input: {
  ...
  if (input.change === "removed") {
    ...
    await deleteVenomCoworkProviders({ organizationId: input.organizationId, memberId: input.memberId })
  }
...
async function createOpenRouterOrgApiKey(input: { organizationId: OrgId }) {
  ...
  const body: Record<string, unknown> = {
    name: `VenomCowork org ${input.organizationId}`,
...
export async function setInferenceEnabled(input: { organizationId: OrgId; enabled: boolean; tier?: InferenceTier }) {
  ...
  if (!input.enabled) {
    ...
    await deleteVenomCoworkProviders({ organizationId: input.organizationId })
