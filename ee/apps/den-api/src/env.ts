...
const EnvSchema = z.object({
  ...
  VENOMCOWORK_DEV_MODE: z.string().optional(),
  ...
  VENOMCOWORK_DAYTONA_ENV_PATH: z.string().optional(),
  ...
  RENDER_WORKER_VENOMCOWORK_VERSION: z.string().optional(),
  ...
  DAYTONA_VENOMCOWORK_PORT: z.string().optional(),
...
const devMode = (parsed.VENOMCOWORK_DEV_MODE ?? "0").trim() === "1"
...
export const env = {
  ...
  render: {
    ...
    workerRepo:
      parsed.RENDER_WORKER_REPO ?? "https://github.com/venom-cowork/workspace",
  ...
  daytona: {
    envPath: optionalString(parsed.VENOMCOWORK_DAYTONA_ENV_PATH),
    ...
    workerProxyBaseUrl:
      optionalString(parsed.DAYTONA_WORKER_PROXY_BASE_URL) ?? "https://workers.den.venomcoworklabs",
    sandboxNamePrefix:
    ...
    dataMountPath:
      optionalString(parsed.DAYTONA_DATA_MOUNT_PATH) ?? "/persist/venomcowork",
    runtimeWorkspacePath:
      optionalString(parsed.DAYTONA_RUNTIME_WORKSPACE_PATH) ??
      "/tmp/venomcowork-workspace",
    runtimeDataPath:
      optionalString(parsed.DAYTONA_RUNTIME_DATA_PATH) ?? "/tmp/venomcowork-data",
    sidecarDir:
      optionalString(parsed.DAYTONA_SIDECAR_DIR) ?? "/tmp/venomcowork-sidecars",
    venomcoworkPort: Number(parsed.DAYTONA_VENOMCOWORK_PORT ?? "8787"),
