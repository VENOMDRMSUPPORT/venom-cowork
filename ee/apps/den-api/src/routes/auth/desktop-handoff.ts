...
function isWebAppHost(hostname: string) {
  ...
  return normalized === "app.venomcowork.local"
    || normalized === "app.venomcowork.software"
...
function resolveDesktopDenBaseUrl(request: Request) {
  ...
  if (!targetHost) {
    return "https://app.venomcowork.local/api/den"
  }
...
function buildVenomcoworkDeepLink(input: {
  ...
  const requestedScheme = input.scheme?.trim() || "venomcowork"
...
export function registerDesktopAuthRoutes<T extends { Variables: AuthContextVariables }>(app: Hono<T>) {
  app.post(
    ...
    describeRoute({
      ...
      description: "Creates a short-lived desktop handoff grant and deep link so a signed-in web user can continue the same account in the VenomCowork desktop app.",
      responses: {
