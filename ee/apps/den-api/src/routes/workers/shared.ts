...
import { and, asc, desc, eq, isNull } from "@venom-cowork-ee/den-db/drizzle"
import {
  AuditEventTable,
  AuthUserTable,
  DaytonaSandboxTable,
...
function parseWorkspaceSelection(payload: unknown): { workspaceId: string; venomcoworkUrl: string } | null {
  ...
  return {
    workspaceId,
    venomcoworkUrl: `${baseUrl}/w/${encodeURIComponent(workspaceId)}`,
  }
}
    ...
    try {
      const response = await fetch(`${normalizeUrl(candidate)}${input.path}`, {
        ...
        headers: {
          ...
          "X-VenomCowork-Host-Token": access.hostToken,
        },
...
export async function getWorkerTokensAndConnect(worker: WorkerRow) {
  ...
  return {
    ...
    connect: connect ?? (instance?.url ? { venomcoworkUrl: instance.url, workspaceId: null } : null),
  }
}
