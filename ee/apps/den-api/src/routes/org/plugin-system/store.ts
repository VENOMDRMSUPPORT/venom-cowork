import { and, asc, desc, eq, inArray, isNull } from "@venom-cowork-ee/den-db/drizzle"
import {
...
const DEFAULT_VENOMCOWORK_MARKETPLACE_NAME = "VenomCowork Marketplace"
const DEFAULT_VENOMCOWORK_MARKETPLACE_DESCRIPTION = "Built-in VenomCowork AI capabilities available in the desktop app after sign-in."
...
const DEFAULT_VENOMCOWORK_EXTENSION_MANIFESTS = [
  {
    schemaVersion: 1,
    ...
    setup: { instructions: "VenomCowork Browser is ready by default in desktop workspaces." },
    ...
    enablement: [{ type: "toggle-enabled", ref: "venomcowork-browser", label: "Enabled" }],
  ...
  },
  {
    ...
    resources: [
      { type: "mcp", id: "computer-use-mcp", label: "Computer Use MCP", mcpServerName: "computer-use", command: ["npx", "-y", "@venom-cowork/handsfree", "mcp"], localCommandRef: "venomcowork.computerUseMcp", required: true },
      { type: "native-binary", id: "computer-use-native", label: "macOS accessibility runtime", packageName: "@venom-cowork/handsfree", required: true },
    ],
    contributions: [
  ...
  },
  {
    ...
    setup: { instructions: "Add an OpenAI API key, then agents can generate image artifacts through VenomCowork extension actions." },
    resources: [
  ...
  },
  {
    ...
    setup: { instructions: "Connect your Google account to use Calendar, Drive, and Gmail drafts in VenomCowork." },
    resources: [
...
function defaultVenomCoworkManifestForPlugin(row: PluginRow) {
  return DEFAULT_VENOMCOWORK_EXTENSION_MANIFESTS.find((manifest) => manifest.name === row.name && manifest.description === row.description) ?? null
}
...
function serializePluginExtension(row: PluginRow, componentCounts: Record<string, number>) {
  const builtInManifest = defaultVenomCoworkManifestForPlugin(row)
  if (builtInManifest) {
    return {
      ...
      sourceFormat: "venomcowork-builtin",
    }
  }
  ...
  return {
    ...
    manifest: {
      ...
      setup: {
        instructions: "Imported from a Claude-compatible plugin. VenomCowork installs its resources into this workspace as extension components.",
      },
      lifecycle: {
...
export async function listMarketplaces(input: { context: PluginArchActorContext; cursor?: string; limit?: number; q?: string; status?: MarketplaceRow["status"] }) {
  await ensureDefaultVenomCoworkMarketplace(input.context)
...
async function ensureDefaultVenomCoworkMarketplace(context: PluginArchActorContext) {
  const organizationId = context.organizationContext.organization.id
  ...
  const marketplace = await ensureDefaultMarketplace({
    context,
    createdAt: now,
    description: DEFAULT_VENOMCOWORK_MARKETPLACE_DESCRIPTION,
    name: DEFAULT_VENOMCOWORK_MARKETPLACE_NAME,
  })

  for (const manifest of DEFAULT_VENOMCOWORK_EXTENSION_MANIFESTS) {
    let plugin = (await db
      .select()
      .from(PluginTable)
      .where(and(
