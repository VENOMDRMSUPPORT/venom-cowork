...
async function provisionWorkerOnRender(
  ...
  const startCommand = [
    ...
    "attempt=0; while [ $attempt -lt 3 ]; do attempt=$((attempt + 1)); venomcowork serve --workspace /tmp/workspace --remote-access --venomcowork-port ${PORT:-10000} --opencode-host 127.0.0.1 --opencode-port 4096 --connect-host 127.0.0.1 --cors '*' --approval manual --allow-external --opencode-source external --opencode-bin ./bin/opencode --no-opencode-router --verbose && exit 0; echo \"venomcowork serve failed (attempt $attempt); retrying in 3s\"; sleep 3; done; exit 1",
  ].join(" && ")
  ...
  const payload = {
    ...
    envVars: [
      { key: "VENOMCOWORK_TOKEN", value: input.clientToken },
      { key: "VENOMCOWORK_HOST_TOKEN", value: input.hostToken },
