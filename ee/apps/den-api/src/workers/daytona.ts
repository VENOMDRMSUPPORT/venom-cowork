...
import { eq } from "@venom-cowork-ee/den-db/drizzle"
import { DaytonaSandboxTable } from "@venom-cowork-ee/den-db/schema"
  ...
  do {
    ...
    const response = await fetch(url, {
      headers: {
        ...
        "X-Daytona-Source": "venomcowork-den-api",
      },
    })
...
function sandboxLabels(workerId: WorkerId) {
  return {
    "venomcowork.den.provider": "daytona",
    "venomcowork.den.worker-id": workerId,
  }
}
...
function buildVenomCoworkStartCommand(input: ProvisionInput) {
  const verifyRuntimeStep = [
    "if ! command -v venomcowork >/dev/null 2>&1; then echo 'venomcowork binary missing from Daytona runtime image; rebuild and republish the Daytona snapshot' >&2; exit 1; fi",
  ...
  const venomcoworkServe = [
    "VENOMCOWORK_DATA_DIR=",
    shellQuote(env.daytona.runtimeDataPath),
    " VENOMCOWORK_SIDECAR_DIR=",
    ...
    ` --remote-access`,
    ` --venomcowork-port ${env.daytona.venomcoworkPort}`,
...
while [ "$attempt" -lt 3 ]; do
  attempt=$((attempt + 1))
  if ${venomcoworkServe}; then
    exit 0
  fi
  status=$?
  echo "venomcowork serve failed (attempt $attempt, exit $status); retrying in 3s"
  sleep 3
done
exit 1
`.trim()
  ...
  while (Date.now() - startedAt < timeoutMs) {
    ...
    try {
      ...
      if (typeof command.exitCode === "number" && command.exitCode !== 0) {
        ...
        throw new Error(
          [
            `venomcowork session exited with ${command.exitCode}`,
      ...
      if (error instanceof Error && error.message.startsWith("venomcowork session exited")) {
        throw error
      }
    }
...
export async function refreshDaytonaSignedPreview(workerId: WorkerId) {
  ...
  const preview = await sandbox.getSignedPreviewUrl(env.daytona.venomcoworkPort, expiresInSeconds)
...
export async function provisionWorkerOnDaytona(
  ...
  try {
    ...
    const sessionId = `venomcowork-${workerHint(input.workerId)}`
    await sandbox.process.createSession(sessionId)
    const command = await sandbox.process.executeSessionCommand(
      sessionId,
      {
        command: buildVenomCoworkStartCommand(input),
        runAsync: true,
      },
      0,
    )
