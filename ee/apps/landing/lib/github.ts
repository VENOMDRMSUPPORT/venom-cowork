...
export const getGithubData = async () => {
  ...
  const [repo, latestRelease, releases] = await Promise.all([
    fetchJson<Repo>("https://api.github.com/repos/venom-cowork/workspace"),
    fetchJson<Release>(
      "https://api.github.com/repos/venom-cowork/workspace/releases/latest"
    ),
    fetchJson<Release[]>(
      "https://api.github.com/repos/venom-cowork/workspace/releases?per_page=50"
    )
  ]);
  ...
  const isElectronDesktopAsset = (name: string) =>
    name.startsWith("venomcowork-mac-") ||
    name.startsWith("venomcowork-win-") ||
    name.startsWith("venomcowork-linux-");
  ...
  const hasWindowsDesktopAsset = (release: Release) => {
    ...
    return assets.some((asset) => {
      const name = String(asset?.name || "").toLowerCase();
      return name.startsWith("venomcowork-win-x64-") && name.endsWith(".exe");
    });
  };
  ...
  const exe = selectAsset(windowsAssets, [".exe"], ["venomcowork-win-"]);
