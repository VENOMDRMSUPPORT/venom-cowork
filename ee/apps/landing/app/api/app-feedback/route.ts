...
import { EmailSendError, sendEmail, type FeedbackEmailProps } from "@venom-cowork/email";
...
type FeedbackContext = {
  ...
  deployment?: string;
  appVersion?: string;
  venomcoworkServerVersion?: string;
...
const DEFAULT_INTERNAL_FEEDBACK_EMAIL = "team@venomcowork.local";
...
function sanitizeContext(input: FeedbackContext | undefined) {
  return {
    ...
    venomcoworkServerVersion: sanitizeValue(input?.venomcoworkServerVersion),
...
function formatDiagnosticsSummary(context: ReturnType<typeof sanitizeContext>) {
  ...
  const lines = [
    ...
    ["VenomCowork server", context.venomcoworkServerVersion],
...
export async function POST(request: Request) {
  ...
  const internalEmail =
    process.env.VENOMCOWORK_FEEDBACK_EMAIL?.trim() ||
