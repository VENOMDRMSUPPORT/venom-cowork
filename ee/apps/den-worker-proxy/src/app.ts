...
import { and, eq, isNull } from "@venom-cowork-ee/den-db/drizzle"
import { createDenDb, DaytonaSandboxTable, RateLimitTable, WorkerTokenTable } from "@venom-cowork-ee/den-db"
...
const publicCorsAllowHeaders = [
  "Authorization",
  "Content-Type",
  "X-VenomCowork-Host-Token",
  "X-VenomCowork-Client-Id",
...
async function resolveWorkerTokenScope(workerId: WorkerId, request: Request): Promise<WorkerTokenScope | "invalid" | null> {
  const hostToken = request.headers.get("x-venomcowork-host-token")?.trim() || null
...
app.all("*", async (c) => {
  ...
  if (requestUrl.pathname === "/") {
    return Response.redirect("https://venomcowork.local", 302)
  }
