# VenomCowork Cloud App (`ee/apps/den-web`)

Frontend for `app.venomcowork.local`.
...
- Offers desktop handoff actions so users can open the generated worker directly in VenomCowork or copy the connect credentials manually.
- Uses a Next.js proxy route (`/api/den/*`) to reach `api.venomcowork.local` without browser CORS issues.
- Uses a same-origin auth proxy (`/api/auth/*`) so GitHub OAuth callbacks can land on `app.venomcowork.local`.
...
1. Install workspace deps from repo root:
   `pnpm install`
2. Run the app:
   `pnpm --filter @venom-cowork-ee/den-web dev`
3. Open:
   `http://localhost:3005`
...
- `DEN_API_BASE` (server-only): upstream API base used by proxy route.
  - default: `https://api.venomcowork.local`
- `DEN_AUTH_ORIGIN` (server-only): Origin header sent to Better Auth endpoints when the browser request does not include one.
  - default: `https://app.venomcowork.local`
- `DEN_AUTH_FALLBACK_BASE` (server-only): fallback Den origin used if `DEN_API_BASE` serves an HTML/5xx error.
  - default: `https://den-control-plane-venomcowork.onrender.com`
- `NEXT_PUBLIC_VENOMCOWORK_APP_CONNECT_URL` (client): Base URL for "Open in App" links.
  - Example: `https://venomcowork.local/app`
  - The web panel appends `/connect-remote` and injects worker URL/token params automatically.
- `NEXT_PUBLIC_VENOMCOWORK_AUTH_CALLBACK_URL` (client): Canonical URL used for GitHub auth callback redirects.
  - default: `https://app.venomcowork.local`
...
- Build command: `cd ../../.. && pnpm --filter @venom-cowork-ee/den-web build`
- Output directory: `.next`
- Install command: `cd ../../.. && pnpm install --frozen-lockfile`

These commands should be configured in the Vercel dashboard rather than committed in `vercel.json`, so the app still builds from the monorepo root and can resolve shared workspace packages like `@venom-cowork-ee/utils`.

Then assign custom domain:

- `app.venomcowork.local`
