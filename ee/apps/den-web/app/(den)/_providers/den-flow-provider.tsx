...
type DenFlowContextValue = {
  ...
  events: LaunchEvent[];
  venomcoworkDeepLink: string | null;
  venomcoworkAppConnectUrl: string | null;
...
export function DenFlowProvider({ children }: { children: ReactNode }) {
  ...
  const [desktopAuthScheme, setDesktopAuthScheme] = useState("venomcowork");
  ...
  async function finalizeEmailPasswordSignIn(
    ...
    if (desktopAuthRequested) {
      setAuthInfo("Signed in. Returning to VenomCowork...");
      return null;
    }
  ...
  async function withResolvedOpenworkCredentials(candidate: WorkerLaunch, options: { quiet?: boolean } = {}) {
    ...
    if (existingConnectUrl && existingWorkspaceId) {
      return {
        ...candidate,
        venomcoworkUrl: existingConnectUrl,
    ...
    if (!accessToken) {
      ...
      return {
        ...candidate,
        venomcoworkUrl: instanceUrl.trim().replace(/\/+$/, ""),
    ...
    try {
      ...
      if (resolved) {
        return {
          ...candidate,
          venomcoworkUrl: resolved.venomcoworkUrl,
    ...
    try {
      const { response, payload } = await requestJson(
        `/v1/workers/${encodeURIComponent(targetWorkerId)}/runtime/upgrade`,
        {
          method: "POST",
          headers: authToken ? { Authorization: `Bearer ${authToken}` } : undefined,
          body: JSON.stringify({ services: ["venomcowork-server", "opencode"] })
        },
        12000
      );
  ...
  async function completeDesktopAuthHandoff() {
    ...
    try {
      ...
      const venomcoworkPayload = payload as { venomcoworkUrl?: unknown } | null;
      const venomcoworkUrl = typeof venomcoworkPayload?.venomcoworkUrl === "string" ? venomcoworkPayload.venomcoworkUrl.trim() : "";
      if (!venomcoworkUrl) {
        setAuthError("Desktop handoff succeeded, but no VenomCowork redirect URL was returned.");
        return;
      }

      setDesktopRedirectUrl(venomcoworkUrl);
      window.location.assign(venomcoworkUrl);
    } catch (error) {
      setAuthError(error instanceof Error ? error.message : "Failed to open VenomCowork.");
    } finally {
      ...
      const nextWorker: WorkerLaunch =
        worker && worker.workerId === summary.workerId
          ...
          : {
              ...
              provider: summary.provider,
              instanceUrl: summary.instanceUrl,
              venomcoworkUrl: summary.instanceUrl,
              workspaceId: null,
              clientToken: null,
              ownerToken: null,
              hostToken: null
    ...
    try {
      ...
      const nextWorker: WorkerLaunch =
        worker && worker.workerId === id
          ? {
              ...worker,
              venomcoworkUrl: tokens.venomcoworkUrl ?? worker.venomcoworkUrl,
  ...
  useEffect(() => {
    ...
    try {
      ...
      const restored: WorkerLaunch = {
        ...parsed,
        venomcoworkUrl: parsed.venomcoworkUrl ?? parsed.instanceUrl,
