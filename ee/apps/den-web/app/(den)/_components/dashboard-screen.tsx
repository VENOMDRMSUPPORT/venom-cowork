...
export function DashboardScreen({ showSidebar = true }: { showSidebar?: boolean }) {
  ...
  const webDisabled = !venomcoworkAppConnectUrl || !isReady;
  const desktopDisabled = !venomcoworkDeepLink || !isReady;
  const showConnectionHint = !venomcoworkDeepLink || !hasWorkspaceScopedUrl;
                ...
                <div className="flex max-w-xl flex-col gap-6">
                  ...
                  <p className="text-[16px] leading-relaxed text-[var(--dls-text-secondary)] md:text-[17px]">
                    {isReady
                      ...
                      : "We are allocating resources and preparing the VenomCowork connection before unlocking the rest of the controls."}
                  </p>
                ...
                <div className="flex w-full flex-col gap-3 md:w-[200px] md:shrink-0 md:items-end">
                  ...
                  <div className="hidden w-full flex-col items-center md:flex">
                    <button
                      ...
                      onClick={() => {
                        if (!desktopDisabled && venomcoworkDeepLink) {
                          window.location.href = venomcoworkDeepLink;
                        }
                      }}
                    ...
                    <span className="mt-2 text-[11px] font-medium text-[var(--dls-text-secondary)]">requires the VenomCowork desktop app</span>
                  </div>
                </div>
              </div>
            </div>
                  ...
                  <div className="px-8 pb-8 pt-0">
                    <div className="flex flex-wrap gap-2">
                      <button
                        ...
                        onClick={() => {
                          if (venomcoworkDeepLink) {
                            window.location.href = venomcoworkDeepLink;
                          }
                        }}
                        disabled={!venomcoworkDeepLink || !isReady}
                      >
                        {venomcoworkDeepLink ? "Open in Desktop" : "Preparing connection..."}
                      </button>
                      <button
                        type="button"
                    ...
                    <div className="mt-6 space-y-4">
                      <CredentialRow
                        ...
                        value={activeWorker?.venomcoworkUrl ?? activeWorker?.instanceUrl ?? null}
                        placeholder="Connection URL is still preparing..."
                        hint={showConnectionHint ? (!venomcoworkDeepLink ? "Getting connection details ready..." : "Finishing your workspace URL...") : undefined}
                        canCopy={Boolean(activeWorker?.venomcoworkUrl ?? activeWorker?.instanceUrl)}
                        copied={copiedField === "venomcowork-url"}
                        onCopy={() => void copyToClipboard("venomcowork-url", activeWorker?.venomcoworkUrl ?? activeWorker?.instanceUrl ?? null)}
                        muted={!isReady}
                      />
                            ...
                            <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                              <div>
                                ...
                                <p className="text-xs text-[var(--dls-text-secondary)]">
                                  Installed {service.actualVersion ?? "unknown"} Â· Target {service.targetVersion ?? "unknown"}
                                </p>
                              </div>
          ...
          <div className="flex flex-col gap-4 md:gap-6">
            <div className="flex items-center justify-between gap-3">
              <div className="flex items-center gap-2">
                ...
                <span className="text-lg font-semibold tracking-tight text-[var(--dls-text-primary)]">VenomCowork</span>
              </div>
              <button
                type="button"
