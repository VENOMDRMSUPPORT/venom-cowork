...
async function fetchInferenceStatus() {
  ...
  if (!response.ok) {
    throw new Error(getErrorMessage(payload, `Failed to load VenomCowork Models status (${response.status}).`));
  }

  const parsed = parseInferenceStatus(payload);
  if (!parsed) {
    throw new Error("VenomCowork Models status response was incomplete.");
  }
    ...
    <div className="mx-auto max-w-[1100px] px-4 pb-10 pt-5 sm:px-6 md:px-8">
        ...
        <div className="grid gap-8 p-6 md:grid-cols-[1.5fr_1fr] md:p-8">
          <div>
            ...
            <h1 className="mt-4 max-w-[620px] text-[30px] font-semibold tracking-[-0.05em] text-white md:text-[38px]">
              Your access in {activeOrg?.name ?? "VenomCowork"}
            </h1>
            <p className="mt-3 max-w-[620px] text-[14px] leading-7 text-white/60">
              Marketplaces contain plugins. VenomCowork Marketplace is built in, and assigned marketplaces become available after you sign in to the app.
            </p>
          </div>
      ...
      <div className="mt-5 grid gap-5 lg:grid-cols-[1fr_1fr]">
        <section className="rounded-[28px] border border-gray-200 bg-white p-6 shadow-[0_18px_45px_-35px_rgba(15,23,42,0.35)]">
          <div className="flex items-center justify-between gap-4">
            <div>
              ...
              <p className="mt-1 text-[13px] text-gray-500">Custom providers you can use from VenomCowork.</p>
            </div>
        ...
        <section className="rounded-[28px] border border-gray-200 bg-white p-6 shadow-[0_18px_45px_-35px_rgba(15,23,42,0.35)]">
          <div className="flex items-center justify-between gap-4">
            <div>
              <h2 className="text-[18px] font-semibold tracking-[-0.03em] text-gray-950">VenomCowork Models</h2>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="mt-1 text-[12px] text-gray-500">
                    {inference?.subscribed === false ? "The workspace needs an active subscription before members can use VenomCowork Models." : `${inference?.memberCount ?? 0} member${inference?.memberCount === 1 ? "" : "s"} included in usage limits.`}
                  </p>
                </div>
              </div>
            </div>
