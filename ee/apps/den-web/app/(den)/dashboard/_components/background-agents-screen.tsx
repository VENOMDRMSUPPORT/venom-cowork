...
function SandboxCard({
  ...
  const connectionUrl = details?.venomcoworkUrl ?? sandbox.instanceUrl ?? null;
  const ownerToken = details?.ownerToken ?? null;
  const clientToken = details?.clientToken ?? null;
  const openWebUrl = details?.venomcoworkAppConnectUrl ?? null;
  const openDesktopUrl = details?.venomcoworkDeepLink ?? null;
  ...
  return (
    ...
    >
      ...
      {workersLoadedOnce && workersBusy ? (
        <p className="mt-4 text-[12px] text-gray-400">Refreshing workspacesâ€¦</p>
      ) : null}
    </DashboardPageTemplate>
  );
}
