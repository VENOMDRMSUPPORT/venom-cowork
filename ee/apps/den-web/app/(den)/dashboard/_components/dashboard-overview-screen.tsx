...
const FALLBACK_RELEASE = "https://github.com/venom-cowork/workspace/releases";
...
async function fetchInstallers(): Promise<{ installers: Installers; releaseTag: string; releaseUrl: string }> {
  ...
  try {
    const res = await fetch("https://api.github.com/repos/venom-cowork/workspace/releases/latest", {
    ...
    const dmg = selectAsset(assets, [".dmg"], ["venomcowork-mac-"]);
  ...
  return (
    <div className="mx-auto max-w-[1100px] px-4 pb-8 pt-4 sm:px-6 md:px-8">
      ...
      <div className="flex flex-wrap items-center gap-2.5 border-b border-[#e7e9f0] pb-3">
        <span className="text-[14px] font-semibold tracking-[-0.01em] text-[#07192C]">{activeOrg?.name ?? "VenomCowork Cloud"}</span>
      ...
      <section className="mt-5 rounded-[18px] border border-[#d7e2f5] bg-gradient-to-br from-[#F4F8FF] to-[#EEF3FF] p-5">
        ...
        <p className="mt-1.5 text-[13px] leading-6 text-[#526582]">
          Sign in with this account to get Computer Use, Browser, Image Gen, Google Workspace, and your team&apos;s marketplace extensions â€” all built in.
        </p>
      ...
      <section className="mt-4 overflow-hidden rounded-[18px] border border-[#e3e7ee] bg-[#07192C]">
        <div className="px-6 py-5">
          <div className="flex items-center gap-2.5">
            ...
            <span className="text-[16px] font-semibold text-white">Download VenomCowork</span>
      ...
      <div className="mt-5 grid gap-3.5 md:grid-cols-2">
        <StatCard icon={<Users className="h-5 w-5 text-[#6F3DFF]" />} title="VenomCowork users" value={`${members}`} sub="Current workspace members" tone="violet" />
