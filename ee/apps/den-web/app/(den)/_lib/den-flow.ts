...
export type RuntimeServiceName = "venomcowork-server" | "opencode" | "opencode-router";
...
export const LAST_WORKER_STORAGE_KEY = "venomcowork:web:last-worker";
export const PENDING_SOCIAL_SIGNUP_STORAGE_KEY = "venomcowork:web:pending-social-signup";
export const AUTH_TOKEN_STORAGE_KEY = "venomcowork:web:auth-token";
export const ONBOARDING_INTENT_STORAGE_KEY = "venomcowork:web:onboarding-intent";
export const WORKER_STATUS_POLL_MS = DEN_WORKER_POLL_INTERVAL_MS;
export const DEFAULT_AUTH_NAME = "VenomCowork User";
export const DEFAULT_WORKER_NAME = "My Worker";
export const VENOMCOWORK_APP_CONNECT_BASE_URL = (process.env.NEXT_PUBLIC_VENOMCOWORK_APP_CONNECT_URL ?? "").trim();
export const VENOMCOWORK_AUTH_CALLBACK_BASE_URL = (process.env.NEXT_PUBLIC_VENOMCOWORK_AUTH_CALLBACK_URL ?? "").trim();
...
export function getSocialCallbackUrl(): string {
  try {
    const origin = typeof window !== "undefined" ? window.location.origin : VENOMCOWORK_AUTH_CALLBACK_BASE_URL || "https://app.venomcowork.local";
    ...
    return "https://app.venomcowork.local/";
  }
}
...
export function getWorker(payload: unknown): WorkerLaunch | null {
  ...
  return {
    ...
    venomcoworkUrl: instance && typeof instance.url === "string" ? instance.url : null,
    workspaceId: null,
...
export function getWorkerTokens(payload: unknown): WorkerTokens | null {
  ...
  const venomcoworkUrl = connect && typeof connect.venomcoworkUrl === "string" ? connect.venomcoworkUrl : null;
  ...
  return { clientToken, ownerToken, hostToken, venomcoworkUrl, workspaceId };
}
...
export function getRuntimeServiceLabel(name: RuntimeServiceName): string {
  switch (name) {
    case "venomcowork-server":
      return "VenomCowork server";
    case "opencode":
      return "OpenCode";
...
export function isWorkerLaunch(value: unknown): value is WorkerLaunch {
  ...
  return (
    ...
    (typeof value.venomcoworkUrl === "string" || value.venomcoworkUrl === null || typeof value.venomcoworkUrl === "undefined") &&
...
export function listItemToWorker(item: WorkerListItem, current: WorkerLaunch | null = null): WorkerLaunch {
  return {
    ...
    venomcoworkUrl: current?.workerId === item.workerId ? current.venomcoworkUrl ?? item.instanceUrl : item.instanceUrl,
...
export function buildVenomcoworkDeepLink(
  ...
  if (!venomcoworkUrl || !accessToken) {
    return null;
  }

  const params = new URLSearchParams({
    venomcoworkHostUrl: venomcoworkUrl,
    venomcoworkToken: accessToken,
    source: "venomcowork-web"
  });
...
export function buildVenomcoworkAppConnectUrl(
  ...
  if (!appConnectBaseUrl || !venomcoworkUrl || !accessToken) {
    return null;
  }
  ...
  connectUrl.searchParams.set("venomcoworkHostUrl", venomcoworkUrl);
  connectUrl.searchParams.set("venomcoworkToken", accessToken);
  if (options?.autoConnect) {
    connectUrl.searchParams.set("autoConnect", "1");
  }
  connectUrl.searchParams.set("source", "venomcowork-web");
...
export async function resolveVenomcoworkWorkspaceUrl(instanceUrl: string, accessToken: string): Promise<{ workspaceId: string; venomcoworkUrl: string } | null> {
  ...
  return {
    workspaceId,
    venomcoworkUrl: buildWorkspaceUrl(baseUrl, workspaceId)
  };
}
