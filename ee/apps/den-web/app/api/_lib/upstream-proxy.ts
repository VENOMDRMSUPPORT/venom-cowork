...
const DEFAULT_API_BASE = "https://api.venomcowork.local";
const DEFAULT_AUTH_ORIGIN = "https://app.venomcowork.local";
const DEFAULT_AUTH_FALLBACK_BASE = "https://den-control-plane-venomcowork.onrender.com";
...
const localDevCorsOrigins = process.env.VENOMCOWORK_DEV_MODE === "1"
...
function applyCorsHeaders(request: NextRequest, headers: Headers): void {
  ...
  headers.set("access-control-allow-headers", "Content-Type,Authorization,X-Api-Key,X-Request-Id,X-Requested-With,X-VenomCowork-Legacy-Org-Id");
