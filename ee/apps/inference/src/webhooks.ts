...
import { InferenceKeyTable, InferenceUsageLedgerBucketChargeTable, InferenceUsageLedgerEntryTable, InferenceOrgUsageBucketTable } from "@venom-cowork-ee/den-db"
import { createDenTypeId, normalizeDenTypeId } from "@venom-cowork-ee/utils/typeid"
import type { DenTypeId } from "@venom-cowork-ee/utils/typeid"
import { INFERENCE_USAGE_CONVERSION_FACTOR } from "@venom-cowork/types/den/inference"
...
function parseSpan(span: JsonRecord, resourceAttrs: JsonRecord, scopeAttrs: JsonRecord): ParsedSpan | null {
  ...
  const venomcoworkRequestId = stringAttr(attrs, ["trace.metadata.venomcowork_request_id", "trace.venomcowork_request_id", "metadata.venomcowork_request_id", "venomcowork_request_id", "trace_id"])
  ...
  if (!orgMembershipId || !inferenceKeyId || !venomcoworkRequestId || !upstreamModel || inputCost === null || outputCost === null) {
    return null
  }
...
async function ingestSpan(span: ParsedSpan) {
  ...
  const [existing] = await db.select({ id: InferenceUsageLedgerEntryTable.id }).from(InferenceUsageLedgerEntryTable)
    .where(and(eq(InferenceUsageLedgerEntryTable.external_job_id, span.venomcoworkRequestId), eq(InferenceUsageLedgerEntryTable.event_type, "openrouter_usage"))).limit(1)
  const entry = existing ?? await (async () => {
    ...
    await db.insert(InferenceUsageLedgerEntryTable).values({
      ...
      external_job_id: span.venomcoworkRequestId,
