...
function resolveOrchestratorPackageJson() {
  ...
  if (globalRoot.status === 0) {
    const candidate = resolve(globalRoot.stdout.trim(), "venomcowork-orchestrator", "package.json");
  ...
  throw new Error("Unable to locate venomcowork-orchestrator package.json");
}
...
const downloadUrl = process.env.VENOMCOWORK_OPENCODE_DOWNLOAD_URL?.trim()
  || (version
