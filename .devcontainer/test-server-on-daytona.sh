...
# test-server-on-daytona.sh â€” one-command Daytona Den server sandbox.
#
# Usage:
...
SANDBOX="venomcowork-server-$(date +%Y%m%d-%H%M%S)"
DAYTONA_SERVER_SNAPSHOT="${DAYTONA_SERVER_SNAPSHOT:-venomcowork-server}"
...
echo "==> Starting VenomCowork Den server stack..."
