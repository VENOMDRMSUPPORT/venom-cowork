...
# Start the VenomCowork desktop dev stack inside a Daytona/devcontainer sandbox.
...
if [ "${1:-}" = "--help" ] || [ "${1:-}" = "-h" ]; then
  printf '%s\n' \
    ...
    "" \
    "Starts the VenomCowork desktop dev stack inside a Daytona/devcontainer sandbox." \
...
cd "$REPO_DIR"
export VENOMCOWORK_WORKSPACE_DIR="$REPO_DIR"
export VENOMCOWORK_DEV_MODE="${VENOMCOWORK_DEV_MODE:-1}"
export DISPLAY="${DISPLAY:-:99}"
export ELECTRON_DISABLE_SANDBOX="${ELECTRON_DISABLE_SANDBOX:-1}"
export VENOMCOWORK_REACT_DEVTOOLS="${VENOMCOWORK_REACT_DEVTOOLS:-0}"
export VENOMCOWORK_ELECTRON_REMOTE_DEBUG_PORT="${VENOMCOWORK_ELECTRON_REMOTE_DEBUG_PORT:-9825}"
...
echo "==> Starting VenomCowork dev stack in background..."
bash "$ELECTRON_SCRIPT" --detach

echo "==> Waiting for Electron CDP on :$VENOMCOWORK_ELECTRON_REMOTE_DEBUG_PORT..."
for _ in $(seq 1 30); do
  if curl -sf "http://127.0.0.1:$VENOMCOWORK_ELECTRON_REMOTE_DEBUG_PORT/json/list" >/dev/null 2>&1; then
    echo "VenomCowork sandbox dev stack is ready."
