...
| **CDP Debug** | 9825 | Chrome DevTools Protocol â€” for app and browser automation |
...
```bash
bash .devcontainer/create-daytona-venomcowork-snapshot.sh   # one-time / refresh when deps change
bash .devcontainer/test-on-daytona.sh [branch-or-commit]
```

The test script creates a sandbox from the reusable `venomcowork-eval-vnc` snapshot
...
`venomcowork-eval-pnpm-store` volume so the image stays under Daytona's 20 GB limit.
...
Future Daytona test sandboxes mount `venomcowork-eval-secrets:/daytona-secrets`
...
The artifacts flow mounts `venomcowork-eval-artifacts:/daytona-artifacts`, starts a
...
Do not use the generic `daytona create https://github.com/venom-cowork/workspace`
...
```bash
bash .devcontainer/create-daytona-venomcowork-server-snapshot.sh  # one-time / refresh when deps change
...
2. `.devcontainer/create-daytona-venomcowork-snapshot.sh` bakes that image into
   `venomcowork-eval-vnc` without `node_modules`.
3. `/opt/venomcowork-daytona/start-daytona-vnc.sh` starts Xvfb, XFCE, x11vnc, and
   noVNC on display `:99`.
4. `test-on-daytona.sh` installs dependencies through the reusable
   `venomcowork-eval-pnpm-store` volume when `node_modules` is missing or the
   lockfile changed.
5. Vite serves the React UI on port 5173.
6. `/opt/venomcowork-daytona/start-daytona-electron.sh` sources optional secrets,
...
2. Sign up â†’ create org â†’ Org Settings â†’ UI Customization
3. Set overrides â†’ Save
4. In the **Electron app** (noVNC on port 6080):
   - Cloud â†’ developer mode â†’ base URL `http://localhost:3005`
   - Sign in â†’ Customization â†’ see locked toggles
...
Your Browser
    â”‚
    â”œâ”€â”€ :6080 noVNC â”€â”€â–¶ x11vnc â”€â”€â–¶ XFCE/Xvfb â”€â”€â–¶ Electron App
    â”‚                              â”‚
    â”‚                              â”œâ”€â”€ CDP :9825 (automatable)
    â”‚                              â””â”€â”€ Vite HMR :5173
    â”‚
    â”œâ”€â”€ :3005 Den Web (Next.js)
    â”‚
    â””â”€â”€ :8788 Den API (Hono) â”€â”€â–¶ MySQL :3306
```
