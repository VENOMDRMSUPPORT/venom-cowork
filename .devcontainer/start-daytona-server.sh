...
if [ -n "${VENOMCOWORK_WORKSPACE_DIR:-}" ]; then
  REPO_DIR="$VENOMCOWORK_WORKSPACE_DIR"
...
PNPM_STORE="${PNPM_STORE:-$REPO_DIR/.venomcowork-daytona/pnpm-store}"
...
export VENOMCOWORK_DEV_MODE="${VENOMCOWORK_DEV_MODE:-1}"
export DATABASE_URL="${DATABASE_URL:-mysql://root:password@127.0.0.1:3306/venomcowork_den}"
...
export NEXT_PUBLIC_VENOMCOWORK_AUTH_CALLBACK_URL="${NEXT_PUBLIC_VENOMCOWORK_AUTH_CALLBACK_URL:-$DEN_WEB_PUBLIC_URL}"
...
echo "==> Starting MySQL..."
run_root service mysql start >/tmp/venomcowork-mysql-service.log 2>&1 || run_root service mariadb start >/tmp/venomcowork-mysql-service.log 2>&1
...
"${MYSQL_ROOT_CMD[@]}" <<'SQL'
CREATE DATABASE IF NOT EXISTS venomcowork_den;
...
mkdir -p "$PNPM_STORE" .venomcowork-daytona
baseline=.venomcowork-daytona/pnpm-lock.sha256
...
pnpm --filter @venom-cowork-ee/den-db db:push > /tmp/den-db-push.log 2>&1
...
nohup env \
  ...
  VENOMCOWORK_DEV_MODE="$VENOMCOWORK_DEV_MODE" \
  NODE_OPTIONS="--conditions=development" \
  pnpm --filter @venom-cowork-ee/den-api exec tsx watch src/server.ts > /tmp/den-api.log 2>&1 &
...
nohup env \
  ...
  DAYTONA_VENOMCOWORK_PORT="${DAYTONA_VENOMCOWORK_PORT:-8787}" \
  DAYTONA_SIGNED_PREVIEW_EXPIRES_SECONDS="${DAYTONA_SIGNED_PREVIEW_EXPIRES_SECONDS:-86400}" \
  pnpm --filter @venom-cowork-ee/den-worker-proxy exec tsx watch src/server.ts > /tmp/den-worker-proxy.log 2>&1 &
...
nohup env \
  ...
  NEXT_PUBLIC_VENOMCOWORK_AUTH_CALLBACK_URL="$NEXT_PUBLIC_VENOMCOWORK_AUTH_CALLBACK_URL" \
  NEXT_PUBLIC_POSTHOG_KEY= \
  NEXT_PUBLIC_POSTHOG_API_KEY= \
  VENOMCOWORK_DEV_MODE="$VENOMCOWORK_DEV_MODE" \
  DEN_WEB_ALLOWED_DEV_ORIGINS="$DEN_WEB_ALLOWED_DEV_ORIGINS" \
  pnpm --filter @venom-cowork-ee/den-web exec next dev --hostname 0.0.0.0 --port "$DEN_WEB_PORT" > /tmp/den-web.log 2>&1 &

wait_for_http "http://127.0.0.1:$DEN_WEB_PORT/api/den/health" "Den Web" 180

cat > .venomcowork-daytona/server-env <<EOF
...
echo ""
echo "============================================"
echo "  VenomCowork Daytona server stack ready"
echo ""
