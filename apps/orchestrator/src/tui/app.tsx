...
const serviceCycle = ["all", "venomcowork-orchestrator", "opencode", "venomcowork-server", "router"];
        ...
        <box flexDirection="column" width={dimensions().width} height={dimensions().height} paddingLeft={2} paddingRight={2}>
          ...
          <Show when={state.view === "overview"}>
            <box flexDirection="row" gap={4} paddingTop={1}>
                ...
                <For each={state.services}>
                  {(service) => (
                    <box flexDirection="row" gap={1}>
                      <text fg={statusColor[service.status]}>â—</text>
              ...
              <box width={Math.floor(dimensions().width / 2) - 4} flexDirection="column" gap={1}>
                ...
                <text fg={theme.textMuted}>VenomCowork URL (LAN)</text>
                <text fg={theme.text}>{state.connect.venomcoworkUrl}</text>
                <text fg={theme.textMuted}>VenomCowork Collaborator Token</text>
                <text fg={theme.text}>{state.connect.venomcoworkToken}</text>
                <Show when={state.connect.ownerToken}>
                  <text fg={theme.textMuted}>VenomCowork Owner Token</text>
                  <text fg={theme.text}>{state.connect.ownerToken}</text>
                </Show>
                <text fg={theme.textMuted}>VenomCowork Host Admin Token</text>
          ...
          <Show when={state.view === "logs"}>
            <box flexDirection="column" paddingTop={1} gap={1}>
              <text fg={theme.textMuted}>
                Filters: {state.serviceFilter} Â· level: {state.levelFilter} Â· follow: {state.follow ? "on" : "off"}
              </text>
                    ...
                    <For each={state.routerTelegramIdentities?.items ?? []}>
                      {(item) => (
                        <text fg={item.running ? theme.success : theme.textMuted}>
                          {item.running ? "â—" : "â—‹"} {item.id} {item.enabled ? "" : "(disabled)"}
                        </text>
                      )}
                    </For>
