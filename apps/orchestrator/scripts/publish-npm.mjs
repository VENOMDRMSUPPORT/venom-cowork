...
if (!version) {
  throw new Error("venomcowork-orchestrator version missing in apps/orchestrator/package.json")
}
...
function platformPkgName(target) {
  ...
  return `venomcowork-orchestrator-${platform}-${arch}`
}
...
for (const target of targets) {
  ...
  const src = join(root, "dist", "bin", `venomcowork-${target.bun}${ext}`)
  if (!existsSync(src)) {
    throw new Error(`Missing venomcowork binary at ${src}. Run: pnpm --filter venomcowork-orchestrator build:bin:all`)
  }
  ...
  const dest = join(bindir, `venomcowork${ext}`)
  ...
  writeJson(join(dir, "package.json"), {
    name,
    version,
    description: "Platform binary for venomcowork-orchestrator",
    license: "MIT",
    os: [target.os],
    cpu: [target.cpu],
    bin: {
      venomcowork: `./bin/venomcowork${ext}`,
      "venomcowork-orchestrator": `./bin/venomcowork${ext}`,
    },
    files: ["bin"],
  })
...
const meta = join(outroot, "venomcowork-orchestrator")
mkdirSync(join(meta, "bin"), { recursive: true })

const wrapperSrc = join(root, "bin", "venomcowork")
if (!existsSync(wrapperSrc)) {
  throw new Error(`Missing wrapper at ${wrapperSrc}`)
}
copyFileSync(wrapperSrc, join(meta, "bin", "venomcowork"))
chmodSync(join(meta, "bin", "venomcowork"), 0o755)
...
writeJson(join(meta, "package.json"), {
  name: "venomcowork-orchestrator",
  version,
  description: "VenomCowork host orchestrator for opencode + VenomCowork server + opencode-router",
  license: "MIT",
  bin: {
    venomcowork: "./bin/venomcowork",
    "venomcowork-orchestrator": "./bin/venomcowork",
  },
  scripts: {
...
published.push({ name: "venomcowork-orchestrator", dir: meta })
