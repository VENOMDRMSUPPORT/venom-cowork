...
if (!orchestratorVersion) {
  throw new Error(
    "venomcowork-orchestrator version missing in apps/orchestrator/package.json",
  );
}
...
if (!serverVersion) {
  throw new Error(
    "venomcowork-server version missing in apps/server/package.json",
  );
}
...
run("pnpm", ["--filter", "venomcowork-server", "build:bin:all"], repoRoot);
...
const entries = {
  "venomcowork-server": { version: serverVersion, targets: {} },
  "opencode-router": { version: routerVersion, targets: {} },
};

for (const target of targets) {
  const ext = target.id.startsWith("windows") ? ".exe" : "";
  const serverSrc = join(serverDir, `venomcowork-server-${target.bun}${ext}`);
  if (!existsSync(serverSrc)) {
    throw new Error(`Missing venomcowork-server binary at ${serverSrc}`);
  }
  const serverDest = join(outdir, `venomcowork-server-${target.id}${ext}`);
  ...
  entries["venomcowork-server"].targets[target.id] = {
...
writeFileSync(
  join(outdir, "venomcowork-orchestrator-sidecars.json"),
