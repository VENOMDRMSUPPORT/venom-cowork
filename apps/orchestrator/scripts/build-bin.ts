...
const serverBin = resolve(root, "..", "server", "dist", "bin", "venomcowork-server");
...
await copyFile(serverBin, resolve(targetDir, "venomcowork-server"));
...
const versions = {
  "venomcowork-server": {
    ...
    sha256: await sha256(resolve(targetDir, "venomcowork-server")),
  },
  "opencode-router": {
