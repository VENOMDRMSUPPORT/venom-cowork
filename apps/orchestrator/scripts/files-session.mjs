...
async function runCli(args) {
  ...
  if (code !== 0) {
    throw new Error(stderr.trim() || `venomcowork CLI failed with code ${code}`);
  }
...
const root = await mkdtemp(join(tmpdir(), "venomcowork-file-session-"));
...
try {
  await waitFor(`${venomcoworkUrl}/health`);

  const workspaces = await fetchJson(`${venomcoworkUrl}/workspaces`, {
  ...
  assert.equal(diskContent, "updated by venomcowork cli\n");
  ...
  console.log(JSON.stringify({ ok: true, venomcoworkUrl, workspaceId, sessionId }, null, 2));
} catch (error) {
  console.error(
    JSON.stringify(
      {
