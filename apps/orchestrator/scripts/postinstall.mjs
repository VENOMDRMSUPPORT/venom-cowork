...
function binaryName() {
  ...
  return platform === "windows" ? "venomcowork.exe" : "venomcowork"
}

function fallbackAssetName() {
  const { platform, arch } = detect()
  return `venomcowork-bun-${platform}-${arch}${platform === "windows" ? ".exe" : ""}`
}
...
function readOwnVersion() {
  ...
  if (!version) {
    throw new Error("venomcowork-orchestrator: package version is missing")
  }
  return version
}

function resolveFallbackBaseUrl(version) {
  const override = String(process.env.VENOMCOWORK_ORCHESTRATOR_DOWNLOAD_BASE_URL || "").trim()
  if (override) {
...
async function downloadFallbackBinary() {
  ...
  console.log(`venomcowork-orchestrator: downloading fallback binary ${asset}`)
  ...
  if (binaryName() !== "venomcowork.exe") {
    chmodSync(destination, 0o755)
  }

  console.log(`venomcowork-orchestrator: installed fallback binary at ${destination}`)
}
...
async function main() {
  try {
    ...
    console.log(`venomcowork-orchestrator: verified platform package: ${pkg}`)
    return
  } catch {
    if (existsSync(fallbackBinaryPath())) {
      console.log(`venomcowork-orchestrator: using existing fallback binary at ${fallbackBinaryPath()}`)
      return
    }
  }
  ...
  try {
    ...
    console.error(
      `venomcowork-orchestrator: failed to locate platform binary package (${pkg}).\n` +
