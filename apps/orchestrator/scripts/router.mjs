...
async function runCli(args, dataDir) {
  ...
  if (code !== 0) {
    throw new Error(stderr.trim() || `venomcowork failed with code ${code}`);
  }
...
const root = await mkdtemp(join(tmpdir(), "venomcowork-orchestrator-router-"));
