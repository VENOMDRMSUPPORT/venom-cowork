...
const stopStaleSidecars = () => {
  ...
  const command = [
    `$targetDir = \"${targetDir}\"`,
    "$names = @('opencode.exe','venomcowork-server.exe','venomcowork-orchestrator.exe')",
...
const env = {
  ...
  VENOMCOWORK_DATA_DIR:
    process.env.VENOMCOWORK_DATA_DIR ||
    `${homedir()}${process.platform === "win32" ? "\\" : "/"}.venomcowork${process.platform === "win32" ? "\\" : "/"}venomcowork-orchestrator-dev`,
  VENOMCOWORK_USE_COREPACK_PNPM: "1",
