...
const forceBuild = hasFlag("--force") || process.env.VENOMCOWORK_SIDECAR_FORCE_BUILD === "1";
const sidecarOverride = process.env.VENOMCOWORK_SIDECAR_DIR?.trim() || readArg("--outdir");
...
const opencodeGithubRepo = (() => {
  const raw =
    ...
    process.env.VENOMCOWORK_OPENCODE_GITHUB_REPO?.trim() ||
    "anomalyco/opencode";
...
// venomcowork-server paths
const venomcoworkServerBaseName = "venomcowork-server";
const venomcoworkServerName = isWindowsTarget ? `${venomcoworkServerBaseName}.exe` : venomcoworkServerBaseName;
const venomcoworkServerPath = join(sidecarDir, venomcoworkServerName);
const venomcoworkServerBuildName = bunTarget
  ? `${venomcoworkServerBaseName}-${bunTarget}${bunTarget.includes("windows") ? ".exe" : ""}`
  : venomcoworkServerName;
const venomcoworkServerBuildPath = join(sidecarDir, venomcoworkServerBuildName);
const venomcoworkServerTargetTriple = resolvedTargetTriple;
const venomcoworkServerTargetName = venomcoworkServerTargetTriple
  ? `${venomcoworkServerBaseName}-${venomcoworkServerTargetTriple}${venomcoworkServerTargetTriple.includes("windows") ? ".exe" : ""}`
  : null;
const venomcoworkServerTargetPath = venomcoworkServerTargetName ? join(sidecarDir, venomcoworkServerTargetName) : null;

const venomcoworkServerDir = resolve(__dirname, "..", "..", "server");
...
// orchestrator paths
const orchestratorBaseName = "venomcowork-orchestrator";
...
adHocSignDarwinSidecars([
  opencodePath,
  opencodeTargetPath,
  // venomcowork-server runs in-process â€” no binary to sign.
  orchestratorBuildPath,
  orchestratorPath,
  orchestratorTargetPath,
]);

const venomcoworkServerVersion = (() => {
  try {
    const raw = readFileSync(resolve(venomcoworkServerDir, "package.json"), "utf8");
...
const versions = {
  ...
  "venomcowork-server": {
    version: venomcoworkServerVersion,
    sha256: "in-process",
  },
