...
run(pnpmCmd, ["--filter", "venomcowork-server", "build"], repoRoot);
// VENOMCOWORK_ELECTRON_BUILD tells Vite to emit relative asset paths so
// index.html resolves /assets/* correctly when loaded via file:// from
// inside the packaged .app bundle.
run(pnpmCmd, ["--filter", "@venom-cowork/app", "build"], repoRoot, {
  VENOMCOWORK_ELECTRON_BUILD: "1",
});
