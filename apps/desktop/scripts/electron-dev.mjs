...
const explicitStartUrl = process.env.VENOMCOWORK_ELECTRON_START_URL?.trim() || "";
...
// Build the server TS â†’ JS so Electron can import it in-process
console.log("[electron-dev] Building venomcowork-server (tsc)...");
runSync(pnpmCmd, ["--filter", "venomcowork-server", "build"], { cwd: repoRoot });
...
if (!viteReady) {
  uiChild = run(pnpmCmd, ["-w", "dev:ui"], {
    ...
    env: {
      ...process.env,
      PORT: String(devPort),
      VENOMCOWORK_DEV_MODE: process.env.VENOMCOWORK_DEV_MODE ?? "1",
      VENOMCOWORK_DATA_DIR: process.env.VENOMCOWORK_DATA_DIR ?? defaultDevDataDir,
    },
  });
}
...
// Set VENOMCOWORK_ELECTRON_REMOTE_DEBUG_PORT=9823 to enable.
const cdpPortRaw = process.env.VENOMCOWORK_ELECTRON_REMOTE_DEBUG_PORT?.trim() ?? "";
...
electronChild = run(pnpmCmd, ["exec", "electron", "./electron/main.mjs"], {
  ...
  env: {
    ...
    VENOMCOWORK_DEV_MODE: process.env.VENOMCOWORK_DEV_MODE ?? "1",
    VENOMCOWORK_DATA_DIR: process.env.VENOMCOWORK_DATA_DIR ?? defaultDevDataDir,
    VENOMCOWORK_ELECTRON_START_URL: resolvedStartUrl,
    ...(cdpPort ? { VENOMCOWORK_ELECTRON_REMOTE_DEBUG_PORT: cdpPort } : {}),
  },
});

if (cdpPort) {
  console.log(`[venomcowork] Electron CDP exposed at http://127.0.0.1:${cdpPort}`);
}
