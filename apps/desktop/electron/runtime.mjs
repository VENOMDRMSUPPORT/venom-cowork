...
const ORCHESTRATOR_RUNTIME = "venomcowork-orchestrator";
const VENOMCOWORK_SERVER_PORT_RANGE_START = 48_000;
const VENOMCOWORK_SERVER_PORT_RANGE_END = 51_000;
...
export function resolveVenomcoworkServerConfigPath(env = process.env) {
  ...
  if (process.platform === "win32") {
    ...
    return path.join(root, "venomcowork", "server.json");
  }
...
function resolveUserEnvFilePath() {
  ...
  if (process.platform === "win32") {
    ...
    const root = appData || path.join(os.homedir(), "AppData", "Roaming");
    return path.join(root, "venomcowork", "env.json");
  }
  return path.join(os.homedir(), ".config", "venomcowork", "env.json");
}

const USER_ENV_KEY_PATTERN = /^[A-Za-z_][A-Za-z0-9_]*$/;
const USER_ENV_RESERVED_PREFIXES = ["VENOMCOWORK_", "OPENCODE_"];

// Synchronous, best-effort; absent or malformed returns {}. Reserved prefixes
// are stripped so a tampered file can never shadow VENOMCOWORK_* / OPENCODE_*.
...
export function createRuntimeManager({ app, desktopRoot, listLocalWorkspacePaths }) {
  ...
  function venomcoworkServerTokenStorePath() {
    return path.join(userDataDir, "venomcowork-server-tokens.json");
  }

  function venomcoworkServerStatePath() {
    return path.join(userDataDir, "venomcowork-server-state.json");
  }
  ...
  function orchestratorDataDir() {
    const envDir = process.env.VENOMCOWORK_DATA_DIR?.trim();
    if (envDir) return envDir;
    return path.join(app.getPath("home"), ".venomcowork", "venomcowork-orchestrator");
  }

  function orchestratorStatePath(dataDir) {
    return path.join(dataDir, "venomcowork-orchestrator-state.json");
  }

  function orchestratorAuthPath(dataDir) {
    return path.join(dataDir, "venomcowork-orchestrator-auth.json");
  }
  ...
  async function loadTokenStore() {
    return readJsonFile(venomcoworkServerTokenStorePath(), { version: 1, workspaces: {} });
  }

  async function saveTokenStore(store) {
    const filePath = venomcoworkServerTokenStorePath();
    await mkdir(path.dirname(filePath), { recursive: true });
    await writeFile(filePath, `${JSON.stringify(store, null, 2)}\n`, "utf8");
  }

  async function loadPortState() {
    return readJsonFile(venomcoworkServerStatePath(), {
      version: 3,
      workspacePorts: {},
      preferredPort: null,
    });
  ...
  async function savePortState(state) {
    const filePath = venomcoworkServerStatePath();
    await mkdir(path.dirname(filePath), { recursive: true });
  ...
  async function ensureDevModePaths() {
    const root = path.join(userDataDir, "venomcowork-dev-data");
    const paths = {
  ...
  function runDockerCommandDetailed(args, timeoutMs = 8000) {
    ...
    throw new Error(
      `Failed to run docker: ${errors.join("; ")} (Set VENOMCOWORK_DOCKER_BIN to your docker binary if needed)`,
    );
  }
  ...
  async function listVenomcoworkManagedContainers() {
    ...
    return result.stdout
      .split(/\r?\n/)
      .map((line) => line.trim())
      .filter((name) => name && (name.startsWith("venomcowork-orchestrator-") || name.startsWith("venomcowork-dev-") || name.startsWith("openwrk-")))
      .sort();
  }
  ...
  function processMatchesSidecar(command) {
    ...
    return sidecarDirs.some((dir) => value.includes(dir)) &&
      (
        value.includes("venomcowork-orchestrator") ||
        value.includes("venomcowork-server") ||
  ...
  async function issueOwnerToken(baseUrl, hostToken) {
    const payload = await fetchJson(
      `${baseUrl.replace(/\/+$/, "")}/tokens`,
      {
        ...
        body: JSON.stringify({ scope: "owner", label: "VenomCowork desktop owner token" }),
      },
      5000,
    );
  ...
  async function startVenomcoworkServer(options) {
    ...
    await stopChild(venomcoworkServerState);
    ...
    venomcoworkServerState.managedOpencodeBinPath = managedOpencode?.path ?? null;
    venomcoworkServerState.managedOpencodeBinSource = managedOpencode?.source ?? null;
    ...
    const candidates = process.env.VENOMCOWORK_DEV_MODE === "1"
      ? [devPath, ...packagedPaths]
      : [...packagedPaths, devPath];
    const embeddedPath = candidates.find((candidate) => existsSync(candidate));
    if (!embeddedPath) {
      throw new Error(`Cannot find VenomCowork embedded server bundle. Checked: ${candidates.join(", ")}`);
    }
    ...
    venomcoworkServerState.managedOpencodeExecution = handle.managedOpencodeExecution ?? null;

    const boundPort = handle.port;
    const baseUrl = handle.url;

    venomcoworkServerState.inProcess = true;
    venomcoworkServerState.remoteAccessEnabled = options.remoteAccessEnabled;
    venomcoworkServerState.host = host;
    venomcoworkServerState.port = boundPort;
    venomcoworkServerState.baseUrl = baseUrl;
    venomcoworkServerState.clientToken = tokens.clientToken;
    venomcoworkServerState.hostToken = tokens.hostToken;

    const connectUrls = options.remoteAccessEnabled ? buildConnectUrls(boundPort) : { connectUrl: null, mdnsUrl: null, lanUrl: null };
    venomcoworkServerState.connectUrl = connectUrls.connectUrl;
    venomcoworkServerState.mdnsUrl = connectUrls.mdnsUrl;
    venomcoworkServerState.lanUrl = connectUrls.lanUrl;
    ...
    venomcoworkServerState.ownerToken = ownerToken;
    if (ownerToken) {
  ...
  async function startOrchestratorRuntime(projectDir, options = {}) {
    ...
    const orchestratorProgram = resolveBinary("venomcowork-orchestrator") ?? resolveBinary("venomcowork");
    if (!orchestratorProgram) {
      throw new Error("Failed to locate venomcowork-orchestrator.");
    }
    ...
    const env = await buildChildEnv({
      VENOMCOWORK_INTERNAL_ALLOW_OPENCODE_CREDENTIALS: "1",
      VENOMCOWORK_OPENCODE_USERNAME: username,
      VENOMCOWORK_OPENCODE_PASSWORD: password,
  ...
  async function ensureVenomcowork(options) {
    ...
    try {
      ...
      appendOutput(engineState, "lastStderr", `VenomCowork server: ${error instanceof Error ? error.message : String(error)}\n`);
      throw error;
    }
  ...
  async function engineStart(projectDir, options = {}) {
    ...
    const requestedRemoteAccess = options.venomcoworkRemoteAccess === true;
    if (
      venomcoworkServerState.inProcess &&
  ...
  async function engineRestart(options = {}) {
    ...
    return engineStart(projectDir, {
      ...
      venomcoworkRemoteAccess: options.venomcoworkRemoteAccess,
    });
  }
  ...
  async function venomcoworkServerRestart(options = {}) {
    ...
    return startVenomcoworkServer({
      ...
      opencodeBinPath: engineState.opencodeBinPath ?? venomcoworkServerState.managedOpencodeBinPath,
    });
  }
  ...
  async function orchestratorStatus() {
    ...
    return {
      ...
      dataDir: null,
      daemon: venomcoworkServer.running
        ? { baseUrl: venomcoworkServer.baseUrl, port: venomcoworkServer.port, pid: venomcoworkServer.pid, runtime: "direct" }
        : null,
  ...
  async function engineInstall() {
    if (process.platform === "win32") {
      return {
        ...
        stdout: "",
        stderr:
          "Guided install is not supported on Windows yet. Install the VenomCowork-pinned OpenCode version manually, then restart VenomCowork.",
      };
    }
  ...
  async function sandboxStop(containerName) {
    ...
    if (!name.startsWith("venomcowork-orchestrator-")) {
      throw new Error("Refusing to stop container: expected name starting with 'venomcowork-orchestrator-'");
    }
  ...
  async function orchestratorStartDetached(options = {}) {
    ...
    const child = spawn(program, args, {
      env: { ...(await buildChildEnv()), VENOMCOWORK_TOKEN: token, VENOMCOWORK_HOST_TOKEN: hostToken },
      detached: true,
      stdio: "ignore",
      windowsHide: true,
    });
    ...
    await waitForHttpOk(`${venomcoworkUrl}/health`, wantsDockerSandbox ? 90_000 : 12_000);
    const ownerToken = await issueOwnerToken(venomcoworkUrl, hostToken).catch(() => null);
  ...
  async function sandboxDebugProbe() {
    ...
    const workspacePath = path.join(os.tmpdir(), `venomcowork-sandbox-probe-${randomUUID()}`);
