...
const NATIVE_DEEP_LINK_EVENT = "venomcowork:deep-link-native";
const NATIVE_MENU_OPEN_SETTINGS_EVENT = "venomcowork:native-menu:open-settings";
const NATIVE_MENU_TOGGLE_SIDEBAR_EVENT = "venomcowork:native-menu:toggle-sidebar";
const TAURI_APP_IDENTIFIER = "com.venom.cowork";
const DEV_APP_IDENTIFIER = "com.venom.cowork.dev";
const DESKTOP_PROTOCOL_SCHEME = "venomcowork";
...
function computerUseHelperExecutablePath() {
  ...
  const explicitBinary = process.env.VENOMCOWORK_COMPUTER_USE_BINARY?.trim();
  const candidates = [
    explicitBinary,
...
function computerUseHelperAppPath() {
  const explicitApp = process.env.VENOMCOWORK_COMPUTER_USE_APP?.trim();
  const candidates = [
    explicitApp,
...
function getComputerUseMcpCommand() {
  ...
  if (app.isPackaged) {
    throw new Error("VenomCowork Computer Use is missing from this VenomCowork build.");
  }

  if (process.env.VENOMCOWORK_DEV_MODE === "1") {
    return ["node", path.resolve(__dirname, "../../..", "packages/handsfree/bin/venomcowork-handsfree-computer-use.mjs"), "mcp"];
  }
  return ["npx", "-y", "@venom-cowork/handsfree", "mcp"];
}

// ---------------------------------------------------------------------------
// Permission checks â€” spawn the binary with --check, read stdout, done.
// Fresh process = fresh TCC read = always accurate. No HTTP server needed.
// ---------------------------------------------------------------------------

function resolveComputerUseExecutable() {
  // 1. Explicit env override.
  const explicit = process.env.VENOMCOWORK_COMPUTER_USE_BINARY?.trim();
  ...
  // 3. Dev fallback â€” raw Swift build output.
  if (!app.isPackaged) {
...
async function checkComputerUsePermissions() {
  // Spawn binary --check â†’ read JSON from stdout â†’ exit. Always fresh.
...
//
// Override via VENOMCOWORK_ELECTRON_USERDATA so dogfooders can isolate their
...
const userDataOverride = process.env.VENOMCOWORK_ELECTRON_USERDATA?.trim();
if (userDataOverride) {
...
async function resolveArchitectureInfo() {
  ...
  const assetName = `venomcowork-${platformDownloadSlug()}-${downloadAssetArch(targetArch)}-${version}.${downloadAssetExtension()}`;
...
// drive the built-in browser panel.  Use VENOMCOWORK_ELECTRON_REMOTE_DEBUG_PORT to
...
const explicitCdpPort = Number.parseInt(
  process.env.VENOMCOWORK_ELECTRON_REMOTE_DEBUG_PORT?.trim() ?? "",
  10,
);
...
const IDLE_VENOMCOWORK_SERVER_INFO = Object.freeze({
  running: false,
...
// â”€â”€ Embedded browser panel â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
function browserTargetMarkerUrl(tabId) {
  const marker = `venomcowork-browser-tab:${tabId}`;
  const html = `<!doctype html><title>${marker}</title><meta name="venomcowork-browser-tab" content="${tabId}"><body>${marker}</body>`;
...
async function resolveBrowserCdpTargetId(tabId) {
  const marker = encodeURIComponent(`venomcowork-browser-tab:${tabId}`);
...
async function showBrowserTabContextMenu(tabId, point) {
  ...
  view.webContents.send("venomcowork:menu-overlay:show", {
    id: request.id,
...
function createBrowserTab(url = "about:blank", { select = true } = {}) {
  ...
  const view = new WebContentsView({
    webPreferences: {
      ...
      partition: "persist:venomcowork-browser",
    },
  });
  ...
  // Cookies live on the session object, not the document â€” they survive this.
  ...
  view.webContents.on("did-start-navigation", (_event, targetUrl, isInPlace, isMainFrame) => {
    if (isMainFrame && !isInPlace && targetUrl !== "about:blank") {
      sendToRenderer("venomcowork:browser:panel-opened");
    }
  });
...
function closeBrowserTab(tabId = activeBrowserTabId) {
  ...
  if (wasActive) {
    ...
    if (nextTabId) {
      ...
      hideBrowserView();
      sendToRenderer("venomcowork:browser:panel-closed");
    }
  }
...
function sendBrowserState() {
  sendToRenderer("venomcowork:browser:state", browserStatePayload());
}

/**
 * Attach the browser view to the main window.
 * @param {object} bounds â€” { x, y, width, height }
 * @param {object} [opts]
...
function forwardedDeepLinks(argv) {
  return argv
    ...
    .filter(
      (entry) =>
        entry.startsWith("venomcowork://") ||
        entry.startsWith("venomcowork-dev://") ||
...
function desktopBootstrapPath() {
  if (process.env.VENOMCOWORK_DESKTOP_BOOTSTRAP_PATH?.trim()) {
    return process.env.VENOMCOWORK_DESKTOP_BOOTSTRAP_PATH.trim();
  }
  return path.join(os.homedir(), ".config", "venomcowork", "desktop-bootstrap.json");
}

function workspaceStatePath() {
  return path.join(app.getPath("userData"), "venomcowork-workspaces.json");
}

// Earlier Electron alpha builds copied Tauri's venomcowork-workspaces.json into an
// Electron-only workspace-state.json. Keep importing that file when the shared
// canonical file is missing, but write venomcowork-workspaces.json going forward so
...
async function migrateLegacyElectronWorkspaceStateIfNeeded() {
  ...
  try {
    ...
    console.info(
      "[migration] copied workspace-state.json â†’ venomcowork-workspaces.json",
    );
    return true;
  } catch (error) {
...
async function debugDesktopBootstrapConfig() {
  ...
  const result = {
    ...
    envOverride: process.env.VENOMCOWORK_DESKTOP_BOOTSTRAP_PATH ?? null,
...
async function fetchVenomcoworkWorkspaceList(hostUrl, token, hostToken) {
  ...
  if (hostAuthToken) headers.set("X-VenomCowork-Host-Token", hostAuthToken);

  try {
    const response = await fetch(url, { headers, signal: controller.signal });
    if (!response.ok) {
      throw new Error(`VenomCowork workspace discovery failed (${response.status} ${response.statusText || "HTTP error"})`);
    }
...
async function writeWorkspaceVenomcoworkConfig(workspacePath, config) {
  ...
  return execResult(true, `Wrote ${venomcoworkPath}`);
}
...
async function readWorkspaceState() {
  ...
  const migratedWorkspaces = workspaces.map((entry) => {
    ...
    const nextWorkspace = {
      ...
      id: nextId,
      baseUrl: hostUrl,
      venomcoworkWorkspaceId: remoteWorkspaceId,
      venomcoworkHostUrl: hostUrl,
    };
    if (workspace.id !== nextWorkspace.id || workspace.baseUrl !== nextWorkspace.baseUrl || workspace.venomcoworkWorkspaceId !== nextWorkspace.venomcoworkWorkspaceId || workspace.venomcoworkHostUrl !== nextWorkspace.venomcoworkHostUrl) {
      changed = true;
    }
    return nextWorkspace;
  });
  // Older desktop state can contain multiple VenomCowork remote entries that
...
<html>
  ...
  <body>
    <main>
      ...
      <div class="title">Stopping VenomCowork services</div>
...
function assertVenomcoworkServerReady(info) {
  ...
  if (!info.baseUrl) {
    throw new Error("VenomCowork server did not report a base URL after startup.");
  }
  if (!info.ownerToken && !info.clientToken) {
    throw new Error("VenomCowork server did not report an access token after startup.");
  }
  return info;
}
...
function normalizeWorkspaceEntry(input) {
  return {
    ...
    venomcoworkHostUrl: input.venomcoworkHostUrl ?? null,
    ...
    venomcoworkWorkspaceId: input.venomcoworkWorkspaceId ?? null,
    venomcoworkWorkspaceName: input.venomcoworkWorkspaceName ?? null,
  ...
  switch (command) {
    ...
    case "workspaceCreateRemote": {
      ...
      if (remoteType === "venomcowork" && !resolvedVenomcoworkWorkspaceId) {
        ...
        if (!discovered?.id) {
          throw new Error(
            directory
              ? `VenomCowork server has no workspace matching ${directory}.`
      ...
      const workspace = normalizeWorkspaceEntry({
        ...
        remoteType,
        baseUrl: remoteType === "venomcowork" ? (venomcoworkHostUrl ?? baseUrl) : baseUrl,
        directory,
        venomcoworkHostUrl,
        venomcoworkToken: input.venomcoworkToken ?? null,
        venomcoworkClientToken: input.venomcoworkClientToken ?? null,
    ...
    case "workspaceUpdateRemote": {
      ...
      return mutateWorkspaceState(async (state) => {
        ...
        const nextRemoteType = nextWorkspace.remoteType === "opencode" ? "opencode" : "venomcowork";
        if (nextRemoteType === "venomcowork") {
          const rawHostUrl = typeof nextWorkspace.venomcoworkHostUrl === "string" && nextWorkspace.venomcoworkHostUrl.trim()
          ...
          if (!remoteWorkspaceId) {
            ...
            remoteWorkspaceName = venomcoworkWorkspaceDisplayName(discovered);
          }
          const nextId = venomcoworkRemoteWorkspaceId(hostUrl ?? nextBaseUrl, remoteWorkspaceId);
          nextWorkspace = normalizeWorkspaceEntry({
            ...
            remoteType: "venomcowork",
            venomcoworkWorkspaceId: remoteWorkspaceId,
            venomcoworkWorkspaceName: remoteWorkspaceName,
          });
    ...
    case "appBuildInfo":
      return {
        ...
        gitSha: process.env.VENOMCOWORK_GIT_SHA ?? null,
        buildEpoch: process.env.VENOMCOWORK_BUILD_EPOCH ?? null,
        venomcoworkDevMode: process.env.VENOMCOWORK_DEV_MODE === "1",
      };
    case "getUiControlBridgeInfo":
      try {
        const raw = await readFile(path.join(app.getPath("userData"), "venomcowork-ui-control.json"), "utf8");
        return JSON.parse(raw);
      } catch {
        return null;
      }
    case "getVenomcoworkUiMcpCommand": {
      ...
      return ["npx", "-y", "venomcowork-ui-mcp"];
    }
    case "getComputerUseMcpCommand": {
      return getComputerUseMcpCommand();
    }
    case "checkComputerUsePermissions": {
      // Spawn --check â†’ fresh TCC read â†’ always accurate.
      return checkComputerUsePermissions();
    }
    case "openComputerUsePermissionSetup": {
      // Open the GUI app. Returns immediately â€” React shows "verify" CTA.
    ...
    case "venomcoworkServerInfo":
      return runtimeManager.venomcoworkServerInfo();
    case "venomcoworkServerRestart":
      return runtimeManager.venomcoworkServerRestart(args[0] ?? {});
    case "pickDirectory": {
  ...
  if (command === "execute") {
    return evaluateVenomcoworkControl(`(async () => {
      ...
      if (!input || typeof input.actionId !== "string" || !input.actionId.trim()) {
        return { ok: false, error: "Missing VenomCowork actionId." };
      }
      control.setEnabled?.(true);
      return control.execute(input.actionId, input.args ?? {});
    })()`, { focus: true });
  }
  return { ok: false, error: `Unknown VenomCowork control command: ${command}` };
}
...
async function startUiControlServer() {
  ...
  if (!port) throw new Error("Could not start VenomCowork UI control bridge.");
  uiControlDiscoveryPath = path.join(app.getPath("userData"), "venomcowork-ui-control.json");
  await writeFile(
  ...
  // Make the discovery path available to child processes (server â†’ managed OpenCode â†’ plugin).
  process.env.VENOMCOWORK_UI_CONTROL_DISCOVERY = uiControlDiscoveryPath;
}
...
async function createMainWindow() {
  ...
  const startUrl = process.env.VENOMCOWORK_ELECTRON_START_URL?.trim() || process.env.ELECTRON_START_URL?.trim();
  if (startUrl) {
...
ipcMain.handle("venomcowork:desktop", handleDesktopInvoke);
ipcMain.handle("venomcowork:shell:openExternal", async (_event, url) => {
  if (typeof url === "string" && url.trim().length > 0) {
    await shell.openExternal(url);
  }
});
ipcMain.handle("venomcowork:shell:relaunch", async () => {
  app.relaunch();
  app.exit(0);
});
ipcMain.handle("venomcowork:system:architecture", async () => resolveArchitectureInfo());
ipcMain.handle("venomcowork:system:microphoneStatus", async () => {
...
ipcMain.handle("venomcowork:system:askMicrophoneAccess", async () => {
...
// â”€â”€ Terminal IPC â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
ipcMain.handle("venomcowork:terminal:create", async (event, options = {}) => {
  ...
  child.onData((data) => {
    ...
    event.sender.send("venomcowork:terminal:data", { terminalId, data });
  });
  child.onExit(({ exitCode, signal }) => {
    terminalProcesses.delete(terminalId);
    if (event.sender.isDestroyed()) return;
    event.sender.send("venomcowork:terminal:exit", { terminalId, exitCode, signal });
  });

  return { terminalId };
});
ipcMain.handle("venomcowork:terminal:write", (event, terminalId, data) => {
  const terminal = terminalForSender(event, terminalId);
  if (!terminal || typeof data !== "string") return;
  terminal.process.write(data);
});
ipcMain.handle("venomcowork:terminal:resize", (event, terminalId, cols, rows) => {
  const terminal = terminalForSender(event, terminalId);
...
ipcMain.handle("venomcowork:terminal:kill", (event, terminalId) => {
  const terminal = terminalForSender(event, terminalId);
  if (!terminal) return;
  killTerminal(String(terminalId));
});

// â”€â”€ Embedded browser IPC â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
ipcMain.handle("venomcowork:browser:show", (_event, bounds) => attachBrowserView(bounds));
ipcMain.handle("venomcowork:browser:hide", () => hideBrowserView());
ipcMain.handle("venomcowork:browser:openUrl", (_event, url, provider) => openBrowserUrlForAutomation(url, provider));
ipcMain.handle("venomcowork:browser:navigate", (_event, url) => {
...
ipcMain.handle("venomcowork:browser:back", () => {
  const webContents = getActiveWebContents();
  if (webContents?.canGoBack()) webContents.goBack();
});
ipcMain.handle("venomcowork:browser:forward", () => {
  const webContents = getActiveWebContents();
  if (webContents?.canGoForward()) webContents.goForward();
});
ipcMain.handle("venomcowork:browser:reload", () => getActiveWebContents()?.reload());
ipcMain.handle("venomcowork:browser:bounds", (_event, bounds) => {
  lastBrowserBounds = bounds;
  const view = getActiveBrowserView();
...
ipcMain.handle("venomcowork:browser:state", () => browserStatePayload());
ipcMain.handle("venomcowork:browser:createTab", (_event, url) => {
  const target = typeof url === "string" && url.trim() ? url : BROWSER_NEW_TAB_URL;
  const tab = createBrowserTab(target, { select: true });
  return { tabId: tab.tabId };
});
ipcMain.handle("venomcowork:browser:closeTab", (_event, tabId) => closeBrowserTab(tabId == null ? undefined : String(tabId)));
ipcMain.handle("venomcowork:browser:closeAllTabs", () => closeAllBrowserTabs());
ipcMain.handle("venomcowork:browser:selectTab", (_event, tabId) => selectBrowserTab(String(tabId ?? "")).tabId);
ipcMain.handle("venomcowork:browser:reorderTabs", (_event, tabIds) => reorderBrowserTabs(tabIds));
ipcMain.handle("venomcowork:browser:listTabs", () => listBrowserTabs());
ipcMain.handle("venomcowork:browser:tabContextMenu", (_event, tabId, point) => showBrowserTabContextMenu(tabId, point));
ipcMain.handle("venomcowork:browser:destroy", () => destroyBrowserView());
ipcMain.on("venomcowork:menu-overlay:ready", (event) => {
  if (event.sender !== menuOverlayView?.webContents) return;
  markMenuOverlayReady(menuOverlayView);
});
ipcMain.on("venomcowork:menu-overlay:choose", (event, payload) => {
  if (event.sender !== menuOverlayView?.webContents) return;
  handleMenuOverlayChoice(payload);
});
ipcMain.on("venomcowork:menu-overlay:close", (event, payload) => {
  if (event.sender !== menuOverlayView?.webContents) return;
  if (payload?.requestId && payload.requestId !== menuOverlayRequest?.id) return;
  hideMenuOverlay();
});
ipcMain.on("venomcowork:menu-overlay:dismiss", (event) => {
  if (event.sender === menuOverlayView?.webContents) return;
  hideMenuOverlay();
});
