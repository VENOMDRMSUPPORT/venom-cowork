...
// (e.g. "35.7.5") instead of the VenomCowork app version. Read from
...
const SHIP_IT_DEFAULTS_DOMAIN = "com.venom.cowork.ShipIt";
...
// producing:
//   "Failed to copy bundle â€¦ no such file or directory"
...
export function registerUpdaterIpc({ app, ipcMain, getMainWindow }) {
  ...
  async function ensureAutoUpdater() {
    ...
    try {
      ...
      if (autoUpdaterInstance) {
        ...
        // for the "Failed to copy bundle â€¦ no such file" abort. Download the
        // full zip instead â€” alpha builds are swapped wholesale anyway.
        ...
        autoUpdaterInstance.on("download-progress", (info) => {
          sendToRenderer("venomcowork:updater:download-progress", {
  ...
  ipcMain.handle("venomcowork:updater:getChannel", async () => {
    const channel = await readElectronUpdaterChannel(app);
    return updaterChannelState(app, channel);
  });

  ipcMain.handle("venomcowork:updater:setChannel", async (_event, rawChannel) => {
  ...
  ipcMain.handle("venomcowork:updater:check", async (_event, rawChannel) => {
  ...
  ipcMain.handle("venomcowork:updater:download", async () => {
    const updater = await ensureAutoUpdater();
  ...
  ipcMain.handle("venomcowork:updater:installAndRestart", async () => {
    const updater = await ensureAutoUpdater();
    if (!updater) return { ok: false, reason: "unavailable" };
    try {
