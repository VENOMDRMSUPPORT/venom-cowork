...
Filesystem-backed API for VenomCowork remote clients. This package provides the VenomCowork server layer described in `apps/app/pr/venomcowork-server.md` and is intentionally independent from the desktop app.

## Quick start

```bash
npm install -g venomcowork-server
venomcowork-server --workspace /path/to/workspace --approval auto
```

`venomcowork-server` ships as a compiled binary, so Bun is not required at runtime.

Or from source:

```bash
pnpm --filter venomcowork-server dev -- \
...
Defaults to `~/.config/venomcowork/server.json` (override with `VENOMCOWORK_SERVER_CONFIG` or `--config`).
...
- `VENOMCOWORK_SERVER_CONFIG` path to config JSON
- `VENOMCOWORK_HOST` / `VENOMCOWORK_PORT`
- `VENOMCOWORK_TOKEN` client bearer token
- `VENOMCOWORK_HOST_TOKEN` host approval token
- `VENOMCOWORK_APPROVAL_MODE` (`manual` | `auto`)
- `VENOMCOWORK_APPROVAL_TIMEOUT_MS`
- `VENOMCOWORK_WORKSPACES` (JSON array or comma-separated list of paths)
- `VENOMCOWORK_CORS_ORIGINS` (comma-separated list or `*`)
- `VENOMCOWORK_OPENCODE_BASE_URL`
- `VENOMCOWORK_OPENCODE_DIRECTORY`
- `VENOMCOWORK_OPENCODE_USERNAME`
- `VENOMCOWORK_OPENCODE_PASSWORD`

Token management (scoped tokens):

- `VENOMCOWORK_TOKEN_STORE` path to token store JSON (default: alongside `server.json`)

File injection / artifacts:

- `VENOMCOWORK_INBOX_ENABLED` (`1` | `0`)
- `VENOMCOWORK_INBOX_MAX_BYTES` (default: 50MB, capped)
- `VENOMCOWORK_OUTBOX_ENABLED` (`1` | `0`)

Sandbox advertisement (for capability discovery):

- `VENOMCOWORK_SANDBOX_ENABLED` (`1` | `0`)
- `VENOMCOWORK_SANDBOX_BACKEND` (`docker` | `container` | `none`)
...
- `POST /workspace/:id/inbox` (multipart upload into `.opencode/venomcowork/inbox/`)
...
- `X-VenomCowork-Host-Token: <token>` (legacy host token), or
...
Set `VENOMCOWORK_APPROVAL_MODE=auto` to auto-approve during local development.
