...
const venomcoworkWorkspaceConfigs = sqliteTable("venomcowork_workspace_configs", {
...
function runtimeDbPath(config: ServerConfig): string {
  const override = process.env.VENOMCOWORK_RUNTIME_DB?.trim();
  if (override) return resolve(override);
  const configPath = config.configPath?.trim();
  const configDir = configPath ? dirname(configPath) : join(homedir(), ".config", "venomcowork");
...
async function openDb(path: string): Promise<OpenworkWorkspaceConfigDb> {
  ...
  if (typeof process.versions.bun === "string") {
    ...
    sqlite.run("CREATE TABLE IF NOT EXISTS venomcowork_workspace_configs (workspace_id TEXT PRIMARY KEY NOT NULL, config_json TEXT NOT NULL, updated_at INTEGER NOT NULL)");
    const db = drizzle(sqlite);
    return {
      get: (workspaceId) => db
        .select()
        .from(venomcoworkWorkspaceConfigs)
        .where(eq(venomcoworkWorkspaceConfigs.workspaceId, workspaceId))
        .get(),
      upsert: ({ workspaceId, configJson, updatedAt }) => {
        db
          .insert(venomcoworkWorkspaceConfigs)
          .values({ workspaceId, configJson, updatedAt })
          .onConflictDoUpdate({
            target: venomcoworkWorkspaceConfigs.workspaceId,
  ...
  sqlite.exec("CREATE TABLE IF NOT EXISTS venomcowork_workspace_configs (workspace_id TEXT PRIMARY KEY NOT NULL, config_json TEXT NOT NULL, updated_at INTEGER NOT NULL)");
  const get = sqlite.prepare("SELECT config_json AS configJson FROM venomcowork_workspace_configs WHERE workspace_id = ?");
  const upsert = sqlite.prepare("INSERT INTO venomcowork_workspace_configs (workspace_id, config_json, updated_at) VALUES (?, ?, ?) ON CONFLICT(workspace_id) DO UPDATE SET config_json = excluded.config_json, updated_at = excluded.updated_at");
  return {
    get: (workspaceId) => {
