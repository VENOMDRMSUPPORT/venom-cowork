...
function readWorkspaceCloudImports(venomcowork: Record<string, unknown>): WorkspaceCloudImports {
  const cloudImports = isRecord(venomcowork.cloudImports) ? venomcowork.cloudImports : {};
  return {
...
export function readDesktopCloudSyncState(venomcowork: Record<string, unknown>): DesktopCloudSyncState {
  const raw = isRecord(venomcowork.desktopCloudSync) ? venomcowork.desktopCloudSync : {};
...
export function syncDesktopCloudResources(input: {
  ...
  const state = readDesktopCloudSyncState(input.venomcowork);
  const key = contextKey(input.snapshot);
  const previousEntry = state.entries[key] ?? null;
  const changes = diffInstalledCloudResources(readWorkspaceCloudImports(input.venomcowork), input.snapshot, now);
