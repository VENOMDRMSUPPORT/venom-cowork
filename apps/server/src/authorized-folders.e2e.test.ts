...
const roots: string[] = [];
const priorDataDir = process.env.VENOMCOWORK_DATA_DIR;
...
beforeEach(async () => {
  const envRoot = await createTempRoot("venomcowork-authorized-folders-env-");
  process.env.VENOMCOWORK_DATA_DIR = join(envRoot, "data");
  process.env.VENOMCOWORK_TOKEN_STORE = join(envRoot, "tokens.json");
});
...
afterEach(async () => {
  ...
  if (priorDataDir === undefined) {
    delete process.env.VENOMCOWORK_DATA_DIR;
  } else {
    process.env.VENOMCOWORK_DATA_DIR = priorDataDir;
  }
