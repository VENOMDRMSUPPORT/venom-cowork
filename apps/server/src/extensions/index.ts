...
const VENOMCOWORK_EXPERIMENTAL_EXTENSION_ACTIONS = [
  ...GOOGLE_WORKSPACE_EXTENSION_ACTIONS,
  ...OPENAI_IMAGE_GENERATION_EXTENSION_ACTIONS,
];
...
export function listExperimentalExtensionActions(extensionId: string) {
  ...
  return filter
    ? VENOMCOWORK_EXPERIMENTAL_EXTENSION_ACTIONS.filter((action) => action.extensionId === filter)
    : VENOMCOWORK_EXPERIMENTAL_EXTENSION_ACTIONS;
}
...
export async function callExperimentalExtensionAction(config: ServerConfig, env: EnvService, input: unknown) {
  ...
  const registered = VENOMCOWORK_EXPERIMENTAL_EXTENSION_ACTIONS.find((item) => item.extensionId === extensionId && item.action === action);
  if (!registered) {
    throw new ApiError(404, "extension_action_not_found", "VenomCowork extension action not found");
  }
  ...
  throw new ApiError(501, "extension_action_not_implemented", `${registered.title} is registered but not implemented on venomcowork-server yet.`, { extensionId, action, args });
}
