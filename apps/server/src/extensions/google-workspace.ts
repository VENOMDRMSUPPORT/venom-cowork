...
const GOOGLE_WORKSPACE_CLIENT_ID_ENV = "VENOMCOWORK_GOOGLE_WORKSPACE_OAUTH_CLIENT_ID";
const GOOGLE_WORKSPACE_CLIENT_SECRET_ENV = "GOOGLE_WORKSPACE_OAUTH_CLIENT_SECRET";
const LEGACY_GOOGLE_WORKSPACE_CLIENT_SECRET_ENV = "VENOMCOWORK_GOOGLE_WORKSPACE_OAUTH_CLIENT_SECRET";
const GOOGLE_WORKSPACE_TOKEN_BROKER_URL_ENV = "VENOMCOWORK_GOOGLE_WORKSPACE_TOKEN_BROKER_URL";
const GOOGLE_WORKSPACE_ALLOW_PLAINTEXT_VAULT_ENV = "VENOMCOWORK_GOOGLE_WORKSPACE_ALLOW_PLAINTEXT_VAULT";
...
export const GOOGLE_WORKSPACE_EXTENSION_ACTIONS = [
  {
    ...
    description: "Check whether Google Workspace is connected and ready for VenomCowork extension actions.",
  ...
  },
  {
    ...
    description: "Search files available to VenomCowork through the connected Google Drive scope.",
    inputSchema: {
      type: "object",
      properties: {
  ...
  },
  {
    ...
    description: "Read a Drive file available to VenomCowork by file id.",
    inputSchema: {
      type: "object",
      properties: {
...
function configDir(config: ServerConfig): string {
  return dirname(config.configPath?.trim() || resolve(homedir(), ".config", "venomcowork", "server.json"));
}
...
function googleWorkspacePlainTextVaultEnabled() {
  return process.env.VENOMCOWORK_DEV_MODE === "1" && process.env[GOOGLE_WORKSPACE_ALLOW_PLAINTEXT_VAULT_ENV] === "1";
}
...
async function googleWorkspaceVaultKey(config: ServerConfig): Promise<Buffer> {
  const envKey = process.env.VENOMCOWORK_ENCRYPTION_KEY?.trim();
...
async function googleWorkspaceAccessToken(config: ServerConfig): Promise<{ record: Record<string, unknown>; accessToken: string }> {
  ...
  if (!record) throw new ApiError(400, "google_workspace_not_connected", "Connect Google Workspace in VenomCowork Settings to use this tool.");
...
export async function googleWorkspaceRunScopeSmokeTest(config: ServerConfig) {
  ...
  const driveBoundary = `venomcowork_${randomBytes(8).toString("hex")}`;
  const driveFile = await fetchGoogleJson("https://www.googleapis.com/upload/drive/v3/files?uploadType=multipart&fields=id,name,webViewLink", {
    ...
    body: multipartRelatedBody({ name: "VenomCowork Google Workspace smoke test.txt", mimeType: "text/plain" }, `VenomCowork Google Workspace smoke test created at ${createdAt}.`, driveBoundary),
  });
  ...
  const draft = await fetchGoogleJson("https://gmail.googleapis.com/gmail/v1/users/me/drafts", {
    method: "POST",
    headers: { Authorization: `Bearer ${accessToken}`, "Content-Type": "application/json" },
    body: JSON.stringify({ message: { raw: base64UrlString(gmailRawMessage({ to: [email], subject: "VenomCowork Google Workspace smoke test draft", body: `This draft was created by VenomCowork to verify Gmail draft access at ${createdAt}.\nVenomCowork does not send this email automatically.` })) } }),
  });
  return googleWorkspaceStatusPayload(record, {
    ...
    const port = await new Promise<number>((resolvePort, reject) => {
      callbackServer = createServer(async (request, response) => {
        ...
        try {
          const flow = flows.get(flowId);
          if (!flow) {
            await finish(googleWorkspaceCallbackPage(410, "Google Workspace connection expired", "Return to VenomCowork and start connection again."));
            return;
          }
          ...
          await finish(googleWorkspaceCallbackPage(200, "Google Workspace authorization received", "You can return to VenomCowork while it finishes connecting."));
          try {
            ...
            flow.status = "failed";
            flow.error = `Google authorized VenomCowork, but token exchange failed: ${exchangeError instanceof Error ? exchangeError.message : String(exchangeError)}`;
          }
        } catch (callbackError) {
          const flow = flows.get(flowId);
          if (flow) {
