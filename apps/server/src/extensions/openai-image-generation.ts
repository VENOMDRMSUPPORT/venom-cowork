...
export const OPENAI_IMAGE_GENERATION_EXTENSION_ACTIONS = [
  {
    ...
    description: "Check whether OpenAI image generation is configured and ready for VenomCowork extension actions.",
...
function slugifyImageArtifactName(value: string) {
  return value
    ...
    .replace(/[^a-z0-9]+/g, "-")
    .replace(/^-+|-+$/g, "")
    .slice(0, 48) || "venomcowork-image";
}

async function resolveOpenAiImageApiKey(env: EnvService): Promise<string> {
  const records = await env.list();
  return records.find((entry) => entry.key === "VENOMCOWORK_OPENAI_IMAGE_API_KEY")?.value.trim() ||
    records.find((entry) => entry.key === "OPENAI_API_KEY")?.value.trim() ||
    process.env.VENOMCOWORK_OPENAI_IMAGE_API_KEY?.trim() ||
...
async function generateOpenAiImageArtifact(config: ServerConfig, env: EnvService, args: Record<string, unknown>, context: Record<string, unknown>) {
  ...
  const apiKey = await resolveOpenAiImageApiKey(env);
  if (!apiKey) {
    throw new ApiError(400, "openai_api_key_missing", "OpenAI API key missing. Save OPENAI_API_KEY in VenomCowork Environment Variables or configure the OpenAI Image Gen extension.");
  }
