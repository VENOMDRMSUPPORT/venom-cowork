...
async function createWorkspaceRoot(folderName?: string) {
  const root = await mkdtemp(join(tmpdir(), "venomcowork-session-read-"));
...
describe("workspace session read APIs", () => {
  ...
  test("accepts guest-side rem_ workspace aliases for session reads", async () => {
    ...
    const response = await fetch(`http://127.0.0.1:${venomcowork.server.port}/workspace/rem_ws_1/sessions`, {
      headers: auth(venomcowork.token),
    });
  ...
  test("encodes non-ASCII workspace directory headers for session reads", async () => {
    ...
    const response = await fetch(`http://127.0.0.1:${venomcowork.server.port}/workspace/ws_1/sessions`, {
      headers: auth(venomcowork.token),
    });
  ...
  test("encodes non-ASCII workspace directory headers for opencode proxy requests", async () => {
    ...
    const response = await fetch(`http://127.0.0.1:${venomcowork.server.port}/workspace/ws_1/opencode/session`, {
      headers: auth(venomcowork.token),
    });
  ...
  test("returns 404 when the upstream session is missing", async () => {
    ...
    const response = await fetch(`http://127.0.0.1:${venomcowork.server.port}/workspace/ws_1/sessions/ses_missing/snapshot`, {
      headers: auth(venomcowork.token),
    });
  ...
  test("acknowledges proxied session commands before upstream completion", async () => {
    ...
    const response = await Promise.race([
      fetch(`http://127.0.0.1:${venomcowork.server.port}/workspace/ws_1/opencode/session/ses_1/command`, {
        method: "POST",
        headers: { ...auth(venomcowork.token), "Content-Type": "application/json" },
  ...
  test("keeps legacy /w workspace opencode proxy alias", async () => {
    ...
    const response = await fetch(`http://127.0.0.1:${venomcowork.server.port}/w/ws_1/opencode/session`, {
      headers: auth(venomcowork.token),
    });
