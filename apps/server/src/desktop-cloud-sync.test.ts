...
describe("desktop cloud sync", () => {
  test("queues provider update and remove changes", () => {
    ...
    const result = syncDesktopCloudResources({ now: 1780442400000, venomcowork, snapshot: baseSnapshot });
    ...
    const state = readDesktopCloudSyncState(result.venomcowork);
  ...
  test("syncs large provider snapshots within an interactive budget", () => {
    ...
    const venomcowork = { cloudImports: { providers: importedProviders } };

    const start = performance.now();
    const result = syncDesktopCloudResources({ now: 1780442400000, venomcowork, snapshot });
