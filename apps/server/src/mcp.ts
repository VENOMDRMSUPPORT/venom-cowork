...
export async function listMcp(serverConfig: ServerConfig, workspaceId: string, workspaceRoot: string): Promise<McpItem[]> {
  ...
  // VenomCowork-owned MCPs are stored by the server and injected at runtime.
...
// scope by design â€” only workspace-level entries.
//
// `updateJsoncPath` (vs `updateJsoncTopLevel`) preserves inline comments
// inside the MCP entry â€” see the regression that motivated #1444.
