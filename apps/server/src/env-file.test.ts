...
describe("env-file", () => {
  ...
  beforeEach(() => {
    dir = mkdtempSync(join(tmpdir(), "venomcowork-env-"));
  ...
  test("isReservedEnvKey blocks VENOMCOWORK_ / OPENCODE_ prefixes", () => {
    expect(isReservedEnvKey("VENOMCOWORK_TOKEN")).toBe(true);
  ...
  test("upsertMany rejects reserved keys", async () => {
    const svc = new EnvService({ path });
    const promise = svc.upsertMany([{ key: "VENOMCOWORK_TOKEN", value: "x" }]);
    await expect(promise).rejects.toBeInstanceOf(InvalidEnvKeyError);
  ...
  test("readForInjection strips reserved keys even if present on disk", async () => {
    ...
    writeFileSync(
      ...
      JSON.stringify({
        ...
        updatedAt: Date.now(),
        variables: [
          { key: "VENOMCOWORK_TOKEN", value: "stolen" },
