...
// them into every spawned child (OpenCode, VenomCowork server, opencode-router).
...
// is for service credentials, not VenomCowork/OpenCode runtime knobs; users who
...
// identity.
const RESERVED_PREFIXES = ["VENOMCOWORK_", "OPENCODE_"] as const;
...
// Do NOT key this off ServerConfig.configPath â€” the shell resolves the path
...
export class EnvService {
  ...
  // shell has its own equivalent in src-tauri/src/env_file.rs â€” keep the two
  ...
  constructor(key: string, code: "invalid_env_key" | "reserved_env_key") {
    super(
      code === "reserved_env_key"
        ? `Environment variable name is reserved for VenomCowork internals: ${key}`
