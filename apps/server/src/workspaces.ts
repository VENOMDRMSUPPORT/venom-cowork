...
export function workspaceIdForOpenwork(hostUrl: string, workspaceId?: string | null): string {
  ...
  const key = normalizedWorkspaceId
    ? `venomcowork::${normalizedHostUrl}::${normalizedWorkspaceId}`
    : `venomcowork::${normalizedHostUrl}`;
  return workspaceIdForKey(key);
}
...
export function buildWorkspaceInfos(
  ...
  return workspaces.map((workspace) => {
    ...
    return {
      ...
      venomcoworkHostUrl: workspace.venomcoworkHostUrl,
      venomcoworkToken: workspace.venomcoworkToken,
      venomcoworkWorkspaceId: workspace.venomcoworkWorkspaceId,
      venomcoworkWorkspaceName: workspace.venomcoworkWorkspaceName,
