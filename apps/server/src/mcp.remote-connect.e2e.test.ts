...
describe("mcp remote connect flow", () => {
  test("adds, lists, and removes a remote MCP without OAuth", async () => {
    const workspaceRoot = await mkdtemp(join(tmpdir(), "venomcowork-mcp-remote-e2e-"));
    const previousDb = process.env.VENOMCOWORK_RUNTIME_DB;
    process.env.VENOMCOWORK_RUNTIME_DB = join(workspaceRoot, "runtime.sqlite");
    ...
    try {
      ...
      await expect(stat(join(workspaceRoot, ".opencode", "venomcowork.json"))).rejects.toThrow();
