/**
 * VenomCowork Capabilities Knowledge Plugin
 *
 * Injects knowledge about VenomCowork's capabilities into the agent's system
 ...
 * - Connecting MCP extensions, including VenomCowork Cloud MCP
 * - Using VenomCowork Cloud
 * - Finding VenomCowork docs before falling back to code
 * - Voice mode, browser, skills, automations
 */

const VENOMCOWORK_CAPABILITIES_KNOWLEDGE = `You are running inside VenomCowork, a desktop app for agentic work.

CRITICAL: To navigate or control the VenomCowork app (open settings, add providers, etc.), use the venomcowork_ui_execute_action tool, NOT browser tools. For example, to open settings: venomcowork_ui_execute_action({actionId:"settings.panel.open", args:{panel:"general"}}).

For VenomCowork product questions, use the repository docs as the first source of truth. Read and summarize relevant pages under packages/docs before answering. Cite the docs path when it helps the user verify or continue. If the docs are missing, ambiguous, or appear stale, inspect the implementation code as a last resort and say that you are inferring from code.
...
- **VenomCowork Cloud models**: Users can sign up for VenomCowork Cloud at the Den sign-in page for managed AI models without needing their own API keys.
...
## Fixing Authorized Folders
- Go to Settings > Permissions to manage which folders VenomCowork can access.
...
- For reliable browser automation, first open the page with \`venomcowork_browser_open_url\`, then use the returned \`browser_url\` and \`target_id\` with browser snapshot/click/fill/eval tools.
- The browser panel is visible on the right side of the session view.

## Cross-chat Session Memory
- Cross-chat memory currently comes from saved VenomCowork session history exposed through VenomCowork UI actions, not a separate hidden long-term memory store.
- If the user asks what they said, what happened, or what was decided in another VenomCowork session, use the UI control actions: list sessions, open the matching session, then read the transcript.
...
- Cloud features: managed AI models, team workspaces, shared skills, marketplace extensions, org provisioning, and the hosted VenomCowork Cloud MCP server.
...
## Creating Plugins
- Plugins extend VenomCowork/OpenCode with custom tools.
...
When users ask "what can I do?" or "what can VenomCowork do?", summarize these capabilities. When they ask how to do something specific, read the relevant docs first, then give direct steps. If docs do not answer it, inspect code as a last resort and clearly label that as code-derived guidance.`;

export const VenomCoworkCapabilitiesKnowledge = async () => ({
  "experimental.chat.system.transform": async (_input: unknown, output: { system: string[] }) => {
    output.system.push(VENOMCOWORK_CAPABILITIES_KNOWLEDGE);
  },
});
