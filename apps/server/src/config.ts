...
export async function resolveServerConfig(cli: CliArgs): Promise<ServerConfig> {
  const envConfigPath = process.env.VENOMCOWORK_SERVER_CONFIG;
  const configPath = cli.configPath ?? envConfigPath ?? resolve(homedir(), ".config", "venomcowork", "server.json");
  const fileConfig = await loadFileConfig(configPath);
  const configDir = dirname(configPath);

  const envWorkspaces = parseList(process.env.VENOMCOWORK_WORKSPACES);
  ...
  const envOpencodeBaseUrl = process.env.VENOMCOWORK_OPENCODE_BASE_URL;
  const envOpencodeDirectory = process.env.VENOMCOWORK_OPENCODE_DIRECTORY;
  const envOpencodeUsername = process.env.VENOMCOWORK_OPENCODE_USERNAME;
  const envOpencodePassword = process.env.VENOMCOWORK_OPENCODE_PASSWORD;
  ...
  const tokenFromEnv = process.env.VENOMCOWORK_TOKEN;
  const hostTokenFromEnv = process.env.VENOMCOWORK_HOST_TOKEN;
  ...
  const approvalTimeoutMs =
    ...
    (process.env.VENOMCOWORK_APPROVAL_TIMEOUT_MS ? Number(process.env.VENOMCOWORK_APPROVAL_TIMEOUT_MS) : undefined) ??
  ...
  const envCorsOrigins = process.env.VENOMCOWORK_CORS_ORIGINS;
  ...
  const envReadOnly = process.env.VENOMCOWORK_READONLY;
  ...
  const envLogFormat = process.env.VENOMCOWORK_LOG_FORMAT;
  const logFormat =
    cli.logFormat ??
  ...
  const envLogRequests = parseBoolean(process.env.VENOMCOWORK_LOG_REQUESTS);
  ...
  const host = cli.host ?? process.env.VENOMCOWORK_HOST ?? fileConfig.host ?? DEFAULT_HOST;
  const port = cli.port ?? (process.env.VENOMCOWORK_PORT ? Number(process.env.VENOMCOWORK_PORT) : undefined) ?? fileConfig.port ?? DEFAULT_PORT;
