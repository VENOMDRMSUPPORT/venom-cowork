...
async function listSkillsInDir(dir: string, scope: "project" | "global"): Promise<SkillItem[]> {
  ...
  for (const entry of entries) {
    ...
    if (await exists(skillPath)) {
      ...
      // Domain/category folder: <dir>/<domain>/<name>/SKILL.md â€“ scan one level deeper.
...
export async function listSkills(workspaceRoot: string, includeGlobal: boolean): Promise<SkillItem[]> {
  ...
  if (includeGlobal) {
    const globalVenomCowork = join(homedir(), ".config", "opencode", "skills");
    ...
    items.push(...(await listSkillsInDir(globalVenomCowork, "global")));
