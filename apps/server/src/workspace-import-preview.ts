...
export type WorkspaceImportChangeKind = "opencode" | "venomcowork" | "skill" | "command" | "file";
...
type WorkspaceImportSection = "opencode" | "venomcowork" | "skills" | "commands" | "files";
...
function normalizeModes(value: unknown): Record<string, WorkspaceImportMode> {
  ...
  return {
    ...
    venomcowork: readMode(record.venomcowork),
...
export function normalizeWorkspaceImportPayload(
  ...
  return {
    ...
    sections: {
      ...
      venomcowork: payload.venomcowork !== undefined,
...
export async function buildWorkspaceImportPreview(
  ...
  if (input.venomcowork !== undefined) {
    ...
    changes.push({
      kind: "venomcowork",
      action: actionForTarget(existsBefore, !sameJson(before, after), input.modes.venomcowork),
      label: "VenomCowork config",
      path: rel(workspaceRoot, path),
      absolutePath: path,
