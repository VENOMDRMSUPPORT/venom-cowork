...
const DEFAULT_AGENT = "venomcowork";
const DEFAULT_PROVIDER = "openai";
const DEFAULT_MODEL = "gpt-5.4";
const VENOMCOWORK_DEV_DATA_DIRS = ["venomcowork-dev-data", "opencode-dev"];
...
function opencodeOrchestratorDataDirs(): string[] {
  const root = process.env.VENOMCOWORK_DATA_DIR?.trim();
  if (!root) return [];
  ...
  for (const name of VENOMCOWORK_DEV_DATA_DIRS) {
