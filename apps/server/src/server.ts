...
import { opencodeConfigPath, venomcoworkConfigPath, projectCommandsDir, projectSkillsDir } from "./workspace-files.js";
...
const VENOMCOWORK_VOICE_REALTIME_MODEL = "gpt-realtime-2";
const VENOMCOWORK_VOICE_TRANSCRIPTION_MODEL = "gpt-4o-transcribe";
let desktopCloudSyncQueue: Promise<void> = Promise.resolve();

const VENOMCOWORK_VOICE_REALTIME_TOOLS = [
  {
    type: "function",
    name: "venomcowork_snapshot",
    description: "Read the current VenomCowork UI control snapshot: route, status, narration, and visible action metadata.",
    parameters: { type: "object", properties: {}, additionalProperties: false },
  },
  {
    type: "function",
    name: "venomcowork_list_actions",
    description: "List semantic VenomCowork UI actions. Call this before venomcowork_execute_action when you do not know the exact action id.",
    parameters: { type: "object", properties: {}, additionalProperties: false },
  },
  {
    type: "function",
    name: "venomcowork_execute_action",
    description: "Execute a semantic VenomCowork UI action by id. Prefer this over screen coordinates or DOM guessing.",
    parameters: {
      type: "object",
      properties: {
        actionId: { type: "string", description: "The action id from venomcowork_list_actions, such as composer.set_text or composer.send." },
...
function legacyRuntimeConfigFromVenomcoworkConfig(venomcowork: Record<string, unknown>): {
  ...
  const plugin = Array.isArray(venomcowork.plugin) ? venomcowork.plugin.filter((item) => typeof item === "string") : [];
  const mcp: Record<string, Record<string, unknown>> = {};
  if (isRecord(venomcowork.mcp)) {
    for (const [name, value] of Object.entries(venomcowork.mcp)) {
      if (isRecord(value)) mcp[name] = value;
    }
  }
  const permission = isRecord(venomcowork.permission) ? venomcowork.permission : null;
  const externalDirectory = permission && isRecord(permission.external_directory) ? permission.external_directory : null;
  const provider = isRecord(venomcowork.provider) ? venomcowork.provider : null;
...
function removeLegacyRuntimeConfig(venomcowork: Record<string, unknown>): Record<string, unknown> {
  const next = { ...venomcowork };
...
function userRuntimeConfigFromOpencodeConfig(opencode: Record<string, unknown>): {
  ...
  const defaultAgent = opencode.default_agent === "venomcowork" ? "venomcowork" : undefined;
...
function venomcoworkWorkspaceDisplayName(workspace: Record<string, unknown>): string | null {
  return readStringField(workspace, "displayName")
    || readStringField(workspace, "venomcoworkWorkspaceName")
...
async function fetchVenomcoworkWorkspaceList(hostUrl: string, token: string, hostToken: string): Promise<unknown> {
  ...
  if (hostToken) headers.set("X-VenomCowork-Host-Token", hostToken);
  ...
  try {
    ...
    if (!response.ok) {
      throw new ApiError(
        502,
        "venomcowork_workspace_discovery_failed",
        `VenomCowork workspace discovery failed (${response.status} ${response.statusText || "HTTP error"})`,
      );
    }
    return await response.json();
  } catch (error) {
    if (error instanceof ApiError) throw error;
    throw new ApiError(502, "venomcowork_workspace_discovery_failed", "VenomCowork workspace discovery failed", {
      error: String(error),
    });
  } finally {
...
async function resolveOpenAiRealtimeApiKey(env: EnvService): Promise<string> {
  ...
  return process.env.VENOMCOWORK_OPENAI_REALTIME_API_KEY?.trim() ||
    process.env.OPENAI_REALTIME_API_KEY?.trim() ||
    process.env.OPENAI_API_KEY?.trim() ||
    "";
}

function venomcoworkVoiceRealtimeInstructions() {
  return `# Role and Objective

You are VenomCowork Voice Mode, a voice-first control layer inside VenomCowork.
Help the user control VenomCowork by using the semantic VenomCowork UI tools.

# Tool Policy

- Prefer venomcowork_snapshot, venomcowork_list_actions, and venomcowork_execute_action over visual guessing.
...
- Ignore background speech that is not addressed to VenomCowork.
...
async function createOpenAiRealtimeVoiceSession(env: EnvService, input: unknown) {
  ...
  if (!apiKey) {
    throw new ApiError(
      ...
      "OpenAI API key missing. Save OPENAI_API_KEY in VenomCowork Environment Variables or configure the Voice Mode extension.",
    );
  }

  const model = readStringField(input, "model") || VENOMCOWORK_VOICE_REALTIME_MODEL;
  const response = await fetch("https://api.openai.com/v1/realtime/client_secrets", {
    ...
    body: JSON.stringify({
      session: {
        ...
        audio: {
          input: {
            transcription: { model: VENOMCOWORK_VOICE_TRANSCRIPTION_MODEL, language: "en" },
            turn_detection: {
              type: "server_vad",
              threshold: 0.58,
        ...
        instructions: venomcoworkVoiceRealtimeInstructions(),
        tool_choice: "auto",
        tools: VENOMCOWORK_VOICE_REALTIME_TOOLS,
      },
    }),
  });
  ...
  return {
    ...
    expiresAt,
    model,
    transcriptionModel: VENOMCOWORK_VOICE_TRANSCRIPTION_MODEL,
    tools: VENOMCOWORK_VOICE_REALTIME_TOOLS.map((tool) => tool.name),
  };
}
...
export function createServerLogger(config: ServerConfig): ServerLogger {
  const runId = process.env.VENOMCOWORK_RUN_ID ?? shortId();
    ...
    fetch: async (request: Request) => {
      ...
      try {
        ...
        if (!(error instanceof ApiError)) {
          console.error("[venomcowork-server] Unhandled error:", error);
        }
        const apiError = error instanceof ApiError
          ? error
          : new ApiError(500, "internal_error", "Unexpected server error");
...
async function proxyOpencodeRequest(input: {
  ...
  headers.delete("x-venomcowork-host-token");
  headers.delete("x-venomcowork-client-id");
  headers.delete("host");
...
function withCors(response: Response, request: Request, config: ServerConfig) {
  ...
  headers.set(
    ...
    "Authorization, Content-Type, X-VenomCowork-Host-Token, X-VenomCowork-Client-Id, X-OpenCode-Directory, X-Opencode-Directory, x-opencode-directory",
  );
...
async function requireClient(request: Request, config: ServerConfig, tokens: TokenService): Promise<Actor> {
  ...
  const clientId = request.headers.get("x-venomcowork-client-id") ?? undefined;
...
function requireHostToken(request: Request, config: ServerConfig): Actor {
  const hostToken = request.headers.get("x-venomcowork-host-token");
  ...
  return {
    ...
    toolProviders: {
      ...
      files: {
        ...
        inboxPath: ".opencode/venomcowork/inbox/",
        outboxPath: ".opencode/venomcowork/outbox/",
        maxBytes,
      },
    },
  };
...
function resolveSandboxBackend(): Capabilities["sandbox"]["backend"] {
  const raw = (process.env.VENOMCOWORK_SANDBOX_BACKEND ?? "").trim().toLowerCase();
...
function resolveSandboxEnabled(backend: Capabilities["sandbox"]["backend"]): boolean {
  const raw = (process.env.VENOMCOWORK_SANDBOX_ENABLED ?? "").trim().toLowerCase();
...
function resolveInboxEnabled(): boolean {
  const raw = (process.env.VENOMCOWORK_INBOX_ENABLED ?? "").trim().toLowerCase();
  if (!raw) return true;
  return ["1", "true", "yes", "on"].includes(raw);
}

function resolveOutboxEnabled(): boolean {
  const raw = (process.env.VENOMCOWORK_OUTBOX_ENABLED ?? "").trim().toLowerCase();
  if (!raw) return true;
  return ["1", "true", "yes", "on"].includes(raw);
}

function resolveInboxMaxBytes(): number {
  const raw = (process.env.VENOMCOWORK_INBOX_MAX_BYTES ?? "").trim();
...
function resolveToyUiEnabled(): boolean {
  const raw = (process.env.VENOMCOWORK_TOY_UI ?? "").trim().toLowerCase();
  if (!raw) return true;
  return ["1", "true", "yes", "on"].includes(raw);
}

// Dev-only log sink target. When VENOMCOWORK_DEV_LOG_FILE is set to a path, the
...
function resolveDevLogPath(): string | null {
  const raw = (process.env.VENOMCOWORK_DEV_LOG_FILE ?? "").trim();
  return raw.length > 0 ? raw : null;
}

function resolveBrowserProvider(): Capabilities["toolProviders"]["browser"] {
  const raw = (process.env.VENOMCOWORK_BROWSER_PROVIDER ?? "").trim().toLowerCase();
...
function resolveInboxDir(workspaceRoot: string): string {
  return join(workspaceRoot, ".opencode", "venomcowork", "inbox");
}

function resolveOutboxDir(workspaceRoot: string): string {
  return join(workspaceRoot, ".opencode", "venomcowork", "outbox");
}
...
function createRoutes(
  ...
  // tokens; it is also a no-op when VENOMCOWORK_DEV_LOG_FILE is unset.
  ...
  addRoute(routes, "GET", "/ui/assets/venomcowork-mark.svg", "none", async () => {
    if (!resolveToyUiEnabled()) {
      throw new ApiError(404, "ui_disabled", "Toy UI is disabled");
    }
      ...
      if (error instanceof InvalidEnvKeyError) {
        throw new ApiError(
          ...
          error.code === "reserved_env_key"
            ? "Environment variable name is reserved for VenomCowork internals"
  ...
  addRoute(routes, "POST", "/workspaces/remote", "host", async (ctx) => {
    ...
    if (remoteType === "venomcowork" && !venomcoworkWorkspaceId) {
      ...
      venomcoworkWorkspaceId = discovered ? readStringField(discovered, "id") : "";
      venomcoworkWorkspaceName = discovered ? venomcoworkWorkspaceDisplayName(discovered) : venomcoworkWorkspaceName;
      if (!venomcoworkWorkspaceId) {
        throw new ApiError(
          400,
          "venomcowork_workspace_not_found",
          directory
    ...
    const workspace: WorkspaceInfo = {
      id: remoteType === "venomcowork"
        ? venomcoworkRemoteWorkspaceId(venomcoworkHostUrl ?? baseUrl, venomcoworkWorkspaceId)
        : workspaceIdForRemote(baseUrl, directory),
      name: displayName ?? venomcoworkWorkspaceName ?? "Remote workspace",
      path: directory ?? "",
      preset: "remote",
      ...
      ...(remoteType === "venomcowork" && venomcoworkHostUrl ? { venomcoworkHostUrl } : {}),
      ...(venomcoworkToken ? { venomcoworkToken } : {}),
      ...(remoteType === "venomcowork" && venomcoworkWorkspaceId ? { venomcoworkWorkspaceId } : {}),
      ...(remoteType === "venomcowork" && venomcoworkWorkspaceName ? { venomcoworkWorkspaceName } : {}),
  ...
  addRoute(routes, "DELETE", "/workspaces/:id", "host", async (ctx) => {
    ...
    await recordAudit(workspace.path, {
      ...
      target: "workspace",
      summary: "Deleted workspace from VenomCowork server",
      timestamp: Date.now(),
    });
  ...
  addRoute(routes, "GET", "/workspace/:id/config", "client", async (ctx) => {
    ...
    return jsonResponse({ opencode, venomcowork, updatedAt: lastAudit?.timestamp ?? null });
  });
  ...
  addRoute(routes, "POST", "/workspace/:id/desktop-cloud-sync", "client", async (ctx) => {
    ...
    const result = await enqueueDesktopCloudSync(async () => {
      ...
      await recordAudit(workspace.path, {
        ...
        target: venomcoworkConfigPath(workspace.path),
  ...
  addRoute(routes, "POST", "/workspace/:id/cloud-plugins", "client", async (ctx) => {
    ...
    await requireApproval(ctx, {
      ...
      paths: [venomcoworkConfigPath(workspace.path), join(workspace.path, ".opencode")],
    });
  ...
  addRoute(routes, "PUT", "/workspace/:id/authorized-folders", "client", async (ctx) => {
    ...
    const configPath = venomcoworkConfigPath(workspace.path);
  ...
  addRoute(routes, "POST", "/workspace/:id/runtime-config/migrate", "client", async (ctx) => {
    ...
    return jsonResponse({ migrated: true, keys, legacyKeys: legacy.keys, userOpencodeKeys: user.keys, updatedAt, legacyError: venomcowork.error });
  });
  ...
  addRoute(routes, "PATCH", "/workspace/:id/config", "client", async (ctx) => {
    ...
    const venomcowork = body.venomcowork as Record<string, unknown> | undefined;

    if (!opencode && !venomcowork) {
      throw new ApiError(400, "invalid_payload", "opencode or venomcowork updates required");
    }
    ...
    await requireApproval(ctx, {
      ...
      paths: [opencode || venomcowork ? venomcoworkConfigPath(workspace.path) : null].filter(Boolean) as string[],
    });
    ...
    if (opencode) {
      emitReloadEvent(ctx.reloadEvents, workspace, "config", buildConfigTrigger(venomcoworkConfigPath(workspace.path)));
    }
  ...
  addRoute(routes, "POST", "/workspace/:id/plugins", "client", async (ctx) => {
    ...
    await requireApproval(ctx, {
      ...
      paths: [venomcoworkConfigPath(workspace.path)],
    });
  ...
  // Toggle `enabled` on a workspace MCP. Strict body validation â€” `Boolean(body.enabled)`
...
function serializeWorkspaceConfigEntry(workspace: WorkspaceInfo): Record<string, unknown> {
  ...
  return {
    ...
    ...(workspace.venomcoworkHostUrl ? { venomcoworkHostUrl: workspace.venomcoworkHostUrl } : {}),
    ...(workspace.venomcoworkToken ? { venomcoworkToken: workspace.venomcoworkToken } : {}),
    ...(workspace.venomcoworkWorkspaceId ? { venomcoworkWorkspaceId: workspace.venomcoworkWorkspaceId } : {}),
    ...(workspace.venomcoworkWorkspaceName ? { venomcoworkWorkspaceName: workspace.venomcoworkWorkspaceName } : {}),
...
function getRuntimeControlConfig(): { baseUrl: string; token: string } | null {
  const baseUrl = process.env.VENOMCOWORK_CONTROL_BASE_URL?.trim() ?? "";
  const token = process.env.VENOMCOWORK_CONTROL_TOKEN?.trim() ?? "";
...
async function readVenomcoworkConfig(workspaceRoot: string): Promise<Record<string, unknown>> {
  ...
  try {
    ...
    throw new ApiError(422, "invalid_json", "Failed to parse venomcowork.json");
  }
}
...
async function materializeBlueprintSessions(config: ServerConfig, workspace: WorkspaceInfo): Promise<{
  ...
  const existing = readMaterializedBlueprintSessions(venomcowork);
