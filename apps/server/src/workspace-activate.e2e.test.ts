...
async function createWorkspaceRoot() {
  const root = await mkdtemp(join(tmpdir(), "venomcowork-activate-"));
...
function hostAuth(token: string) {
  return { "X-VenomCowork-Host-Token": token };
}
...
describe("workspace activation", () => {
  test("reloads the bound OpenCode engine on activate", async () => {
    ...
    const response = await fetch(`${base}/workspaces/ws_1/activate`, {
      method: "POST",
      headers: hostAuth(venomcowork.hostToken),
    });
  ...
  test("persists activation order only when requested", async () => {
    ...
    const bodyPersistedResponse = await fetch(`${base}/workspaces/ws_1/activate`, {
      method: "POST",
      headers: { ...hostAuth(venomcowork.hostToken), "Content-Type": "application/json" },
...
describe("workspace lifecycle registry", () => {
  ...
  test("creates and persists remote VenomCowork workspace records", async () => {
    const workspaceRoot = await createWorkspaceRoot();
    ...
    const response = await fetch(`${base}/workspaces/remote`, {
      ...
      body: JSON.stringify({
        ...
        venomcoworkHostUrl: `http://127.0.0.1:${remote.server.port}`,
        venomcoworkToken: "remote_token",
    ...
    expect(body.workspaces[0].venomcoworkWorkspaceId).toBe("ws_remote");
    expect(body.workspaces[0].venomcoworkWorkspaceName).toBe("Remote Project");
    ...
    expect(workspaces[0]?.remoteType).toBe("venomcowork");
