...
async function withWorkspace(fn: (input: { root: string; config: ServerConfig }) => Promise<void>) {
  const root = await mkdtemp(join(tmpdir(), "venomcowork-runtime-config-"));
  const previousDb = process.env.VENOMCOWORK_RUNTIME_DB;
  const dbPath = join(root, "runtime.sqlite");
  process.env.VENOMCOWORK_RUNTIME_DB = dbPath;
  try {
    await fn({ root, config: serverConfig(root, dbPath) });
  } finally {
    if (previousDb === undefined) delete process.env.VENOMCOWORK_RUNTIME_DB;
    else process.env.VENOMCOWORK_RUNTIME_DB = previousDb;
...
describe("runtime OpenCode config store", () => {
  test("stores MCP changes in the VenomCowork runtime DB without rewriting workspace files", async () => {
    await withWorkspace(async ({ root, config }) => {
      ...
      await expectMissing(join(root, ".opencode", "venomcowork.json"));
  ...
  test("stores plugin changes in the VenomCowork runtime DB without rewriting workspace files", async () => {
    await withWorkspace(async ({ root, config }) => {
      const opencodePath = join(root, "opencode.jsonc");
  ...
  test("stores VenomCowork-owned workspace config in the runtime DB without writing legacy files", async () => {
    await withWorkspace(async ({ root, config }) => {
  ...
  test("explicitly migrates legacy VenomCowork runtime config into the runtime DB", async () => {
    await withWorkspace(async ({ root, config }) => {
      await mkdir(join(root, ".opencode"), { recursive: true });
      const venomcoworkPath = join(root, ".opencode", "venomcowork.json");
      await writeFile(venomcoworkPath, JSON.stringify({
        version: 1,
        workspace: { name: "Test" },
      ...
      try {
        ...
        const venomcowork = JSON.parse(await readFile(venomcoworkPath, "utf8")) as Record<string, unknown>;
        expect(venomcowork.version).toBe(1);
        expect(venomcowork.workspace).toEqual({ name: "Test" });
        expect(venomcowork.plugin).toBeUndefined();
        expect(venomcowork.mcp).toBeUndefined();
        expect(venomcowork.permission).toBeUndefined();
        expect(venomcowork.provider).toBeUndefined();
        ...
        expect(status.effectiveRuntime.agent).toMatchObject({ venomcowork: { mode: "primary" } });
        expect(status.effectiveRuntime.provider).toMatchObject({ legacy: { npm: "legacy-provider" } });
        expect(status.sources.injected.config?.agent).toMatchObject({ venomcowork: { mode: "primary" } });
  ...
  test("runtime config status tolerates malformed legacy VenomCowork metadata", async () => {
    await withWorkspace(async ({ root, config }) => {
      await mkdir(join(root, ".opencode"), { recursive: true });
      await writeFile(join(root, ".opencode", "venomcowork.json"), "{ invalid\n", "utf8");
  ...
  test("explicitly migrates safe VenomCowork-managed keys from user opencode config", async () => {
    await withWorkspace(async ({ root, config }) => {
      const opencodePath = join(root, "opencode.jsonc");
      ...
      try {
        ...
        const runtime = await readRuntimeOpencodeConfig(config, WORKSPACE_ID);
        expect(runtime.default_agent).toBe("venomcowork");
