...
try {
  const result = await run(
    "npx",
    ["--yes", "--package", tarballPath, "venomcowork-router", "--help"],
    {
      env,
      timeoutMs: 60000,
    },
  ...
  if (result.code !== 0) {
    throw new Error(result.stderr || "npx venomcowork-router failed");
  }

  if (!result.stdout.includes("venomcowork-router") && !result.stdout.includes("owpenbot")) {
    throw new Error("npx output missing expected command name");
  }

  console.log("npx venomcowork-router ok");
} finally {
