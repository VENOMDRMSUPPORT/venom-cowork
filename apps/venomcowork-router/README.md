...
- `OPENCODE_ROUTER_HEALTH_PORT` controls the port (VenomCowork defaults to a random free port when using `venomcowork`).
...
- SQLite at `~/.venomcowork/opencode-router/opencode-router.db` unless overridden.
- Config stored at `~/.venomcowork/opencode-router/opencode-router.json` (created by `opencode-router` or `pnpm -C apps/opencode-router setup`).
