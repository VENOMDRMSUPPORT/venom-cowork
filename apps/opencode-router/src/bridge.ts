...
export async function startBridge(config: Config, logger: Logger, reporter?: BridgeReporter, deps: BridgeDeps = {}) {
  ...
  async function handleTelegramPairingGate(input: {
    ...
    if (!pairingCode) {
      await sendText(
        ...
        input.identityId,
        input.peerId,
        "This Telegram bot is private. Ask your VenomCowork host for the pairing code, then send /pair <code>.",
        { kind: "system" },
      );
      return "handled";
    }
    ...
    if (!access.pairingCodeHash) {
      await sendText(
        ...
        input.identityId,
        input.peerId,
        "This Telegram bot is private but missing a pairing code. Ask your VenomCowork host to reconnect it.",
        { kind: "system" },
      );
      return "handled";
    }
    ...
    if (!boundDirectoryCandidate || (!hasExplicitBinding && isDangerousRootDirectory(boundDirectoryCandidate))) {
      await sendText(
        ...
        input.identityId,
        input.peerId,
        "No workspace directory configured for this identity. Ask your VenomCowork host to set it, or reply with /dir <path>.",
        { kind: "system" },
      );
      return "handled";
    }
  ...
  async function handleInbound(message: InboundMessage) {
    ...
    enqueue(key, async () => {
      ...
      try {
        ...
        const promptText = [
          "You are handling a Slack/Telegram message via VenomCowork.",
  ...
  async function createSession(input: {
    ...
    await sendText(input.channel, input.identityId, input.peerId, "ðŸ§­ Session started.", { kind: "system" });
    return sessionID;
  }
