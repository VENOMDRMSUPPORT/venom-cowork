 ...
 * refreshRouteState() call â€” without crawling the UI tree.
 *
 * Consumers read via `window.__venomcowork` from a devtools console or from
...
declare global {
  ...
  var __venomcowork: InspectorAPI | undefined;
}
...
function installIfNeeded() {
  ...
  Object.defineProperty(window, "__venomcowork", {
    value: api,
    configurable: true,
    writable: false,
  });
...
export function recordInspectorEvent(name: string, data?: unknown) {
  installIfNeeded();
  window.__venomcowork?.record(name, data);
}
