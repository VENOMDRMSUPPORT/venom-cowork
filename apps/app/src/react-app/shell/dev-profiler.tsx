 ...
 * localStorage.venomcowork.debug.profilerOverlay = "1" / "0".
 *
 * In prod builds the wrapper is a pass-through (no Profiler overhead) and
 * the overlay renders null.
 *
 * Findings also land on window.__venomcowork.slice("profiler") so external
 * tools can read them.
 */
...
//
// Explicit opt-ins:
//   - VITE_VENOMCOWORK_PROFILER=1 at `pnpm dev`
//   - window.localStorage.setItem("venomcowork.debug.profiler", "1")
...
const PROFILER_ENABLED = (() => {
  ...
  try {
    ...
    const flag = env.VITE_VENOMCOWORK_PROFILER;
    if (flag === "1" || flag === "true" || flag === true) return true;
  } catch {
    // ignore
  }
  try {
    if (window.localStorage.getItem("venomcowork.debug.profiler") === "1") return true;
  } catch {
    // ignore
  }
  return false;
...
// window.__venomcowork.slice("profiler") â€” even for operators who aren't
...
function recordCommit(record: CommitRecord) {
  ...
  // reader), don't mutate the map at all. This is critical â€” otherwise the
...
function readOverlayStoredPreference(): boolean | null {
  ...
  try {
    const raw = window.localStorage.getItem("venomcowork.debug.profilerOverlay");
...
function writeOverlayStoredPreference(value: boolean) {
  if (typeof window === "undefined") return;
  try {
    window.localStorage.setItem("venomcowork.debug.profilerOverlay", value ? "1" : "0");
  } catch {
    // ignore
  }
}
 ...
 * `recordCommit` short-circuits â€” the profiler becomes free.
 */
      ...
      <div className="flex items-center justify-between border-b border-dls-border px-2.5 py-1.5">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] text-dls-secondary">
            {snapshot.totalCommits} commits Â· {snapshot.totalActualMs}ms
          </span>
        </div>
                    ...
                    >
                      <td className="px-2 py-1 font-mono text-[11px] text-dls-text">
                        ...
                        <span className="block text-[9px] uppercase tracking-[0.1em] text-dls-secondary">
                          {zone.mountCount}m Â· {zone.updateCount}u
                        </span>
                      </td>
