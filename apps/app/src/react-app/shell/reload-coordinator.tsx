...
const RELOAD_TOAST_ID = "venomcowork-reload-required";
...
export const orgOnboardingVisibilityEvent = "venomcowork-org-onboarding-visibility";
...
export function ReloadCoordinatorProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    window.addEventListener("venomcowork-reload-required", handler);

    return () => window.removeEventListener("venomcowork-reload-required", handler);
