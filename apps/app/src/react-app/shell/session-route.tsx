...
import { VenomCoworkModelsStartupDialog } from "@/react-app/domains/cloud/venomcowork-models-startup-dialog";
import {
...
const reloadAfterOrgOnboardingKey = "venomcowork.reloadAfterOrgOnboarding";
...
function describeTaskCreateError(error: unknown) {
  ...
  if (
    ...
    return "OpenCode is unavailable for this workspace. Retry once it restarts, or restart VenomCowork if the problem continues.";
  }
  return message;
}
...
function focusPromptSoon() {
  ...
  const focus = () => window.dispatchEvent(new Event("venomcowork:focusPrompt"));
...
function mergeRouteWorkspaces(
  ...
  // skip the server's view entirely. The local VenomCowork server may have stale
...
export function SessionRoute() {
  ...
  const hasVenomCoworkModels = useMemo(
    () => hasVenomCoworkModelsProvider(providerConnectedIds),
    [providerConnectedIds],
  );

  const subscribeToVenomCoworkModels = useCallback(() => {
    setVenomCoworkModelsStartupOpen(false);
    markVenomCoworkModelsStartupPromoShown();
  ...
  const continueWithoutVenomCoworkModels = useCallback(() => {
    setVenomCoworkModelsStartupOpen(false);
    markVenomCoworkModelsStartupPromoShown();
    hideVenomCoworkModelsPromo();
  ...
  const shareWorkspaceState = useShareWorkspaceState({
    workspaces,
    venomcoworkServerHostInfo: venomcoworkServerHostInfoState,
    venomcoworkServerSettings,
  ...
  const refreshRouteState = useCallback(async () => {
    ...
    try {
      ...
      const list = await venomcoworkClient.listWorkspaces();
  ...
  // `window.__venomcowork.snapshot()` or `window.__venomcowork.slice("route")` and
  ...
  // local workspaces it's the user's local VenomCowork server.
  ...
  const sessionProviderAuthStore = useMemo(
    () =>
      createProviderAuthStore({
        ...
        venomcoworkServer: {
          getSnapshot: () => ({
            venomcoworkServerStatus: sessionProviderAuthStateRef.current.selectedWorkspaceEndpoint ? "connected" : "disconnected",
            venomcoworkServerClient: sessionProviderAuthStateRef.current.selectedWorkspaceEndpoint?.client ?? null,
            venomcoworkServerCapabilities: sessionProviderAuthStateRef.current.selectedWorkspaceEndpoint
              ? {
  ...
  const surfaceProps = useMemo(() => {
    ...
    // `opencodeBaseUrl`, or `venomcoworkToken` here. SessionPage forwards those
  ...
  const handleCreateTaskInWorkspace = useCallback(async (workspaceId: string) => {
    ...
    const workspaceClient = createClient(
      ...
      { token: endpoint.token, mode: "venomcowork" },
    );
    try {
      setErrorsByWorkspaceId((current) => ({ ...current, [workspaceId]: null }));
      setRouteError(null);
  ...
  return (
    ...
    {opencodeClient && selectedWorkspaceEndpoint && opencodeBaseUrl && selectedWorkspaceServerToken ? (
      <ReactSessionRuntime
        ...
        opencodeBaseUrl={opencodeBaseUrl}
        venomcoworkToken={selectedWorkspaceServerToken}
    ...
    <SessionPage
      ...
      venomcoworkServerStatus={client ? "connected" : "disconnected"}
      venomcoworkServerClient={selectedWorkspaceEndpoint?.client ?? client}
      venomcoworkServerToken={selectedWorkspaceServerToken}
      developerMode={typeof window !== "undefined" && window.localStorage.getItem("venomcowork.developerMode") === "1"}
      ...
      settingsSlot={
        <SettingsSurface
          ...
          onClose={() => {
            try {
              window.dispatchEvent(new CustomEvent("venomcowork-close-right-pane"));
            } catch {
              // ignore
            }
          }}
      ...
      sidebar={{
        ...
        onSelectWorkspace: async (workspaceId) => {
          ...
          // Tell the VenomCowork server this workspace is now active so it can
          ...
          ? {
              ...
              remoteAccess:
                isDesktopRuntime() && shareWorkspaceState.shareWorkspace?.workspaceType === "local"
                  ? {
                      enabled: venomcoworkServerSettings.remoteAccessEnabled === true,
    ...
    <VenomCoworkModelsStartupDialog
      ...
      onContinueWithout={continueWithoutVenomCoworkModels}
    />
    ...
    <CommandPalette
      ...
      onOpenAccessibleTarget={(target) => {
        try {
          window.dispatchEvent(new CustomEvent("venomcowork-open-accessible-target", { detail: target }));
        } catch {
      ...
      onHideAccessibleTarget={(target) => {
        try {
          window.dispatchEvent(new CustomEvent("venomcowork-hide-accessible-target", { detail: target }));
        } catch {
          // ignore event dispatch failures
        }
      }}
