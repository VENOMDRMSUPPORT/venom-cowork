 ...
 *   2) if a local workspace is selected, restart the embedded VenomCowork server
 *   3) start the OpenCode engine pointed at the workspace
 *   4) activate the workspace on the running VenomCowork server
 ...
 *      localhost ports/tokens into VenomCowork settings.
 *
 * Safe to call multiple times â€” gated by a `didBoot` ref so it runs once per mount.
 */
    ...
    void (async () => {
      try {
        ...
        const startServerWithoutDesktopWorkspace = async () => {
          setPhase("starting-engine", "Starting VenomCowork server");
          const serverInfo = await venomcoworkServerRestart({ remoteAccessEnabled: preferredRemoteAccess }).catch((error) => {
        ...
        if (isElectronRuntime()) {
          ...
          if (boot.ok === false) {
            setError(boot.error || "Failed to start VenomCowork runtime");
            return;
          }
          ...
          let serverInfo = boot.venomcoworkServer;
          if (preferredRemoteAccess && serverInfo?.remoteAccessEnabled !== true) {
            const restarted = await venomcoworkServerRestart({ remoteAccessEnabled: true }).catch((error) => {
              console.warn("[desktop-boot] venomcoworkServerRestart failed:", error);
              return null;
            });
        ...
        // FAST PATH â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        // Cheap status probe: if engine is already running just publish the
        // current venomcowork-server base URL + token and finish in <1s.
            ...
            if (fresh?.baseUrl) {
              ...
              try {
                window.dispatchEvent(
                  new CustomEvent("venomcowork-server-settings-changed"),
                );
              } catch {
                /* ignore */
              }
        ...
        // SLOW PATH â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        // No running engine. Tauri now mirrors Electron: engine_start boots
        // venomcowork-server and lets that server manage OpenCode.
        ...
        if (engineStartResult) {
          ...
          try {
            ...
            console.warn("[desktop-boot] post-engineStart venomcoworkServerInfo failed:", error);
          }
        }
