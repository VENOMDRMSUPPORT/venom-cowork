...
import "@/react-app/domains/settings/venomcowork-voice-config";
...
import { isVenomCoworkExtensionEnabled, VENOMCOWORK_EXTENSION_STATE_CHANGED, setVenomCoworkExtensionEnabled } from "@/react-app/domains/settings/extension-state";
...
function isVenomCoworkCloudProvider(provider: {
  providerId?: string | null;
  source?: string | null;
  sourceProviderId?: string | null;
}) {
  return [provider.providerId, provider.source, provider.sourceProviderId].some(
    (value) => value?.trim().toLowerCase() === "venomcowork",
  );
}
...
function describeWorkspaceCreateError(error: unknown) {
  ...
  if (
    ...
    return `${message}\n\nVenomCowork could not read the workspace config before the filesystem timed out. This often happens when the folder is still syncing from iCloud Drive or another remote folder. Wait for the folder to finish downloading, move the workspace to a local folder, or try again.`;
  }
  return message;
}
...
const SETTINGS_HIDE_TITLEBAR_KEY = "venomcowork.react.settings.hide-titlebar";
...
function SettingsRouteContent(props: SettingsSurfaceProps = {}) {
  ...
  const [developerMode, setDeveloperMode] = useState(() => {
    ...
    return window.localStorage.getItem("venomcowork.developerMode") === "1";
  });
    ...
    () =>
      selectedWorkspace
        ? {
            ...
            venomcoworkWorkspaceName: selectedWorkspace.venomcoworkWorkspaceName,
          }
  ...
  routeStateRef.current = {
    ...
    venomcoworkServerStatus: venomcoworkClient ? "connected" : "disconnected",
    venomcoworkServerCapabilities: venomcoworkClient ? ROUTE_VENOMCOWORK_CAPABILITIES : null,
  ...
  const reloadWorkspaceEngineFromUi = useCallback(async () => {
    ...
    if (!venomcoworkClient || !workspaceId) {
      toast.error(t("app.error_connect_first"));
      return false;
    }

    await venomcoworkClient.reloadEngine(workspaceId);
    ...
    return true;
  }, [venomcoworkClient, selectedWorkspaceId]);

  useEffect(() => {
    return reloadCoordinator.registerWorkspaceReloadControls({
      canReloadWorkspaceEngine: () => Boolean(venomcoworkClient && (selectedWorkspace?.id || selectedWorkspaceId)),
  ...
  const providerAuthStore = useMemo(
    ...
    [checkDesktopRestriction, venomcoworkServerStore, reloadCoordinator.markReloadRequired],
  );
    ...
    () =>
      createExtensionsStore({
        ...
        venomcoworkServerConnection: () => ({
          venomcoworkServerClient: routeStateRef.current.venomcoworkServerClient,
          venomcoworkServerStatus: routeStateRef.current.venomcoworkServerStatus,
          venomcoworkServerCapabilities: routeStateRef.current.venomcoworkServerCapabilities,
        }),
        runtimeWorkspaceId: () => routeStateRef.current.runtimeWorkspaceId,
        ensureRuntimeWorkspaceId: async () =>
          routeStateRef.current.runtimeWorkspaceId?.trim() ||
  ...
  const hasVenomCoworkCloudProvider = useMemo(
    () =>
      providerAuthSnapshot.cloudOrgProviders.some(isVenomCoworkCloudProvider) ||
  ...
  const subscribeToVenomCoworkModels = useCallback(() => {
  ...
  const shareWorkspaceState = useShareWorkspaceState({
    workspaces,
    venomcoworkServerHostInfo: venomcoworkServerSnapshot.venomcoworkServerHostInfo,
    venomcoworkServerSettings: venomcoworkServerSnapshot.venomcoworkServerSettings,
    engineInfo: null,
    exportWorkspaceBusy,
    openLink: (url) => platform.openLink(url),
    workspaceLabel,
  ...
  useEffect(() => {
    ...
    window.addEventListener(VENOMCOWORK_EXTENSION_STATE_CHANGED, refresh);
    window.addEventListener("storage", refresh);
    return () => {
      window.removeEventListener(VENOMCOWORK_EXTENSION_STATE_CHANGED, refresh);
  ...
  useEffect(() => {
    const client = selectedWorkspaceEndpoint?.client ?? venomcoworkClient;
    if (!client) {
  ...
  }, [venomcoworkClient, selectedWorkspaceEndpoint]);

  useEffect(() => {
    if (!venomcoworkClient) {
      setUserEnvKeys([]);
      return;
    }
    let cancelled = false;
    void venomcoworkClient.listUserEnvKeys()
  ...
  const installOpenAiImageExtension = useCallback(async (apiKey: string) => {
    ...
    if (!venomcoworkClient) {
      setImageExtensionError("VenomCowork server is not connected.");
      return;
    }
    if (!resolvedApiKey) {
    ...
    try {
      await venomcoworkClient.upsertUserEnv([{ key: "OPENAI_API_KEY", value: resolvedApiKey }]);
      setUserEnvKeys((current) => Array.from(new Set([...current, "OPENAI_API_KEY"])));
      setImageExtensionStatus("Saved OPENAI_API_KEY. Agents can use VenomCowork extension actions for image generation.");
    } catch (error) {
  ...
  const generateOpenAiTestImage = useCallback(async (input: { apiKey: string; prompt: string }) => {
    ...
    if (!client || !workspaceId) {
      setImageGenerationError("VenomCowork server is not connected for this workspace.");
      return;
    }
    if (!apiKey) {
    ...
    try {
      if (venomcoworkClient) {
        await venomcoworkClient.upsertUserEnv([{ key: "OPENAI_API_KEY", value: apiKey }]);
        setUserEnvKeys((current) => Array.from(new Set([...current, "OPENAI_API_KEY"])));
      }
  ...
  }, [venomcoworkClient, runtimeWorkspaceId, selectedWorkspaceEndpoint, selectedWorkspaceRoot]);

  const saveVoiceApiKey = useCallback(async (apiKey: string) => {
    const resolvedApiKey = apiKey.trim();
    if (!venomcoworkClient || !resolvedApiKey) {
  ...
  const testVoiceSession = useCallback(async () => {
    if (!venomcoworkClient) {
      setVoiceError("VenomCowork server is not connected.");
      return;
    }
    setVoiceBusy(true);
    setVoiceStatus(null);
    setVoiceError(null);
    try {
      const session = await venomcoworkClient.createVoiceRealtimeSession();
      setVoiceStatus(`Realtime ready with ${session.model} (${session.tools.length} VenomCowork tools).`);
    } catch (error) {
      setVoiceError(describeRouteError(error));
    } finally {
      setVoiceBusy(false);
  ...
  const installLocalProvider = useCallback(async (input: LocalProviderInstallInput) => {
    ...
    if (!client || !workspaceId) {
      setLocalProviderError("VenomCowork server is not connected for this workspace.");
      return;
    }
    if (!modelId) {
  ...
  }, [local, venomcoworkClient, reloadCoordinator, runtimeWorkspaceId, selectedWorkspaceEndpoint]);
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        try {
          const raw = window.localStorage.getItem("venomcowork.seenProviderIds");
  ...
  }, [loading, venomcoworkClient, selectedWorkspace, workspaces]);
  ...
  useEffect(() => {
    ...
    return () => {
      window.removeEventListener("venomcowork-server-settings-changed", handleSettingsChange);
    };
  }, [refreshRouteState]);

  // Load auto-compaction state from OpenCode config on workspace change.
  useEffect(() => {
    if (!venomcoworkClient || !selectedWorkspaceId) return;
    const workspaceId = routeStateRef.current.runtimeWorkspaceId?.trim() || selectedWorkspaceId;
    let cancelled = false;
    (async () => {
      try {
        const config = await venomcoworkClient.getConfig(workspaceId);
        if (cancelled) return;
  ...
  const toggleAutoCompactContext = useCallback(async () => {
    ...
    const workspaceId = routeStateRef.current.runtimeWorkspaceId?.trim() || selectedWorkspaceId;
    if (!venomcoworkClient || !workspaceId) return;
    const next = !autoCompactContext;
    setAutoCompactContext(next);
    setAutoCompactContextBusy(true);
    try {
      await venomcoworkClient.patchConfig(workspaceId, {
  ...
  }, [autoCompactContext, autoCompactContextBusy, venomcoworkClient, reloadCoordinator, selectedWorkspaceId]);

  useEffect(() => {
    venomcoworkServerStore.start();
    ...
    return () => {
      ...
      venomcoworkServerStore.dispose();
    };
  }, [connectionsStore, extensionsStore, venomcoworkServerStore, providerAuthStore]);
  ...
  useEffect(() => {
    venomcoworkServerStore.syncFromOptions();
  ...
  const canWriteWorkspaceSkills =
    !isRemoteWorkspace || venomcoworkServerSnapshot.venomcoworkServerCanWriteSkills;
  const canWriteWorkspacePlugins =
    !isRemoteWorkspace || venomcoworkServerSnapshot.venomcoworkServerCanWritePlugins;
  ...
  const enablementContext = useMemo<EnablementContext>(() => {
    ...
    return {
      ...
      isToggleEnabled: (ref: string) => {
        ...
        return match ? isVenomCoworkExtensionEnabled(match) : false;
      },
    };
  ...
  const handleSaveRenameWorkspace = useCallback(async () => {
    ...
    try {
      if (!venomcoworkClient) {
        toast.error("VenomCowork server is unavailable. Reconnect the server before renaming workspaces.");
        return;
      }
      await venomcoworkClient.updateWorkspaceDisplayName(renameWorkspaceId, trimmed);
  ...
  }, [venomcoworkClient, refreshRouteState, renameWorkspaceId, renameWorkspaceTitle]);
  ...
  const handleExportWorkspaceConfig = useCallback(async (workspaceId: string) => {
    ...
    throw new Error("VenomCowork server is unavailable. Reconnect the server before exporting workspace config.");
  ...
  const handleForgetWorkspace = useCallback(async (workspaceId: string) => {
    ...
    if (venomcoworkClient) {
      await venomcoworkClient.deleteWorkspace(workspaceId).catch(() => undefined);
    }
    if (isDesktopRuntime()) {
    ...
    await refreshRouteState();
  }, [venomcoworkClient, refreshRouteState, selectedWorkspaceId, workspaces]);
  ...
  const handleCreateWorkspace = async (preset: WorkspacePreset, folder: string | null) => {
    ...
    try {
      ...
      if (venomcoworkClient) {
        list = await venomcoworkClient
          .createLocalWorkspace({ folderPath: folder, name: workspaceName, preset })
          .catch(() => null);
      }
      if (!list) {
        throw new Error("VenomCowork server is unavailable. Start or reconnect the server before creating a workspace.");
      }
  ...
  const handleCreateRemoteWorkspace = async (input: {
    ...
    venomcoworkToken?: string | null;
    directory?: string | null;
    displayName?: string | null;
  }) => {
    const baseUrlValue = input.venomcoworkHostUrl?.trim() ?? "";
    if (!baseUrlValue) return false;
    setCreateWorkspaceRemoteBusy(true);
    setCreateWorkspaceRemoteError(null);
    try {
      const remoteType: "venomcowork" = "venomcowork";
      const payload = {
        baseUrl: baseUrlValue,
        venomcoworkHostUrl: baseUrlValue,
        venomcoworkToken: input.venomcoworkToken?.trim() || null,
      ...
      let list: WorkspaceList | null = null;
      if (venomcoworkClient) {
        list = await venomcoworkClient.createRemoteWorkspace(payload).catch(() => null);
      }
      if (!list) {
        throw new Error("VenomCowork server is unavailable. Start or reconnect the server before connecting a remote workspace.");
      }
      const createdId = resolveWorkspaceListSelectedId(list) || list.workspaces[list.workspaces.length - 1]?.id || "";
      if (createdId) {
        await workspaceSetSelected(createdId).catch(() => undefined);
  ...
  const messagingViewProps = useMessagingViewProps({
    busy,
    venomcoworkServerStatus: venomcoworkServerSnapshot.venomcoworkServerStatus,
    venomcoworkServerUrl: venomcoworkServerSnapshot.venomcoworkServerUrl,
    ...
    switch (route.tab) {
      ...
      case "ai":
        return (
          <AiSettingsView
            ...
            showVenomCoworkModelsSubscribe={showVenomCoworkModelsSubscribe}
            onSubscribeVenomCoworkModels={subscribeToVenomCoworkModels}
            onDismissVenomCoworkModels={dismissVenomCoworkModelsPromo}
            cloudProvidersView={
              <CloudProvidersView
                embedded
          ...
          <ExtensionsView
            ...
            cloudMarketplaceView={
              <CloudMarketplacesView
                ...
                setBuiltInEnabled={setVenomCoworkExtensionEnabled}
              />
            }
          />
        );
        ...
        return (
          <AdvancedView
            ...
            toggleDeveloperMode={() => setDeveloperMode((current) => {
              const next = !current;
              try { window.localStorage.setItem("venomcowork.developerMode", next ? "1" : "0"); } catch {}
              return next;
            })}
            ...
            canMigrateRuntimeConfig={Boolean(venomcoworkClient && selectedWorkspaceId)}
            migrateRuntimeConfig={async () => {
              if (!venomcoworkClient || !selectedWorkspaceId) {
                throw new Error("Select a workspace before migrating legacy runtime config.");
              }
              const result = await venomcoworkClient.migrateRuntimeConfig(selectedWorkspaceId);
              if (result.migrated) {
                void connectionsStore.refreshMcpServers();
                void extensionsStore.refreshPlugins();
              }
            ...
            getRuntimeConfigStatus={async () => {
              ...
              return venomcoworkClient.getRuntimeConfigStatus(selectedWorkspaceId);
            }}
          />
        );
      case "appearance":
      ...
      case "recovery":
        return (
          <RecoveryView
            anyActiveRuns={false}
            workspaceConfigPath={selectedWorkspaceRoot ? `${selectedWorkspaceRoot}/.opencode/venomcowork.json` : ""}
      ...
      case "environment":
        return (
          <EnvironmentView
            client={venomcoworkServerSnapshot.venomcoworkServerClient}
            isRemoteWorkspace={isRemoteWorkspace}
