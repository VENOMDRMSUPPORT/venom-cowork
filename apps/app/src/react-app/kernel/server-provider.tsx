...
function readStoredList(): string[] {
  ...
  try {
    const raw = window.localStorage.getItem("venomcowork.server.list");
...
function readStoredActive(): string {
  if (typeof window === "undefined") return "";
  try {
    const stored = window.localStorage.getItem("venomcowork.server.active");
...
export function ServerProvider({ children, defaultUrl }: ServerProviderProps) {
  ...
  useEffect(() => {
    ...
    // Hosted web deployments served by VenomCowork must reuse the OpenCode proxy
    // rather than any persisted localhost target.
    const forceProxy =
      !isDesktopRuntime() &&
      isWebDeployment() &&
      (import.meta.env.PROD ||
        (typeof import.meta.env?.VITE_VENOMCOWORK_URL === "string" &&
          import.meta.env.VITE_VENOMCOWORK_URL.trim().length > 0));
  ...
  useEffect(() => {
    ...
    if (typeof window === "undefined") return;
    try {
      window.localStorage.setItem("venomcowork.server.list", JSON.stringify(list));
      window.localStorage.setItem("venomcowork.server.active", active);
    } catch {
      // ignore
    }
  }, [active, list]);
  ...
  useEffect(() => {
    ...
    if (isDesktopRuntime() && !active.includes("/opencode")) {
      // Desktop React routes now talk to VenomCowork server workspace-mounted
