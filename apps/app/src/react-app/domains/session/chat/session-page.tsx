...
const GLOBAL_VOICE_SIDE_PANEL_KEY = "__venomcowork_voice__";
...
export type SessionPageSurfaceProps = Omit<
  SessionSurfaceProps,
  "client" | "workspaceId" | "sessionId" | "opencodeBaseUrl" | "venomcoworkToken"
>;
...
function hiddenAccessibleTargetsStorageKey(workspaceId: string | null | undefined, sessionId: string | null | undefined) {
  ...
  return `venomcowork.session.hiddenAccessibleTargets.v1:${workspaceId}:${sessionId}`;
}
...
export function SessionPage(props: SessionPageProps) {
  ...
  const voiceExtension = useMemo(
    () => VENOMCOWORK_EXTENSION_CATALOG.find((entry) => getExtensionId(entry) === "venomcowork-voice") ?? null,
    [],
  );
  const voiceExtensionEnabled = voiceExtension ? isVenomCoworkExtensionEnabled(voiceExtension) : false;
  ...
  useEffect(() => {
    ...
    const browser = (window as Window).__VENOMCOWORK_ELECTRON__?.browser;
    if (!browser) return;
  ...
  const openTarget = useCallback((target: OpenTarget, options?: { auto?: boolean }, sourceSessionId?: string) => {
    if (target.kind === "url" || target.preview === "browser") {
      ...
      if (isElectronRuntime()) {
        setCurrentSidePanel("panel");
        void window.__VENOMCOWORK_ELECTRON__?.browser?.createTab?.(url);
      } else {
  ...
  const openBrowserRailPane = useCallback(() => {
    ...
    if (opening && isElectronRuntime()) {
      ...
      if (!hasBrowserTab) {
        void window.__VENOMCOWORK_ELECTRON__?.browser?.createTab?.();
      }
    }
  ...
  const openBrowserUrlControlAction = useMemo<VenomcoworkControlAction>(() => ({
    ...
    execute: async (args) => {
      ...
      setCurrentSidePanel("panel");
      return window.__VENOMCOWORK_ELECTRON__?.browser?.openUrl?.(url, provider);
    },
  }), [setCurrentSidePanel]);
  ...
  useEffect(() => {
    ...
    window.addEventListener("venomcowork-open-accessible-target", open);
    window.addEventListener("venomcowork-hide-accessible-target", hide);
    return () => {
      window.removeEventListener("venomcowork-open-accessible-target", open);
      window.removeEventListener("venomcowork-hide-accessible-target", hide);
    };
  }, [accessibleTargets, openTarget, removeAccessibleTarget]);
  useEffect(() => {
    const handler = () => setCurrentSidePanel(null);
    window.addEventListener("venomcowork-close-right-pane", handler);
    return () => window.removeEventListener("venomcowork-close-right-pane", handler);
  }, [setCurrentSidePanel]);
  useEffect(() => {
  ...
  const reactSessionToken =
    props.venomcoworkServerToken?.trim() ||
    props.venomcoworkServerClient?.token?.trim() ||
    "";
  const canRenderReactSurface = Boolean(
    props.selectedSessionId &&
      props.runtimeWorkspaceId &&
      props.venomcoworkServerClient &&
      reactSessionBaseUrl &&
      reactSessionToken &&
      props.surface,
  );
            ...
            <div className="flex items-center gap-1.5 text-gray-10 mac:titlebar-no-drag">
              ...
              {props.developerMode ? (
                <Button
                  ...
                  onClick={() => {
                    try {
                      window.localStorage.removeItem("venomcowork.acknowledgedProviders");
                      window.localStorage.removeItem("venomcowork.orgOnboardingSeen");
                    } catch {}
                  }}
                  ...
                  <div className="flex min-h-0 flex-1 flex-col lg:flex-row">
                    <div className={cn("min-h-0 min-w-0 flex-1", canRenderSplitSurface && "lg:border-r lg:border-border")}>
                      <SessionSurface
                        ...
                        // server target (client/workspaceId/sessionId/opencodeBaseUrl/venomcoworkToken)
                        // must come from the resolved workspace endpoint passed by
                        // SessionRoute, not from anything in `surface`.
                        {...props.surface!}
                        client={props.venomcoworkServerClient!}
                        workspaceId={props.runtimeWorkspaceId!}
                        sessionId={props.selectedSessionId!}
                        opencodeBaseUrl={reactSessionBaseUrl}
                        venomcoworkToken={reactSessionToken}
                        todos={props.todos}
                    ...
                    <div className="flex flex-1 items-center justify-center px-6 py-16">
                      <div className="w-full max-w-md space-y-6">
                        ...
                        <div className="space-y-2">
                          ...
                          >
                            <img src="/venomcowork-mark.svg" alt="" width={20} height={20} className="mt-0.5 shrink-0" />
                            <div>
