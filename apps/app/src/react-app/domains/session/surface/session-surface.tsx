...
import { useControlAction, type VenomcoworkControlAction } from "@/react-app/shell/control/control-provider";
...
const DEFAULT_COMPOSER_CONTROL_TEXT = "Help me outline the next VenomCowork task.";
...
function messageToReadableText(message: UIMessage) {
  const header = message.role === "user" ? "You" : message.role === "assistant" ? "VenomCowork" : message.role;
...
function statusLabel(snapshot: VenomcoworkSessionSnapshot | undefined, busy: boolean) {
...
function TodoPanel(props: { todos: TodoItem[] }) {
  ...
  const label = expanded ? progressLabel : `${progressLabel} Â· ${completedTodos}/${todos.length}`;
...
function parseSessionError(thrown: unknown): SessionError {
  ...
  try {
    ...
    // Not JSON â€” fall through to plain message
  }
...
export function SessionSurface(props: SessionSurfaceProps) {
  ...
  const [rendered, setRendered] = useState<{ sessionId: string; snapshot: VenomcoworkSessionSnapshot } | null>(null);
  ...
  const opencodeClient = useMemo(
    () => createClient(props.opencodeBaseUrl, undefined, { token: props.venomcoworkToken, mode: "venomcowork" }),
    [props.opencodeBaseUrl, props.venomcoworkToken],
  );
  ...
  const snapshotQuery = useQuery<VenomcoworkSessionSnapshot>({
  ...
  // accepts follow-up user turns mid-run (steering) â€” the running loop picks
  // up the new message â€” so this is safe to call while the agent is busy.
  ...
  const handlePasteText = (text: string) => {
    ...
    const label = `${id.slice(-4)} Â· ${text.split(/\r?\n/).length} lines`;
  ...
  const typeComposerText = useCallback(async (text: string) => {
    window.dispatchEvent(new Event("venomcowork:focusPrompt"));
  ...
  useEffect(() => {
    ...
    window.addEventListener("venomcowork:voice-transcript", handleVoiceTranscript);
    return () => window.removeEventListener("venomcowork:voice-transcript", handleVoiceTranscript);
  }, [attachments, buildDraft, props.onDraftChange, props.sessionId, props.workspaceId, typeComposerText]);

  const composerSetTextControlAction = useMemo<VenomcoworkControlAction>(() => ({
    ...
    execute: async (args, helpers) => {
      ...
      helpers.setNarration(`Typing ${text.length.toLocaleString()} characters into the composerâ€¦`);
  ...
  const composerSendControlAction = useMemo<VenomcoworkControlAction>(() => ({
    id: "composer.send",
  ...
  const composerStopControlAction = useMemo<VenomcoworkControlAction>(() => ({
    id: "composer.stop",
  ...
  const listImportedPlugins = async (): Promise<CloudImportedPlugin[]> => {
    ...
    const plugins = Object.values(readWorkspaceCloudImports(response.venomcowork).plugins)
  ...
  const sessionScrollTopControlAction = useMemo<VenomcoworkControlAction>(() => ({
  ...
  const sessionScrollBottomControlAction = useMemo<VenomcoworkControlAction>(() => ({
  ...
  const sessionLatestMessageControlAction = useMemo<VenomcoworkControlAction>(() => ({
  ...
  const sessionReadTranscriptControlAction = useMemo<VenomcoworkControlAction>(() => ({
            ...
            {showDelayedLoading && pendingSessionLoad ? (
              <div className="px-6 py-16">
                <div className="mx-auto max-w-sm rounded-3xl border border-dls-border bg-dls-hover/60 px-8 py-10 text-center">
                  <div className="text-sm text-dls-secondary">Opening sessionâ€¦</div>
                </div>
              </div>
