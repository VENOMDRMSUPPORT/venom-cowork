...
const baseMarkedOptions = {
  ...
  renderer: {
    html({ text }) {
      return text.includes('data-venomcowork-shiki="true"') ? text : "";
    },
    paragraph({ tokens }) {
...
const highlightedMarkdownParser = new Marked<string, string>({
  ...
  markedShiki({
    ...
    container: `<div data-venomcowork-shiki="true" class="my-4 overflow-hidden rounded-[18px] border border-dls-border/70 bg-gray-1/80 p-4 text-xs leading-6">%s</div>`,
  }),
);
...
/**
 * Memoize so a message block that has already been rendered â€” the usual
 * case for every assistant bubble above the currently-streaming one â€”
