...
function pastedTextChipLabel(lines: number) {
  return `Pasted Â· ${lines} line${lines === 1 ? "" : "s"}`;
}
...
function SyncPlugin(props: { value: string; mentions: Record<string, "agent" | "file">; pastedText?: Array<{ label: string; lines: number }>; disabled: boolean }) {
  ...
  useEffect(() => {
    ...
    // content). The read() above already gave us `currentText` â€” reuse it.
    ...
    editor.update(() => {
      ...
      // token (chip) â€” Lexical can't position a cursor inside a token,
  ...
  useEffect(() => {
    return editor.registerCommand(
      ...
      (event: KeyboardEvent | null) => {
        ...
        // Shift+Enter inserts a newline â€” let the editor handle it.
...
export function LexicalPromptEditor(props: EditorProps) {
  ...
  const initialConfig = useMemo(
    () => ({
      namespace: "venomcowork-react-session-composer",
      onError(error: Error) {
        throw error;
      },
  ...
  return (
    <LexicalComposer initialConfig={initialConfig}>
      {/*
        ...
        - max-h caps the composer â€” long pastes / multi-paragraph drafts scroll
