...
function syncKey(input: SyncOptions) {
  return `${input.workspaceId}:${input.baseUrl}:${input.venomcoworkToken}`;
}
...
function appendDelta(messages: UIMessage[], messageId: string, partId: string, delta: string, reasoning: boolean) {
  ...
  if (partIndex === -1) {
    // No existing matching part â€” append a fresh one so the delta is not lost.
...
function applyEvent(entry: SyncEntry, workspaceId: string, event: OpencodeEvent) {
  ...
  if (event.type === "message.part.updated") {
    ...
    // declaration. We deliberately ignore `pending.reasoning` â€” it
    ...
    // buffer â€” they've already been incorporated into `mapped.text`.
    ...
    queryClient.setQueryData<UIMessage[]>(transcriptKey(workspaceId, part.sessionID), (current = []) => {
      ...
      // the inference â€” upsertMessage preserves existing role when the
    ...
    queryClient.setQueryData<UIMessage[]>(
      ...
      (current = []) => {
        ...
        for (const item of items) {
          ...
          // part â€” and reasoning content leaks into the response markdown
...
function startSync(input: SyncOptions) {
  const client = createClient(input.baseUrl, undefined, { token: input.venomcoworkToken, mode: "venomcowork" });
