...
const DEFAULT_TEXT_COMMAND = "Summarize the current VenomCowork session and put the next step in the composer.";
...
const TOOL_LABELS: Record<string, string> = {
  venomcowork_snapshot: "Checking VenomCowork",
  venomcowork_list_actions: "Listing controls",
  venomcowork_execute_action: "Running UI action",
};
...
function humanToolLabel(toolName?: string) {
  if (!toolName) return "VenomCowork action";
...
async function requestMacMicrophoneAccess() {
  const ask = window.__VENOMCOWORK_ELECTRON__?.system?.askMicrophoneAccess;
  if (!ask) return true;
...
async function executeVenomCoworkTool(name: string, args: Record<string, unknown>) {
  const control = window.__venomcoworkControl;
  if (!control) return { ok: false, error: "VenomCowork control surface is not available." };

  if (name === "venomcowork_snapshot") return { ok: true, snapshot: control.snapshot() };
  if (name === "venomcowork_list_actions") return { ok: true, actions: control.listActions() };
  if (name === "venomcowork_execute_action") {
  ...
  return { ok: false, error: `Unknown VenomCowork voice tool: ${name}` };
}
...
export function VoicePanel(props: VoicePanelProps) {
    ...
    setVoiceRuntimeSnapshot((current) => ({
      ...
      statusText: text ?? (
        nextStatus === "connecting" ? "Connecting to OpenAI Realtime..." :
          nextStatus === "listening" ? "Listening. Ask VenomCowork to act." :
            nextStatus === "speaking" ? "VenomCowork is speaking..." :
  ...
  const handleRealtimeMessage = useCallback(async (raw: string) => {
    ...
    if (type === "response.function_call_arguments.done") {
      ...
      const output = await executeVenomCoworkTool(toolName, args);
  ...
  const connectRealtime = useCallback(async (audioInput = true) => {
    ...
    if (!client) throw new Error("VenomCowork host connection is not ready.");
    ...
    if (audioInput) {
      ...
      if (!macPermissionGranted) throw new Error("macOS denied microphone access. Enable VenomCowork in System Settings > Privacy & Security > Microphone, then restart VenomCowork.");
    ...
    addEntry("system", `Realtime connected with ${realtimeSession.model} and ${realtimeSession.tools.length} VenomCowork tools.`);
  ...
  const injectTranscript = useCallback(async (args: unknown) => {
    ...
    addEntry("user", text);
    window.dispatchEvent(new CustomEvent("venomcowork:voice-transcript", { detail: { text } }));
    ...
    <div ref={panelRef} className="flex h-full min-h-0 flex-col bg-background">
      <div className="flex shrink-0 items-center justify-between border-b border-border px-4 py-3">
        <div className="min-w-0">
          ...
          <div className="truncate text-xs text-muted-foreground">Realtime voice over VenomCowork UI MCP controls</div>
        </div>
          ...
          {!props.client ? (
            <Card variant="outline" size="sm">
              ...
              <CardContent className="text-sm text-muted-foreground">
                Voice Mode needs the local VenomCowork server so it can mint short-lived Realtime client secrets without exposing your API key to the renderer.
              </CardContent>
            </Card>
          ) : null}
