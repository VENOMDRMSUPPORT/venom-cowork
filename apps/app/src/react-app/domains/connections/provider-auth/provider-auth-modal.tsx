...
const VENOMCOWORK_MODELS_PROVIDER_ID = "venomcowork";
...
export type ProviderAuthModalProps = {
  ...
  showVenomCoworkModelsSubscribe?: boolean;
  onSubscribeVenomCoworkModels?: () => void | Promise<void>;
  onClose: () => void;
};
...
export default function ProviderAuthModal(props: ProviderAuthModalProps) {
  ...
  const [view, setView] = useState<
    "list" | "method" | "api" | "cloud" | "oauth-code" | "oauth-auto" | "venomcowork-subscribe"
  >("list");
  ...
  const entries = useMemo<ProviderAuthEntry[]>(() => {
    ...
    if (props.showVenomCoworkModelsSubscribe) {
      const connectedToVenomCowork = connected.has(VENOMCOWORK_MODELS_PROVIDER_ID);
      return [
        {
          id: VENOMCOWORK_MODELS_PROVIDER_ID,
          name: "VenomCowork",
          methods: [{ type: "cloud", label: "Subscribe" }],
          connected: connectedToVenomCowork,
          env: [],
        },
        ...nextEntries.filter((entry) => entry.id.trim().toLowerCase() !== VENOMCOWORK_MODELS_PROVIDER_ID),
      ];
    }

    return nextEntries;
  }, [isRemoteWorker, props.authMethods, props.connectedProviderIds, props.providers, props.showVenomCoworkModelsSubscribe]);
  ...
  const handleEntrySelect = (entry: ProviderAuthEntry) => {
    ...
    if (props.showVenomCoworkModelsSubscribe && entry.id.trim().toLowerCase() === VENOMCOWORK_MODELS_PROVIDER_ID) {
      setView("venomcowork-subscribe");
      return;
    }
  ...
  const handleBack = () => {
    if (resolvedView === "venomcowork-subscribe") {
      resetState();
      return;
    }
  ...
  const methodDescription = (entry: ProviderAuthEntry, method: ProviderAuthMethod) => {
    ...
    if (method.type === "oauth") {
      return "Continue in the browser and let VenomCowork finish the connection automatically.";
    }
    if (method.type === "cloud") {
    ...
    return "Paste a secret key that VenomCowork stores locally on this device.";
  };
            ...
            <div className="-mr-1 min-h-0 flex-1 space-y-2 overflow-y-auto pr-1">
              ...
              {resolvedView === "api" && selectedEntry ? (
                <div className="rounded-xl border border-gray-6/40 bg-gray-2/50 shadow-sm p-5 space-y-4">
                  ...
                  <div className="flex items-center justify-between gap-3">
                    ...
                    >
                      {props.submitting ? "Savingâ€¦" : "Save key"}
                    </Button>
                  </div>
                </div>
              ) : null}
              ...
              {resolvedView === "cloud" && selectedEntry && selectedCloudMethod ? (
                <div className="rounded-xl border border-gray-6/40 bg-gray-2/50 shadow-sm p-5 space-y-4">
                  ...
                  <div className="flex items-center justify-between gap-3">
                    <div className="text-[11px] text-gray-9">
                      VenomCowork will install the provider config and use the credential stored for your org.
                    </div>
              ...
              {resolvedView === "venomcowork-subscribe" && selectedEntry ? (
                <div className="rounded-xl border border-blue-6/50 bg-blue-2/25 shadow-sm p-5 space-y-4">
                  <div className="flex items-center justify-between gap-4">
                    <div>
                      <div className="text-sm font-medium text-gray-12">VenomCowork Models</div>
                      <div className="text-xs text-gray-10 mt-1">
                  ...
                  <div className="flex items-center justify-end">
                    <Button onClick={() => void props.onSubscribeVenomCoworkModels?.()} disabled={actionDisabled}>
                      Subscribe
                    </Button>
                  </div>
                </div>
                ...
                <div className="rounded-xl border border-gray-6/40 bg-gray-2/50 shadow-sm p-5 space-y-4">
                  ...
                  {isOpenAiHeadlessSession && !oauthBrowserOpened ? (
                    ...
                    <div className="flex items-center gap-2 text-xs text-gray-9">
                      ...
                      <span>Checking connection status automaticallyâ€¦</span>
                    </div>
                  )}
                  <div className="flex items-center justify-between gap-3">
                    <Button
