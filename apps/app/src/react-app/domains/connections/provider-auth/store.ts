...
export function createProviderAuthStore(options: CreateProviderAuthStoreOptions) {
  ...
  ) => provider.source === "venomcowork" ? "venomcowork" : provider.id.trim();
  ...
  const writeWorkspaceOpenworkConfigRecord = async (
    ...
    if (isLocalWorkspace && isDesktopRuntime() && root) {
      ...
      if (!typed.ok) {
        throw new Error(
          typed.stderr || typed.stdout || "Failed to write .opencode/venomcowork.json",
        );
      }
      return true;
    }
  ...
  const cloudProviderComment = (provider: Pick<DenOrgLlmProvider, "id" | "name">) =>
    `// VenomCowork Cloud import: ${provider.name
      .replace(/\s+/g, " ")
  ...
  // The first load (app startup) populates the initial state â€” we don't
  ...
  const applyProviderListState = (value: ProviderListResponse, opts?: { suppressNewProviderEvent?: boolean }) => {
    ...
    // the NewProvidersToast shows â€” regardless of which route is active.
  ...
  async function connectCloudProviderInternal(
    ...
    const orgId = settings.activeOrgId?.trim() ?? "";
    if (!token || !orgId) {
      throw new Error("Sign in to VenomCowork Cloud and choose an organization first.");
    }
