...
export function createOpenworkServerStore(options: CreateOpenworkServerStoreOptions) {
  ...
  let state: MutableState = {
    ...
    venomcoworkServerStatus: "disconnected",
    venomcoworkServerCapabilities: null,
    venomcoworkServerCheckedAt: null,
    venomcoworkServerHostInfo: null,
    venomcoworkServerHostInfoReady: !isDesktopRuntime(),
    venomcoworkServerDiagnostics: null,
    venomcoworkReconnectBusy: false,
    venomcoworkAuditEntries: [],
    venomcoworkAuditStatus: "idle",
    venomcoworkAuditError: null,
  ...
  const refreshSnapshot = () => {
    const venomcoworkServerBaseUrl = getBaseUrl().trim();
    const venomcoworkServerAuth = getAuth();
    ...
    if (pref === "server") venomcoworkServerUrl = settingsUrl;
    state.venomcoworkServerUrl = venomcoworkServerUrl;

    snapshot = {
      venomcoworkServerSettings: state.venomcoworkServerSettings,
      shareRemoteAccessBusy: state.shareRemoteAccessBusy,
      shareRemoteAccessError: state.shareRemoteAccessError,
      venomcoworkServerUrl,
      venomcoworkServerBaseUrl,
      venomcoworkServerAuth,
      venomcoworkServerClient,
      venomcoworkServerStatus: state.venomcoworkServerStatus,
      ...
      venomcoworkAuditEntries: state.venomcoworkAuditEntries,
      venomcoworkAuditStatus: state.venomcoworkAuditStatus,
      venomcoworkAuditError: state.venomcoworkAuditError,
  ...
  const runHealthCheck = async () => {
    ...
    if (!url) {
      ...
      mutateState((current) => ({
        ...
        venomcoworkServerStatus: "disconnected",
        venomcoworkServerCapabilities: null,
        venomcoworkServerCheckedAt: Date.now(),
      }));
      return;
    }
    ...
    try {
      ...
      if (shouldRetryStartupCheck(result.status)) {
        ...
        try {
          ...
          mutateState((current) => ({
            ...current,
            venomcoworkServerHostInfo: info,
            venomcoworkServerHostInfoReady: true,
          }));
      ...
      if (disposed) return;
      const previousStatus = state.venomcoworkServerStatus;
      const previousCapabilities = state.venomcoworkServerCapabilities;
      ...
      mutateState((current) => ({
        ...current,
        venomcoworkServerStatus: preservePrevious ? previousStatus : result.status,
        venomcoworkServerCapabilities: preservePrevious ? previousCapabilities : result.capabilities,
        venomcoworkServerCheckedAt: Date.now(),
      }));
    } catch {
      healthDelayMs = Math.min(healthDelayMs * 2, 60_000);
  ...
  const syncFromOptions = () => {
    ...
    const port = state.venomcoworkServerHostInfo?.port;
    if (!port) return;
  ...
  const start = () => {
    ...
    // each effect in dev: mount â†’ dispose â†’ re-mount). If we early-return when
    ...
    const refreshDiagnostics = () => {
      ...
      if (!options.developerMode()) {
        setStateField("venomcoworkServerDiagnostics", null);
        return;
      }

      const client = getClient();
      if (!client || state.venomcoworkServerStatus === "disconnected") {
        setStateField("venomcoworkServerDiagnostics", null);
        return;
      }
      ...
      void (async () => {
        try {
          ...
          if (!disposed) setStateField("venomcoworkServerDiagnostics", status);
        } catch {
          if (!disposed) setStateField("venomcoworkServerDiagnostics", null);
        }
      })();
    };
    refreshDiagnostics();
    ...
    const refreshAudit = () => {
      ...
      mutateState((current) => ({
        ...current,
        venomcoworkAuditStatus: "loading",
        venomcoworkAuditError: null,
      }));
      ...
      void (async () => {
        try {
          ...
          mutateState((current) => ({
            ...current,
            venomcoworkAuditEntries: Array.isArray(result.items) ? result.items : [],
            venomcoworkAuditStatus: "idle",
          }));
        } catch (error) {
          if (disposed) return;
          mutateState((current) => ({
            ...current,
            venomcoworkAuditEntries: [],
            venomcoworkAuditStatus: "error",
            venomcoworkAuditError:
  ...
  const testOpenworkServerConnection = async (next: OpenworkServerSettings) => {
    ...
    mutateState((current) => ({
      ...
      venomcoworkServerCapabilities: result.capabilities,
      venomcoworkServerCheckedAt: Date.now(),
    }));
    ...
    if (ok && !isDesktopRuntime()) {
      ...
      const shouldAttach =
        ...
        active.remoteType !== "venomcowork";
      if (shouldAttach) {
        await options
          .createRemoteWorkspaceFlow({
            venomcoworkHostUrl: derived,
            venomcoworkToken: next.token ?? null,
          })
          .catch(() => undefined);
      }
    }
    ...
    try {
      ...
      if (isDesktopRuntime()) {
        try {
          ...
          hostInfo = null;
          setStateField("venomcoworkServerHostInfo", null);
        }
      }
      ...
      setStateField("venomcoworkReconnectBusy", false);
    }
  };
