import { getMcpServerName, isBuiltInVenomCoworkExtension, type McpDirectoryInfo } from "../../../app/constants";
...
export function buildExtensionItems(input: ExtensionItemBuildInput) {
  const builtInItems = input.quickConnect.filter(isBuiltInVenomCoworkExtension).map((entry): ExtensionItem => {
  ...
  const standaloneMcpEntries = input.quickConnect.filter((entry) => {
    if (isBuiltInVenomCoworkExtension(entry)) return false;
