...
export function useMessagingViewProps(
  ...
  const serverReady =
    ...
    Boolean(options.venomcoworkServerClient);
  ...
  const loadAgentFile = useCallback(async () => {
    ...
    const client = options.venomcoworkServerClient;
    if (!client) return;
  ...
  }, [agentLoading, options.venomcoworkServerClient, resetAgentState, serverReady, workspaceId]);
  ...
  }, [agentSaving, options.venomcoworkServerClient, serverReady, workspaceId]);
  ...
  }, [agentBaseUpdatedAt, agentDraft, agentSaving, options.venomcoworkServerClient, serverReady, workspaceId]);
  ...
  const sendTestMessage = useCallback(async () => {
    ...
    options.venomcoworkServerClient,
    sendAutoBind,
    sendBusy,
    sendChannel,
    sendDirectory,
  ...
  const repairAndReconnect = useCallback(async () => {
    if (options.venomcoworkReconnectBusy) return;
  ...
  }, [messagingSaving, options.venomcoworkServerClient, refreshAll, serverReady, workspaceId]);
  ...
  }, [options.venomcoworkServerClient, refreshAll, serverReady, telegramSaving, workspaceId]);
  ...
  }, [options.venomcoworkServerClient, refreshAll, serverReady, slackSaving, workspaceId]);
  ...
  return {
    ...
    refreshing,
    venomcoworkReconnectBusy: options.venomcoworkReconnectBusy,
    reconnectStatus,
    reconnectError,
    health,
    healthError,
