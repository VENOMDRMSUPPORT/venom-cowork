...
export function createExtensionsStore(options: {
  ...
  const getVenomcoworkServerSnapshot = () => {
    ...
    return {
      ...snapshot,
      venomcoworkServerClient: connection.venomcoworkServerClient,
      venomcoworkServerStatus: connection.venomcoworkServerStatus,
      venomcoworkServerCapabilities: connection.venomcoworkServerCapabilities,
    };
  };
  ...
  const writeWorkspaceVenomcoworkConfigRecord = async (config: Record<string, unknown>) => {
    ...
    if (isLocalWorkspace && isDesktopRuntime() && root) {
      ...
      if (!result.ok) {
        throw new Error(result.stderr || result.stdout || "Failed to write .opencode/venomcowork.json");
      }
      return true;
    }
  ...
  const refreshImportedCloudPlugins = async () => {
    try {
      ...
      if (target.venomcoworkClient && target.venomcoworkWorkspaceId) {
        const result = await target.venomcoworkClient.listCloudPlugins(target.venomcoworkWorkspaceId);
  ...
  const upsertWorkspaceSkill = async (
    ...
    if (isRemoteWorkspace) {
      throw new Error("VenomCowork server unavailable. Connect to import skills.");
    }
  ...
  const deleteWorkspaceSkill = async (name: string) => {
    ...
    if (isRemoteWorkspace) {
      throw new Error("VenomCowork server unavailable. Connect to remove skills.");
    }
  ...
  const upsertPluginMcpConfig = async (name: string, config: Record<string, unknown>) => {
    ...
    if (
      ...
      venomcoworkClient &&
      venomcoworkWorkspaceId &&
      venomcoworkSnapshot.venomcoworkServerCapabilities?.mcp?.write
    ) {
      await venomcoworkClient.addMcp(venomcoworkWorkspaceId, { name, config });
      return;
    }
    throw new Error("VenomCowork server unavailable. Connect to import MCP servers into this workspace.");
  };
  ...
  const deletePluginMcpConfig = async (name: string) => {
    ...
    throw new Error("VenomCowork server unavailable. Connect to remove imported MCP servers from this workspace.");
  };
  ...
  const writePluginWorkspaceFile = async (path: string, content: string) => {
    ...
    if (
      ...
      venomcoworkWorkspaceId &&
      venomcoworkSnapshot.venomcoworkServerCapabilities?.config?.write !== false &&
      typeof venomcoworkClient.writeWorkspaceFile === "function"
    ) {
      await venomcoworkClient.writeWorkspaceFile(venomcoworkWorkspaceId, { path, content, force: true });
      return;
    }
    throw new Error("VenomCowork server unavailable. Connect to import plugin files into this workspace.");
  };
  ...
  async function importCloudOrgPlugin(
    ...
    try {
      ...
      const token = settings.authToken?.trim() ?? "";
      const orgId = settings.activeOrgId?.trim() ?? "";
      if (!token || !orgId) throw new Error("Sign in to VenomCowork Cloud and choose an organization first.");
      const client = createDenClient({ baseUrl: settings.baseUrl, apiBaseUrl: settings.apiBaseUrl, token });
      ...
      if (target.venomcoworkClient && target.venomcoworkWorkspaceId) {
        ...
        const result = await target.venomcoworkClient.installCloudPlugin(target.venomcoworkWorkspaceId, {
          marketplaceId,
          marketplace,
          resolved,
        });
  ...
  async function removeCloudOrgPlugin(pluginId: string): Promise<{ ok: boolean; message: string }> {
    ...
    try {
      const target = await resolveWorkspaceServerTarget();
      if (target.venomcoworkClient && target.venomcoworkWorkspaceId) {
        const result = await target.venomcoworkClient.removeCloudPlugin(target.venomcoworkWorkspaceId, pluginId);
        await refreshSkills({ force: true });
        await refreshCloudOrgMarketplaces({ force: true });
        void refreshDesktopCloudSync({
          venomcoworkClient: target.venomcoworkClient,
  ...
  async function refreshSkills(optionsOverride?: { force?: boolean }) {
    ...
    if (canUseVenomcoworkServer && venomcoworkClient && venomcoworkWorkspaceId) {
      ...
      try {
        setStateField("skillsStatus", null);
        const response = await venomcoworkClient.listSkills(venomcoworkWorkspaceId, { includeGlobal: isLocalWorkspace });
    ...
    if (hasVenomcoworkTarget) {
      mutateState((current) => ({
        ...current,
        skills: [],
        skillsStatus: "VenomCowork server cannot read skills for this workspace.",
      }));
      return;
    }
    ...
    if (!client) {
      mutateState((current) => ({
        ...current,
        skills: [],
        skillsStatus: "VenomCowork server unavailable. Connect to load skills.",
      }));
      return;
    }
  ...
  async function refreshPlugins(scopeOverride?: PluginScope) {
    ...
    if (scope === "project" && canUseVenomcoworkServer && venomcoworkClient && venomcoworkWorkspaceId) {
      mutateState((current) => ({
        ...current,
        pluginConfig: null,
        pluginConfigPath: `opencode.json (${isRemoteWorkspace ? "remote" : "venomcowork"} server)`,
      }));

      try {
        mutateState((current) => ({ ...current, pluginStatus: null, sidebarPluginStatus: null }));
        if (refreshPluginsAborted) return;
        const result = await venomcoworkClient.listPlugins(venomcoworkWorkspaceId, { includeGlobal: false });
        if (refreshPluginsAborted) return;
    ...
    if (scope === "project" && hasVenomcoworkTarget) {
      mutateState((current) => ({
        ...
        pluginList: [],
        sidebarPluginStatus: "VenomCowork server cannot read plugins for this workspace.",
        sidebarPluginList: [],
      }));
      refreshPluginsInFlight = false;
      return;
    ...
    if (!isLocalWorkspace && !canUseVenomcoworkServer) {
      mutateState((current) => ({
        ...
        pluginList: [],
        sidebarPluginStatus: "Connect an VenomCowork server to load plugins.",
        sidebarPluginList: [],
      }));
      refreshPluginsInFlight = false;
      return;
  ...
  async function addPlugin(pluginNameOverride?: string) {
    ...
    if (!isLocalWorkspace) {
      setStateField("pluginStatus", "VenomCowork server unavailable. Connect to manage plugins.");
      return;
    }
  ...
  async function installSkillCreator(): Promise<{ ok: boolean; message: string }> {
    ...
    if (canUseVenomcoworkServer && venomcoworkClient && venomcoworkWorkspaceId) {
      ...
      try {
        await venomcoworkClient.upsertSkill(venomcoworkWorkspaceId, { name: "skill-creator", content: skillCreatorTemplate });
    ...
    if (isRemoteWorkspace) {
      const message = "VenomCowork server unavailable. Connect to install skills.";
      setStateField("skillsStatus", message);
      return { ok: false, message };
    }
    if (!isDesktopRuntime()) {
  ...
  async function readSkill(name: string): Promise<{ name: string; path: string; content: string } | null> {
    ...
    if (isRemoteWorkspace) {
      setStateField("skillsStatus", "VenomCowork server unavailable. Connect to view skills.");
      return null;
    }
    if (!isDesktopRuntime()) {
      setStateField("skillsStatus", t("skills.desktop_required"));
  ...
  async function saveSkill(input: { name: string; content: string; description?: string }) {
    ...
    if (isRemoteWorkspace) {
      setStateField("skillsStatus", "VenomCowork server unavailable. Connect to edit skills.");
      return;
    }
    if (!isDesktopRuntime()) {
      setStateField("skillsStatus", t("skills.desktop_required"));
  ...
  const start = () => {
    ...
    if (typeof window !== "undefined") {
      ...
      window.addEventListener("venomcowork-den-session-updated", onDenSessionUpdated);
