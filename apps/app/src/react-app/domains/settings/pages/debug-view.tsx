...
type RuntimeSummary = {
  ...
  venomcoworkServerVersionLabel: string;
};
...
export type DebugViewProps = {
  ...
  venomcoworkServerRestarting: boolean;
  opencodeServiceStatus: ServiceStatus;
  venomcoworkServiceStatus: ServiceStatus;
    ...
    <div className="rounded-xl border border-blue-6/30 bg-blue-3/20 p-3">
      <div className="mb-2 flex items-center justify-between gap-3">
        <div>
          ...
          <div className="text-[11px] text-dls-secondary">Command, working directory, and VenomCowork-injected environment.</div>
        </div>
    ...
    <section className="space-y-6 max-w-3xl w-full">
      ...
      <div className={cardClass}>
        ...
        <div className="grid gap-2 text-[12px] text-dls-secondary md:grid-cols-2">
          ...
          <div>
            {t("settings.debug_venomcowork_server_version", {
              version: props.runtimeSummary.venomcoworkServerVersionLabel,
            })}
          </div>
        </div>
      ...
      <div className={cardClass}>
        ...
        <div className="grid gap-3 grid-cols-1 lg:grid-cols-2">
          <ServiceCard
            title={t("settings.venomcowork_server_label")}
            description={t("settings.venomcowork_config_sidecar_desc")}
            pill={props.venomcoworkCard}
            lines={props.venomcoworkCard.lines}
            stdout={props.venomcoworkCard.stdout ?? null}
            stderr={props.venomcoworkCard.stderr ?? null}
            execution={props.venomcoworkCard.execution ?? null}
      ...
      <div className={cardClass}>
        <div className={sectionHeaderClass}>
          <div className={sectionTitleClass}>{t("settings.venomcowork_diagnostics_title")}</div>
          <div className={sectionDescClass}>
            <span className="font-mono text-[11px] text-dls-secondary">
              {props.venomcoworkServerDiagnostics?.version ?? "â€”"}
            </span>
          </div>
        </div>

        {props.venomcoworkServerDiagnostics ? (
          <div className="grid gap-2 text-[12px] text-dls-secondary md:grid-cols-2">
            <div>{t("settings.diag_started", { time: formatUptime(props.venomcoworkServerDiagnostics.uptimeMs) })}</div>
            <div>
              {t("settings.diag_read_only", {
                value: props.venomcoworkServerDiagnostics.readOnly ? "true" : "false",
              })}
            </div>
            <div>
              {t("settings.diag_approval", {
                mode: props.venomcoworkServerDiagnostics.approval.mode,
                ms: String(props.venomcoworkServerDiagnostics.approval.timeoutMs),
              })}
            </div>
            <div>{t("settings.diag_workspaces", { count: String(props.venomcoworkServerDiagnostics.workspaceCount) })}</div>
            <div>
              {t("settings.diag_selected_workspace", {
                id: props.venomcoworkServerDiagnostics.selectedWorkspaceId ?? "â€”",
              })}
            </div>
            <div>
              {t("settings.diag_runtime_workspace", {
                id: props.venomcoworkServerDiagnostics.activeWorkspaceId ?? "â€”",
              })}
            </div>
            <div>
              {t("settings.diag_config_path", {
                path: props.venomcoworkServerDiagnostics.server.configPath ?? t("settings.diag_default"),
              })}
            </div>
            <div>
              {t("settings.diag_token_source", {
                source: props.venomcoworkServerDiagnostics.tokenSource.client,
              })}
            </div>
            <div>
              {t("settings.diag_host_token_source", {
                source: props.venomcoworkServerDiagnostics.tokenSource.host,
              })}
            </div>
          </div>
        ) : (
        ...
        <div className={subCardClass}>
          ...
          {props.venomcoworkServerCapabilities ? (
            <div className="grid gap-2 text-[12px] text-dls-secondary md:grid-cols-2">
              <div>{t("settings.cap_skills", { value: formatCapability(props.venomcoworkServerCapabilities.skills) })}</div>
              <div>{t("settings.cap_plugins", { value: formatCapability(props.venomcoworkServerCapabilities.plugins) })}</div>
              <div>{t("settings.cap_mcp", { value: formatCapability(props.venomcoworkServerCapabilities.mcp) })}</div>
              <div>{t("settings.cap_commands", { value: formatCapability(props.venomcoworkServerCapabilities.commands) })}</div>
              <div>{t("settings.cap_config", { value: formatCapability(props.venomcoworkServerCapabilities.config) })}</div>
              <div>
                {t("settings.cap_browser_tools", {
                  value: (() => {
                    const browser = props.venomcoworkServerCapabilities.toolProviders?.browser;
                    if (!browser?.enabled) return t("settings.disabled");
                    return `${browser.mode} Â· ${browser.placement}`;
                  })(),
                })}
              </div>
              <div>
                {t("settings.cap_file_tools", {
                  value: (() => {
                    const files = props.venomcoworkServerCapabilities.toolProviders?.files;
              ...
              <div>
                {t("settings.cap_sandbox", {
                  value: props.venomcoworkServerCapabilities.sandbox
                    ? `${props.venomcoworkServerCapabilities.sandbox.backend} (${props.venomcoworkServerCapabilities.sandbox.enabled ? t("settings.on") : t("settings.off")})`
      ...
      <div className={cardClass}>
        ...
        <div className={subCardClass}>
          <div className="flex items-center justify-between gap-3">
            ...
            <div className={`rounded-full border px-2 py-1 text-[11px] font-medium ${props.venomcoworkAuditStatus.className}`}>
              {props.venomcoworkAuditStatus.label}
            </div>
          </div>
          {props.venomcoworkAuditError ? <StatusBanner tone="error" message={props.venomcoworkAuditError} /> : null}
          {props.venomcoworkAuditEntries.length > 0 ? (
            <div className="divide-y divide-dls-border/60">
              {props.venomcoworkAuditEntries.map((entry) => (
                <div key={entry.id} className="flex items-start justify-between gap-4 py-2">
                  <div className="min-w-0">
                    ...
                    <div className="truncate text-[11px] text-dls-secondary">
                      {entry.action} Â· {entry.target} Â· {formatActor(entry)}
                    </div>
                  </div>
                  <div className="whitespace-nowrap text-[11px] text-dls-secondary">
                    {entry.timestamp ? formatRelativeTime(entry.timestamp) : "â€”"}
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className={subCardClass}>
          ...
          {props.sandboxProbeResult ? (
            <div className="space-y-1 text-[12px] text-dls-secondary">
              <div>{t("settings.sandbox_run_id", { id: props.sandboxProbeResult.runId ?? "â€”" })}</div>
              <div>
      ...
      {props.electronMigrationAvailable ? (
        <div className={cardClass}>
          ...
          <div className="rounded-xl border border-green-7/25 bg-green-3/10 px-3 py-2 text-[12px] leading-relaxed text-green-11">
            ...
            <code className="font-mono">VenomCowork.app.migrate-bak</code>.
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            >
              {props.electronMigrationBusy ? "Resolvingâ€¦" : "Resolve latest Electron alpha"}
            </Button>
          ...
          <div className="flex flex-wrap items-center gap-2">
            <Button
              ...
              disabled={props.electronMigrationBusy}
            >
              {props.electronMigrationBusy ? "Preparingâ€¦" : "Prepare migration data"}
            </Button>
            <Button
              variant="outline"
              size="sm"
              ...
              title="Requires a trusted artifact URL. macOS keeps VenomCowork.app.migrate-bak for rollback."
            >
              Start install handoffâ€¦
            </Button>
            <Button
              variant="outline"
              size="sm"
        ...
        <div className={cardClass}>
          <div className="flex items-start justify-between gap-3">
            <div>
              ...
              <div className={sectionDescClass}>
                Debug-only controls for migrated Electron users. Stable updates remain the default in Settings â†’ Updates.
              </div>
            </div>
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Button
              ...
              disabled={props.electronAlphaUpdaterBusy}
            >
              {props.electronAlphaUpdaterBusy ? "Checkingâ€¦" : "Check selected feed"}
            </Button>
          </div>
      ...
      {isDesktop ? (
        <div className="space-y-3 rounded-2xl border border-red-7/30 bg-red-3/10 p-5">
          ...
          <div className="flex items-start justify-between gap-3">
            <div>
              <div className="text-sm font-semibold tracking-[-0.1px] text-dls-text">
                {t("settings.reset_venomcowork_title")}
              </div>
              <div className="text-[12px] text-dls-secondary">
                {props.opencodeDevModeEnabled
                  ? t("settings.reset_venomcowork_desc_dev")
                  : t("settings.reset_venomcowork_desc_prod")}
              </div>
            </div>
            <div
