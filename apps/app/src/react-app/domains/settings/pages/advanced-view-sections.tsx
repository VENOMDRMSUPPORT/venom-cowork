...
interface AdvancedRuntimeSectionProps {
  ...
  venomcoworkStatusLabel: string;
  venomcoworkTone: SettingsTone;
  venomcoworkDetailLines: string[];
}
    ...
    <LayoutSection>
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        ...
        <RuntimeStatusCard
          ...
          description={t("settings.venomcowork_server_desc")}
          statusLabel={props.venomcoworkStatusLabel}
          tone={props.venomcoworkTone}
          detailLines={props.venomcoworkDetailLines}
        />
      </div>
    </LayoutSection>
  );
  ...
  return (
    <LayoutSection>
      <LayoutSectionHeader>
        ...
        <LayoutSectionDescription>
          Inspect what VenomCowork controls at runtime versus what belongs to your workspace config. This works through the VenomCowork server and does not require the OpenCode engine to be healthy.
        </LayoutSectionDescription>
      </LayoutSectionHeader>

      <LayoutSectionItem>
        <LayoutSectionItemHeader>
          <LayoutSectionItemTitle>Move VenomCowork-managed config</LayoutSectionItemTitle>
          <LayoutSectionItemDescription>
            Moves older VenomCowork-owned runtime keys from `.opencode/venomcowork.json` and safe VenomCowork-managed keys from `opencode.jsonc` into the runtime database.
          </LayoutSectionItemDescription>
          <LayoutSectionItemHeaderActions>
            <Button
              type="button"
          ...
          <div className="space-y-3 rounded-xl border border-gray-6 bg-gray-1/60 p-3 text-xs text-gray-10">
            <div className="space-y-2 rounded-xl border border-blue-6/50 bg-blue-2/40 p-3">
              ...
              <div className="text-[11px] text-gray-9">
                This is the VenomCowork-built config object injected through `OPENCODE_CONFIG_CONTENT`. It includes VenomCowork defaults plus runtime DB values.
              </div>
            ...
            {props.configStatus.sources ? (
              <div className="space-y-3">
                <div>
                  ...
                  <div className="text-[11px] text-gray-9">
                    OpenCode also reads its own project and global config files. VenomCowork injects the runtime config separately; for VenomCowork-managed keys, the injected config is the source to inspect.
                  </div>
                </div>
                ...
                />
                <RuntimeConfigSourceBlock
                  title="VenomCowork runtime DB"
                  description="VenomCowork-managed runtime values stored outside workspace files."
                  keys={props.configStatus.sources.runtimeDatabase.keys}
                  config={props.configStatus.sources.runtimeDatabase.config}
                />
                <RuntimeConfigSourceBlock
                  title="VenomCowork injected config"
                  description="The object VenomCowork injects into OpenCode at runtime."
      ...
      <LayoutSectionItem className="gap-3">
        ...
        <div className="flex flex-wrap gap-2 pt-2">
          <Button
            ...
            disabled={props.busy || props.venomcoworkReconnectBusy || !props.venomcoworkServerUrl.trim()}
          >
            <RefreshCcw size={14} className={props.venomcoworkReconnectBusy ? "animate-spin" : ""} />
            {props.venomcoworkReconnectBusy ? t("settings.reconnecting") : t("settings.reconnect_server")}
          </Button>
          ...
          {props.isLocalEngineRunning ? (
            ...
            >
              ...
              {props.restartBusy ? t("settings.restarting") : t("settings.restart_venomcowork_server")}
            </Button>
          ) : null}
          ...
          {!props.isLocalEngineRunning && props.venomcoworkServerStatus === "connected" ? (
            <Button
              type="button"
              variant="outline"
              size="sm"
