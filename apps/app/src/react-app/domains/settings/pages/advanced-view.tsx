...
export function AdvancedView(props: AdvancedViewProps) {
  ...
  const venomcoworkTone: AdvancedStatusTone = (() => {
    switch (props.venomcoworkServerStatus) {
      case "connected":
        return "ready";
      case "limited":
  ...
  const clientDetailLines = props.clientConnected
    ...
    : [
        ...
        "VenomCowork server config sources below can still be inspected.",
      ];

  const venomcoworkDetailLines = props.venomcoworkServerStatus === "connected"
    ? ["Runtime DB, workspace config, and migration diagnostics are available."]
    : ["Runtime config diagnostics need the VenomCowork server connection."];
  ...
  return (
    <LayoutStack>
      <AdvancedRuntimeSection
        ...
        venomcoworkTone={venomcoworkTone}
        venomcoworkDetailLines={venomcoworkDetailLines}
      />
