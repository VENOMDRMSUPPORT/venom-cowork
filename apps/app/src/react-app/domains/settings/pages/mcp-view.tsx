...
import { isBuiltInVenomCoworkExtension, getMcpServerName, type McpDirectoryInfo } from "../../../../app/constants";
...
import {
  isVenomCoworkExtensionEnabled,
  isVenomCoworkExtensionHidden,
  VENOMCOWORK_EXTENSION_STATE_CHANGED,
  setVenomCoworkExtensionEnabled,
  setVenomCoworkExtensionHidden,
...
export type McpViewProps = {
  ...
  /** Organization policy restriction for VenomCowork-provided built-in extensions. */
...
const serviceIcon = (name: string) => {
  ...
  if (lower.includes("venomcowork") && lower.includes("cloud")) return Cloud;
  if (lower.includes("venomcowork") && lower.includes("ui")) return MonitorSmartphone;
  return Plug2;
};
...
const serviceColor = (name: string) => {
  ...
  if (lower.includes("venomcowork")) return "text-gray-12";
...
const serviceIconBg = (name: string) => {
  ...
  if (lower.includes("venomcowork")) return "bg-gray-3 border-gray-6";
...
export function McpView(props: McpViewProps) {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        const computerUseCommand = await window.__VENOMCOWORK_ELECTRON__?.invokeDesktop?.("getComputerUseMcpCommand");
  ...
  const launchCommandForEntry = (entry: McpDirectoryInfo) => {
    if (entry.serverName === "venomcowork-ui") return venomcoworkUiMcpCommand ?? undefined;
  ...
  const hiddenCount = quickConnectList.filter((entry) => isVenomCoworkExtensionHidden(entry)).length +
    (props.installedSkills ?? []).filter((skill) => isVenomCoworkExtensionHidden(getSkillHiddenId(skill))).length +
    (props.installedPlugins ?? []).filter((plugin) => isVenomCoworkExtensionHidden(`plugin:${plugin.pluginId}`)).length;
  const policyHiddenBuiltInCount = props.builtInExtensionsDisabled
    ? quickConnectList.filter((entry) => isBuiltInVenomCoworkExtension(entry) && !isVenomCoworkExtensionHidden(entry)).length
    : 0;
    ...
    <section className="space-y-8 max-w-3xl w-full animate-in fade-in duration-300">
      ...
      {props.builtInExtensionsDisabled ? (
        <div className="rounded-xl border border-amber-6 bg-amber-2 px-4 py-3 text-xs text-amber-11">
          Built-in VenomCowork extensions are disabled by your organization. Use Show hidden to review blocked built-ins.
        </div>
      ) : null}
      ...
      <McpQuickConnectSection
        entries={
          quickConnectList.filter((entry) => {
            if (!showHidden && (isVenomCoworkExtensionHidden(entry) || (props.builtInExtensionsDisabled && isBuiltInVenomCoworkExtension(entry)))) return false;
        ...
        installedSkills={
          (props.installedSkills ?? []).filter((skill) => {
            if (!showHidden && isVenomCoworkExtensionHidden(getSkillHiddenId(skill))) return false;
        ...
        installedPlugins={
          (props.installedPlugins ?? []).filter((plugin) => {
            if (!showHidden && isVenomCoworkExtensionHidden(`plugin:${plugin.pluginId}`)) return false;
        ...
        isEntryHidden={(entry) => isVenomCoworkExtensionHidden(entry)}
        isSkillHidden={(skill) => isVenomCoworkExtensionHidden(getSkillHiddenId(skill))}
        isPluginHidden={(plugin) => isVenomCoworkExtensionHidden(`plugin:${plugin.pluginId}`)}
        disabledReasonForEntry={(entry) =>
          props.builtInExtensionsDisabled && isBuiltInVenomCoworkExtension(entry)
            ? builtInExtensionDisabledReason
            : null
        }
        isConfigured={(entry) => {
          if (props.builtInExtensionsDisabled && isBuiltInVenomCoworkExtension(entry)) return false;
          const result = enablementForEntry(entry);
          if (result) return result.active;
          // Fallback for entries without enablement context.
          if (isToggleOnlyExtension(entry)) return isVenomCoworkExtensionEnabled(entry);
      ...
      {detailEntry ? (() => {
        ...
        const hidden = isVenomCoworkExtensionHidden(detailEntry);
        const disabledReason = props.builtInExtensionsDisabled && isBuiltInVenomCoworkExtension(detailEntry)
          ? builtInExtensionDisabledReason
          : null;
        const isConnected = disabledReason
          ? false
          : isToggleOnlyExtension(detailEntry)
          ? isVenomCoworkExtensionEnabled(detailEntry)
        ...
        return (
          <ExtensionDetailModal
            ...
            environment={detailEntry.serverName === "venomcowork-ui" ? venomcoworkUiMcpEnvironment ?? undefined : undefined}
            ...
            onConnect={disabledReason ? undefined : isToggleOnlyExtension(detailEntry) ? () => {
              setVenomCoworkExtensionEnabled(detailEntry, true);
              setDetailEntry(null);
            ...
            onUninstall={disabledReason ? undefined : isToggleOnlyExtension(detailEntry) && isConnected ? () => {
              setVenomCoworkExtensionEnabled(detailEntry, false);
            } : isQuickConnectConfigured(detailEntry) ? () => {
              const slug = getMcpIdentityKey(detailEntry);
              props.removeMcp(slug);
              setDetailEntry(null);
            } : undefined}
            onHide={() => setVenomCoworkExtensionHidden(detailEntry, true)}
            onShow={() => setVenomCoworkExtensionHidden(detailEntry, false)}
          />
        );
      })() : null}

      {detailSkill ? (() => {
        const hidden = isVenomCoworkExtensionHidden(getSkillHiddenId(detailSkill));
        return (
          <ExtensionDetailModal
            open={!!detailSkill}
            ...
            onHide={() => setVenomCoworkExtensionHidden(getSkillHiddenId(detailSkill), true)}
            onShow={() => setVenomCoworkExtensionHidden(getSkillHiddenId(detailSkill), false)}
          />
        );
      })() : null}

      {detailPlugin ? (() => {
        const hidden = isVenomCoworkExtensionHidden(`plugin:${detailPlugin.pluginId}`);
        return (
          <ExtensionDetailModal
            open={!!detailPlugin}
            ...
            onHide={() => setVenomCoworkExtensionHidden(`plugin:${detailPlugin.pluginId}`, true)}
            onShow={() => setVenomCoworkExtensionHidden(`plugin:${detailPlugin.pluginId}`, false)}
          />
        );
      })() : null}
    </section>
