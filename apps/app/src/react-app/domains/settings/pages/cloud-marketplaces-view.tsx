...
type BuiltInMarketplaceRow = {
  source: "built-in";
  marketplaceId: "venomcowork-builtins";
  marketplaceName: string;
...
function isCloudBuiltInPlugin(plugin: DenOrgPlugin) {
  return plugin.extension?.sourceFormat === "venomcowork-builtin";
}
...
export function CloudMarketplacesView({
  ...
  const builtInRows = React.useMemo<BuiltInMarketplaceRow[]>(() => {
    return builtInEntries.map((entry) => {
      ...
      return {
        source: "built-in",
        marketplaceId: "venomcowork-builtins",
        marketplaceName: "VenomCowork Built-ins",
        entry,
        active,
  ...
  const marketplaceOptions = React.useMemo(
    () => canShowRows ? [
      ...(builtInRows.length > 0 ? [{ id: "venomcowork-builtins", name: "VenomCowork Built-ins" }] : []),
    ...
    <SettingsSection>
      <SettingsSectionHeader>
        <SettingsSectionHeaderContent>
          ...
          <SettingsSectionHeaderDescription>
            Browse built-in VenomCowork extensions and organization marketplace extensions. Claude-compatible plugins are normalized into VenomCowork extensions with installable resources such as skills, MCPs, commands, or tools.
          </SettingsSectionHeaderDescription>
        </SettingsSectionHeaderContent>
        <SettingsSectionHeaderActions>
          <RefreshButton
      ...
      {!isSignedIn ? (
        <SettingsNotice>
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <span>You can use VenomCowork without an account. Sign in to VenomCowork Cloud to load the Marketplace, including VenomCowork's built-in extensions and any organization marketplaces.</span>
