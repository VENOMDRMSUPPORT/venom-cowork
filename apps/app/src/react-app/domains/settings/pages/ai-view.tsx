...
export type AiSettingsViewProps = {
  ...
  onDismissVenomCoworkModels?: () => void | Promise<void>;
  ...
  return (
    <LayoutStack>
      ...
      <LayoutSection>
        ...
        {props.showVenomCoworkModelsSubscribe ? (
          <LayoutSectionItem className="relative overflow-hidden rounded-2xl border border-blue-6 bg-blue-2/30 px-4 py-4">
            <button
              ...
              onClick={() => void props.onDismissVenomCoworkModels?.()}
              aria-label="Dismiss VenomCowork Models banner"
            >
              <X className="size-3.5" />
            </button>
            <div className="flex flex-col gap-4 pr-8 sm:flex-row sm:items-start sm:justify-between">
              <div className="flex min-w-0 gap-3">
                <ProviderIcon providerId="venomcowork" size={22} className="mt-0.5 shrink-0 text-blue-11" />
                <div className="min-w-0 space-y-2">
                  <div>
                    <div className="text-sm font-medium text-dls-text">VenomCowork Models</div>
                    <div className="mt-0.5 text-xs text-muted-foreground">
                      Hosted frontier models for VenomCowork tasks without managing provider API keys.
                    </div>
                  </div>
                  <div className="flex flex-wrap gap-2 text-[11px] text-blue-11">
                    <span className="inline-flex items-center gap-1 rounded-full border border-blue-6 bg-blue-3 px-2 py-0.5">
                      <CheckCircle2 className="size-3" /> Managed by VenomCowork Cloud
                    </span>
                    <span className="inline-flex items-center gap-1 rounded-full border border-blue-6 bg-blue-3 px-2 py-0.5">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Pricing is handled through VenomCowork Cloud. You can continue using OpenCode Zen or your own providers.
                  </p>
                </div>
              </div>
              <Button
                className="shrink-0"
                onClick={() => void props.onSubscribeVenomCoworkModels?.()}
                disabled={props.busy || props.providerAuthBusy}
              >
                Subscribe
