...
const EXTENSION_DISABLED_KEY_PREFIX = "venomcowork.extension.disabled.";
const EXTENSION_ENABLED_KEY_PREFIX = "venomcowork.extension.enabled.";
const EXTENSION_HIDDEN_KEY_PREFIX = "venomcowork.extension.hidden.";
export const VENOMCOWORK_EXTENSION_STATE_CHANGED = "venomcowork:extension-state-changed";

export function getExtensionId(entry: McpDirectoryInfo): string {
  return entry.id ?? entry.serverName ?? getMcpServerName(entry);
}

export function isVenomCoworkExtensionEnabled(entry: McpDirectoryInfo): boolean {
...
export function setVenomCoworkExtensionEnabled(entry: McpDirectoryInfo, enabled: boolean) {
  ...
  window.dispatchEvent(new CustomEvent(VENOMCOWORK_EXTENSION_STATE_CHANGED, {
    detail: { id, enabled },
  }));
}

export function isVenomCoworkExtensionHidden(entryOrId: McpDirectoryInfo | string): boolean {
...
export function setVenomCoworkExtensionHidden(entryOrId: McpDirectoryInfo | string, hidden: boolean) {
  const id = typeof entryOrId === "string" ? entryOrId : getExtensionId(entryOrId);
  if (typeof window === "undefined") return;
