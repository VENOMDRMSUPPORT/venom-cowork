...
function hasDesktopBridge() {
  return typeof window !== "undefined" && Boolean(window.__VENOMCOWORK_ELECTRON__?.invokeDesktop);
}
...
export function ComputerUseConfig({
  ...
  const {
    ...
    mutationFn: async () => {
      if (!hasDesktopBridge()) {
        throw new Error("Computer Use is Mac only and requires the VenomCowork desktop app on macOS.");
      }
    ...
    <Card variant="outline" size="sm">
      ...
      <CardContent className="space-y-4">
        ...
        >
          ...
          >
            ...
            <span className="min-w-0 break-words">
              {connected ? "Configured" : connecting ? "Connectingâ€¦" : "Connect MCP"}
            </span>
          </Button>
        </SetupRow>
        ...
        <SetupRow
          ...
          description="Opens the VenomCowork Computer Use helper. Grant both permissions there, then click Verify below."
          complete={allGranted}
        >
          <div className="flex w-full min-w-0 flex-col gap-3">
            ...
            >
              ...
              <span className="min-w-0 wrap-break-word">
                {isBusy ? "Openingâ€¦" : allGranted ? "Reopen helper" : "Grant permissions"}
              </span>
            </Button>
          </div>
        </SetupRow>
...
function Pill({ label, granted, checked }: PillProps) {
  return (
    <div className="flex min-w-0 items-center justify-between gap-2 rounded-lg border border-border bg-muted/40 px-3 py-2">
      ...
      >
        {!checked ? "â€¦" : granted ? "Granted" : "Needed"}
      </span>
    </div>
  );
}
