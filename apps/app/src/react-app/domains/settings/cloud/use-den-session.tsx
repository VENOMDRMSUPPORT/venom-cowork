...
declare global {
  interface WindowEventMap {
    "venomcowork-den-session-updated": CustomEvent<DenSessionUpdatedDetail>;
  }
}
...
function parseManualAuthInput(value: string) {
  ...
  try {
    ...
    if (
      (protocol === "venomcowork:" || protocol === "venomcowork-dev:") &&
...
export function useDenSession({
  ...
  const clearSignedInState = React.useCallback(
    (
      ...
      try {
        const raw = window.localStorage.getItem("venomcowork.acknowledgedProviders");
        if (raw) {
