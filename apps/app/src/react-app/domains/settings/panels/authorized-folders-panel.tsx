...
export function AuthorizedFoldersPanel(props: AuthorizedFoldersPanelProps) {
  ...
  const venomcoworkServerReady = props.venomcoworkServerStatus === "connected";
  const venomcoworkServerWorkspaceReady = Boolean(props.runtimeWorkspaceId);
  const canReadConfig =
    venomcoworkServerReady &&
    venomcoworkServerWorkspaceReady &&
    (props.venomcoworkServerCapabilities?.config?.read ?? false);
  const canWriteConfig =
    venomcoworkServerReady &&
    venomcoworkServerWorkspaceReady &&
    (props.venomcoworkServerCapabilities?.config?.write ?? false);

  const authorizedFoldersHint = useMemo(() => {
    if (!venomcoworkServerReady) return t("context_panel.server_disconnected");
    if (!venomcoworkServerWorkspaceReady) return t("context_panel.no_server_workspace");
    ...
    return null;
  }, [canReadConfig, canWriteConfig, venomcoworkServerReady, venomcoworkServerWorkspaceReady]);
  ...
  useEffect(() => {
    const venomcoworkClient = props.venomcoworkServerClient;
    const venomcoworkWorkspaceId = props.runtimeWorkspaceId;

    if (!venomcoworkClient || !venomcoworkWorkspaceId || !canReadConfig) {
    ...
    void (async () => {
      try {
        const response = await venomcoworkClient.listAuthorizedFolders(venomcoworkWorkspaceId);
        if (cancelled) return;
  ...
  }, [canReadConfig, props.venomcoworkServerClient, props.runtimeWorkspaceId]);

  const persistAuthorizedFolders = useCallback(async (nextFolders: string[]) => {
    const venomcoworkClient = props.venomcoworkServerClient;
    const venomcoworkWorkspaceId = props.runtimeWorkspaceId;
    if (!venomcoworkClient || !venomcoworkWorkspaceId || !canWriteConfig) {
    ...
    try {
      const response = await venomcoworkClient.setAuthorizedFolders(venomcoworkWorkspaceId, nextFolders);
  ...
  }, [canWriteConfig, props.onConfigUpdated, props.venomcoworkServerClient, props.runtimeWorkspaceId]);
