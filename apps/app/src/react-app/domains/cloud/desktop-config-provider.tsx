...
import { desktopPolicyKeys } from "@venom-cowork/types/den/desktop-policies";
...
const DESKTOP_CONFIG_CACHE_PREFIX = "venomcowork.den.desktopConfig:";
...
function writeCachedDesktopConfig(key: string, config: DenDesktopConfig) {
  ...
  try {
    ...
    // Quota / private-browsing failures are non-fatal â€” we just miss the cache next boot.
  }
}
...
export function DesktopConfigProvider({ children }: DesktopConfigProviderProps) {
  ...
  // Monotonic run id â€” same guard-against-stale-resolution pattern as DenAuthProvider.
 ...
 * not the loading state â€” feature gates should read through this.
 */
...
/**
 * Single-restriction hook â€” returns true/false for a specific key.
