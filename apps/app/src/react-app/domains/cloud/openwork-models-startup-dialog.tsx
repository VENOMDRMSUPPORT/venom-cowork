...
import {
  ...
  type VenomCoworkModelPreview,
} from "./venomcowork-models-promo";

type VenomCoworkModelsStartupDialogProps = {
  open: boolean;
  isSignedIn: boolean;
  models: VenomCoworkModelPreview[];
  onSubscribe: () => void;
  onContinueWithout: () => void;
};

export function VenomCoworkModelsStartupDialog(props: VenomCoworkModelsStartupDialogProps) {
      ...
      <DialogContent className="w-full max-w-lg overflow-hidden sm:max-w-lg">
        <DialogHeader>
          <div className="mb-2 flex size-11 items-center justify-center rounded-2xl border border-blue-6 bg-blue-2 text-blue-11">
            <ProviderIcon providerId={VENOMCOWORK_MODELS_PROVIDER_ID} providerName={VENOMCOWORK_MODELS_PROVIDER_NAME} size={22} />
          </div>
          <DialogTitle>Use VenomCowork Models without API keys</DialogTitle>
          <DialogDescription>
            VenomCowork Models gives your workspace hosted frontier models managed by VenomCowork Cloud. You can still use your own providers whenever you prefer.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-3">
          ...
          <div className="grid gap-2 text-xs text-dls-secondary sm:grid-cols-2">
            <div className="flex gap-2 rounded-xl bg-dls-hover/50 p-3">
              ...
              <span>Managed model access for VenomCowork tasks and shared workflows.</span>
            </div>
            <div className="flex gap-2 rounded-xl bg-dls-hover/50 p-3">
          ...
          <p className="text-xs text-dls-secondary">
            Pricing is handled through VenomCowork Cloud. Continue without it to use OpenCode Zen or your own provider keys.
          </p>
        </div>

        <DialogFooter className="gap-2 sm:justify-between">
          <Button variant="ghost" onClick={props.onContinueWithout}>
            Continue without VenomCowork Models
          </Button>
