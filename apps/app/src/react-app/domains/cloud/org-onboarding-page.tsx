...
const RELOAD_AFTER_ONBOARDING_KEY = "venomcowork.reloadAfterOrgOnboarding";
...
function markProvidersSeen(providers: DenOrgLlmProvider[]) {
  ...
  try {
    ...
    window.localStorage.setItem("venomcowork.seenProviderIds", JSON.stringify([...ids]));
  } catch {}
}
...
export function ResourceSelectionPage() {
  ...
  // Redirect if no auth or no org â€” can't show onboarding without them
  useEffect(() => {
    markRouteReady();
  }, [markRouteReady]);
          ...
          <PageContent>
            <Empty className="h-fit flex-none">
              <EmptyHeader>
                ...
                <EmptyDescription>
                  Add AI providers, marketplaces, or workers from the VenomCowork Cloud dashboard.
                </EmptyDescription>
              </EmptyHeader>
              <EmptyContent>
                <Button
    ...
    <div className="flex items-center gap-3 rounded-xl border border-border px-3 py-3 -mx-2">
      <div className="min-w-0 flex-1">
        ...
        <div className="mt-0.5 text-xs text-muted-foreground">
          {worker.status}{worker.provider ? ` Â· ${worker.provider}` : ""}
        </div>
      </div>
    </div>
  );
  ...
  const handleUseAsDefault = () => {
    ...
    if (isSelected) {
      ...
      onSelectDefault({
        ...
        label: `${resolveProviderDisplayName(provider.name || provider.providerId)} Â· ${firstModel.name || resolveModelDisplayName(firstModel.id)}`,
      });
    }
  };
