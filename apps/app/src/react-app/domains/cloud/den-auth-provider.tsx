...
import { parseDenAuthDeepLink } from "../../../app/lib/venomcowork-links";
    ...
    try {
      ...
      setError(
        nextError instanceof Error
          ? nextError.message
          : "Failed to restore VenomCowork Cloud session.",
      );
      setStatus("signed_out");
    }
  }, []);
    ...
    const handleUrls = (urls: readonly string[]) => {
      for (const rawUrl of urls) {
        ...
        void createDenClient({ baseUrl: parsed.denBaseUrl })
          ...
          .then((result) => {
            if (!result.token) {
              throw new Error("Failed to sign in to VenomCowork Cloud.");
            }
            ...
            dispatchDenSessionUpdated({
              ...
              message:
                error instanceof Error
                  ? error.message
                  : "Failed to sign in to VenomCowork Cloud.",
            });
          });
      }
    };
