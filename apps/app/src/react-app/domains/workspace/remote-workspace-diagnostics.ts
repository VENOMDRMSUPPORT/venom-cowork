...
function displayWorkspaceName(workspace: unknown) {
  ...
  return (
    ...
    trim(value.venomcoworkWorkspaceName) ||
    trim(value.name) ||
    trim(value.id)
  );
}
...
export function resolveRemoteWorkspaceConnectionTarget(workspace: WorkspaceInfo): TargetResult {
  ...
  if (workspace.remoteType && workspace.remoteType !== "venomcowork") {
    return {
      ok: false,
      state: {
        status: "error",
        message: "Connection diagnostics are only available for VenomCowork remote workers.",
        checkedAt: Date.now(),
      },
    };
  }

  const rawHostUrl = trim(workspace.venomcoworkHostUrl) || trim(workspace.baseUrl);
  if (!rawHostUrl) {
    return {
      ok: false,
      state: {
...
export async function testRemoteWorkspaceConnection(
  ...
  if (!target.token) {
    return fail(
      remoteSupportMessage(`Token is missing for ${target.endpointLabel}. Edit connection and paste a valid VenomCowork token.`),
      checkedAt,
    );
  }
