...
/**
 * Provider ID â†’ friendly display name.
...
/**
 * Model ID â†’ friendly display name.
...
export function formatModelLabel(model: ModelRef, providers: ProviderListItem[] = []) {
  ...
  return `${providerLabel} Â· ${modelLabel}`;
}

export function isElectronRuntime() {
  return typeof window !== "undefined" && (window as Window).__VENOMCOWORK_ELECTRON__ != null;
}
...
const STARTUP_PREF_KEY = "venomcowork.startupPref";
const LEGACY_PREF_KEY = "venomcowork.modePref";
const LEGACY_PREF_KEY_ALT = "venomcowork_mode_pref";
 ...
 * instead â€” it returns a branded {@link import("../lib/session-scope").TransportDirectory TransportDirectory}
...
export function getWorkspaceTaskLoadErrorDisplay(workspace: WorkspaceInfo, error?: string | null) {
  ...
  const host = `${workspace.baseUrl ?? ""} ${workspace.venomcoworkHostUrl ?? ""}`.toLowerCase();
...
/** Build a concise detail line for a tool call â€” avoids dumping raw output */
...
const TRUNCATED_SYSTEM_PATH_PATTERNS = [
  /com\.[^/]+\.(venomcowork|opencode)/i, // macOS app bundle identifiers
  /\.venomcowork\.dev\//i, // VenomCowork dev paths
...
export function summarizeStep(part: Part): { title: string; detail?: string; isSkill?: boolean; skillName?: string; toolCategory?: string; status?: string } {
  ...
  if (part.type === "reasoning") {
    ...
    if (/^(?:thinking|reasoning)\s*(?::|-|â€“|â€”)?\s*$/i.test(lines[0] ?? "")) {
      lines.shift();
    }
    ...
    headline = headline.replace(/^(?:thinking|reasoning)\s*(?::|-|â€“|â€”)\s*/i, "").trim();
