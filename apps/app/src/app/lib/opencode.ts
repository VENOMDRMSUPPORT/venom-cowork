...
const resolveAuthHeader = (auth?: OpencodeAuth) => {
  if (auth?.mode === "venomcowork" && auth.token) {
 ...
 * minutes. For these endpoints we always use the webview's native fetch â€”
 * CORS is already wide open on the venomcowork/opencode stack, so there's no
...
export function createClient(baseUrl: string, directory?: string, auth?: OpencodeAuth) {
  ...
  sessionOverrides.list = (parameters?: SessionListParameters, options?: { throwOnError?: boolean }) => {
    if (!venomcoworkMount || !venomcoworkSessionClient) {
