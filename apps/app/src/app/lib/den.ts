...
} from "@venom-cowork/types/den/desktop-policies";
...
import { isDesktopDeployment } from "./venomcowork-deployment";
import {
...
import type {
  VenomCoworkExtensionContribution,
  VenomCoworkExtensionContributionType,
  VenomCoworkExtensionLifecycle,
  VenomCoworkExtensionManifest,
  VenomCoworkExtensionResource,
  VenomCoworkExtensionResourceType,
  VenomCoworkExtensionSetup,
  VenomCoworkExtensionSource,
  VenomCoworkExtensionSourceFormat,
} from "../extensions";

const STORAGE_BASE_URL = "venomcowork.den.baseUrl";
const STORAGE_API_BASE_URL = "venomcowork.den.apiBaseUrl";
const STORAGE_AUTH_TOKEN = "venomcowork.den.authToken";
const STORAGE_ACTIVE_ORG_ID = "venomcowork.den.activeOrgId";
const STORAGE_ACTIVE_ORG_SLUG = "venomcowork.den.activeOrgSlug";
const STORAGE_ACTIVE_ORG_NAME = "venomcowork.den.activeOrgName";
const ORG_PROXY_HEADER = "x-venomcowork-legacy-org-id";
...
export type DenOrgLlmProvider = {
  id: string;
  source: "models_dev" | "custom" | "venomcowork";
  providerId: string;
  name: string;
...
export type DenOrgExtensionProjection = {
  ...
  name: string;
  description: string | null;
  sourceFormat: VenomCoworkExtensionSourceFormat;
  manifest: VenomCoworkExtensionManifest | null;
};
...
export type DenOrgPlugin = {
  ...
  /** Preferred Den surface: plugins are normalized into VenomCowork extensions. */
...
export function buildDenAuthUrl(baseUrl: string, mode: "sign-in" | "sign-up"): string {
  ...
  if (isDesktopDeployment()) {
    ...
    target.searchParams.set("desktopScheme", "venomcowork");
  }
...
function getWorkerTokens(payload: unknown): DenWorkerTokens | null {
  ...
  return {
    ...
    venomcoworkUrl: connect && typeof connect.venomcoworkUrl === "string" ? connect.venomcoworkUrl : null,
...
function parseExtensionSourceFormat(value: unknown): VenomCoworkExtensionSourceFormat | null {
  switch (value) {
    case "venomcowork-builtin":
    case "venomcowork-extension-manifest":
    case "claude-plugin":
...
function parseExtensionSourceOrigin(value: unknown): VenomCoworkExtensionSource["origin"] | undefined {
  switch (value) {
    case "builtin":
    case "den":
    case "workspace":
...
function parseExtensionSource(value: unknown): VenomCoworkExtensionSource | null {
...
function parseExtensionResourceType(value: unknown): VenomCoworkExtensionResourceType | null {
  switch (value) {
    case "skill":
    case "agent":
    case "command":
...
function parseExtensionLocalCommandRef(value: unknown): VenomCoworkExtensionResource["localCommandRef"] | undefined {
  switch (value) {
    case "venomcowork.computerUseMcp":
    case "venomcowork.uiMcp":
      return value;
    default:
      return undefined;
  }
...
function parseExtensionResource(value: unknown): VenomCoworkExtensionResource | null {
  if (!isRecord(value) || typeof value.id !== "string") return null;
...
function parseExtensionContributionType(value: unknown): VenomCoworkExtensionContributionType | null {
  switch (value) {
    case "settings-panel":
...
function parseExtensionContributionLocation(value: unknown): VenomCoworkExtensionContribution["location"] | undefined {
  switch (value) {
...
function parseExtensionContribution(value: unknown): VenomCoworkExtensionContribution | null {
...
function parseExtensionSetup(value: unknown): VenomCoworkExtensionSetup | undefined {
...
function parseExtensionLifecycle(value: unknown): VenomCoworkExtensionLifecycle | undefined {
  if (!isRecord(value)) return undefined;
...
function parseExtensionPlatform(value: unknown): VenomCoworkExtensionManifest["platform"] | undefined {
  if (!Array.isArray(value)) return undefined;
...
function parseVenomCoworkExtensionManifest(value: unknown): VenomCoworkExtensionManifest | null {
  if (
    !isRecord(value) ||
...
function parseDenExtensionProjection(value: unknown): DenOrgExtensionProjection | null {
  ...
  return {
    ...
    description: typeof value.description === "string" ? value.description : null,
    sourceFormat,
    manifest: parseVenomCoworkExtensionManifest(value.manifest),
  };
}
