// Tauri â†’ Electron migration snapshot ingestion (Electron side only).
//
...
export const MIGRATION_KEY_PATTERNS: Array<RegExp> = [
  /^venomcowork\.react\.activeWorkspace$/,
  /^venomcowork\.react\.sessionByWorkspace$/,
  /^venomcowork\.server\.list$/,
  /^venomcowork\.server\.active$/,
  /^venomcowork\.server\.urlOverride$/,
  /^venomcowork\.server\.token$/,
];
...
function electronMigrationBridge(): ElectronMigrationBridge | null {
  ...
  const bridge = (window as unknown as {
    __VENOMCOWORK_ELECTRON__?: { migration?: ElectronMigrationBridge };
...
export const MIGRATION_DEFER_KEY = "venomcowork.migration.deferredUntil";
