...
type DevRoot = typeof globalThis & {
  __venomcoworkDevLogSeq?: number;
  __venomcoworkDevLogs?: DevLogRecord[];
};
...
export const recordDevLog = (
  ...
  const id = (root.__venomcoworkDevLogSeq ?? 0) + 1;
  root.__venomcoworkDevLogSeq = id;
...
export const clearDevLogs = () => {
  const root = globalThis as DevRoot;
  root.__venomcoworkDevLogs = [];
  root.__venomcoworkDevLogSeq = 0;
};
