...
export type VenomcoworkWorkspaceExport = {
  ...
  venomcowork?: Record<string, unknown>;
...
export const DEFAULT_VENOMCOWORK_SERVER_PORT = 8787;

const STORAGE_URL_OVERRIDE = "venomcowork.server.urlOverride";
const STORAGE_PORT_OVERRIDE = "venomcowork.server.port";
const STORAGE_TOKEN = "venomcowork.server.token";
...
const VENOMCOWORK_INVITE_PARAM_URL = "ow_url";
const VENOMCOWORK_INVITE_PARAM_TOKEN = "ow_token";
...
export function readVenomcoworkConnectInviteFromSearch(input: string | URLSearchParams) {
  ...
  const startupRaw = search.get(VENOMCOWORK_INVITE_PARAM_STARTUP)?.trim() ?? "";
  const startup = startupRaw === "server" ? "server" : undefined;
  const autoConnect = search.get(VENOMCOWORK_INVITE_PARAM_AUTO_CONNECT)?.trim() === "1";
...
export function stripVenomcoworkConnectInviteFromUrl(input: string) {
  try {
    ...
    url.searchParams.delete(VENOMCOWORK_INVITE_PARAM_TOKEN);
    url.searchParams.delete(VENOMCOWORK_INVITE_PARAM_STARTUP);
    url.searchParams.delete(VENOMCOWORK_INVITE_PARAM_AUTO_CONNECT);
    return url.toString();
  } catch {
    return input;
  }
...
export function hydrateVenomcoworkServerSettingsFromEnv() {
  ...
  const envUrl = typeof import.meta.env?.VITE_VENOMCOWORK_URL === "string"
    ? import.meta.env.VITE_VENOMCOWORK_URL.trim()
    : "";
  const envPort = typeof import.meta.env?.VITE_VENOMCOWORK_PORT === "string"
    ? import.meta.env.VITE_VENOMCOWORK_PORT.trim()
    : "";
  const envToken = typeof import.meta.env?.VITE_VENOMCOWORK_TOKEN === "string"
    ? import.meta.env.VITE_VENOMCOWORK_TOKEN.trim()
    : "";
  const envHostToken = typeof import.meta.env?.VITE_VENOMCOWORK_HOST_TOKEN === "string"
    ? import.meta.env.VITE_VENOMCOWORK_HOST_TOKEN.trim()
    : "";
...
function buildHeaders(
  ...
  if (hostToken) {
    headers["X-VenomCowork-Host-Token"] = hostToken;
  }
  if (extra) {
...
// blocks until the body closes â€” that freezes the webview for infinite bodies.
const VENOMCOWORK_STREAM_URL_RE = /\/events(\b|\?)|\/event-stream\b|\/stream\b/;

function isStreamUrl(url: string): boolean {
  return VENOMCOWORK_STREAM_URL_RE.test(url);
}
...
const DEFAULT_VENOMCOWORK_SERVER_TIMEOUT_MS = 10_000;
...
async function requestJson<T>(
  ...
  const response = await fetchWithTimeout(
    ...
    options.timeoutMs ?? DEFAULT_VENOMCOWORK_SERVER_TIMEOUT_MS,
  );
...
export function createVenomcoworkServerClient(options: { baseUrl: string; token?: string; hostToken?: string }) {
  ...
  return {
    ...
    createRemoteWorkspace: (payload: {
      ...
      venomcoworkWorkspaceId?: string | null;
      venomcoworkWorkspaceName?: string | null;
      displayName?: string | null;
      directory?: string | null;
      remoteType?: "venomcowork" | "opencode";
    ...
    getConfig: (workspaceId: string) =>
      requestJson<{ opencode: Record<string, unknown>; venomcowork: Record<string, unknown>; updatedAt?: number | null }>(
        baseUrl,
    ...
    patchConfig: (workspaceId: string, payload: { opencode?: Record<string, unknown>; venomcowork?: Record<string, unknown> }) =>
    ...
    // User-level env vars (host-auth only â€” desktop shell is the sole caller).
