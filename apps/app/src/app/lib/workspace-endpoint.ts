 ...
 * local VenomCowork server. For workspaces hosted on a remote VenomCowork worker
 * (`id` starts with `rem_` and `workspaceType === "remote"`), it's the
 * `baseUrl`/`venomcoworkHostUrl` and `venomcoworkToken` saved on the workspace
 ...
 *
 * Don't compose `<baseUrl>/workspace/<id>` by hand â€” that pattern is what
...
export type ResolvedWorkspaceEndpoint = {
  /** Host URL of the VenomCowork server that owns this workspace (no `/workspace` mount). */
  baseUrl: string;
...
export function workspaceServerId(workspace: WorkspaceEndpointInput): string {
  ...
  const explicit = workspace.venomcoworkWorkspaceId?.trim();
...
function pickRemoteBaseUrl(workspace: WorkspaceEndpointInput): string {
  if (!workspace) return "";
  return (workspace.baseUrl ?? workspace.venomcoworkHostUrl ?? "").trim();
}

function pickRemoteToken(workspace: WorkspaceEndpointInput): string {
  if (!workspace) return "";
  return (
    workspace.venomcoworkToken ??
    workspace.venomcoworkClientToken ??
    workspace.venomcoworkHostToken ??
    ""
  ).trim();
}
