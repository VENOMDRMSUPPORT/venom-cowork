...
export type {
  ...
  WorkspaceVenomcoworkConfig,
  AppBuildInfo,
  DesktopBootstrapConfig,
  OrchestratorDetachedHost,
  SandboxDoctorResult,
  VenomcoworkDockerCleanupResult,
...
declare global {
  interface Window {
    __VENOMCOWORK_ZOOM_FACTOR__?: number;
    __VENOMCOWORK_ELECTRON__?: {
...
async function invokeElectronHelper<T>(command: string, ...args: unknown[]): Promise<T> {
  const invokeDesktop = window.__VENOMCOWORK_ELECTRON__?.invokeDesktop;
  if (!invokeDesktop) {
...
// Pure utility â€” resolves the selected workspace ID from a workspace list
...
// ---------------------------------------------------------------------------
// desktopFetch â€” proxies non-loopback requests through Electron main process
// ---------------------------------------------------------------------------
...
export const desktopFetch: typeof globalThis.fetch = async (input, init) => {
  ...
  // â€” the remote would then reject every request with "Invalid bearer token".
  let url: string;
  ...
  if (typeof Request !== "undefined" && input instanceof Request) {
    ...
    if (typeof init?.body === "string") {
      ...
      // Request body is a stream â€” buffer to text so it survives the IPC hop
...
export async function openDesktopUrl(url: string): Promise<void> {
  const openExternal = window.__VENOMCOWORK_ELECTRON__?.shell?.openExternal;
  if (openExternal) {
...
export async function relaunchDesktopApp(): Promise<void> {
  await window.__VENOMCOWORK_ELECTRON__?.shell?.relaunch?.();
}
...
export async function subscribeDesktopDeepLinks(
  ...
  const initialUrls = window.__VENOMCOWORK_ELECTRON__?.meta?.initialDeepLinks;
