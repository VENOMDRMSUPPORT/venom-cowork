...
function isSupportedDeepLinkProtocol(protocol: string): boolean {
  ...
  return normalized === "venomcowork:" || normalized === "venomcowork-dev:" || normalized === "https:" || normalized === "http:";
}
...
export function parseRemoteConnectDeepLink(rawUrl: string): RemoteWorkspaceDefaults | null {
  ...
  return {
    venomcoworkHostUrl: normalizedHostUrl,
    venomcoworkToken: token,
    directory: null,
...
function normalizeDebugDeepLinkInput(rawValue: string): string {
  ...
  const directMatch = trimmed.match(/(?:venomcowork-dev|venomcowork|https?):\/\/[^\s"'<>]+/i);
