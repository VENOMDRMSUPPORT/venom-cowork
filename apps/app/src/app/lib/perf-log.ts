...
type PerfRoot = typeof globalThis & {
  __venomcoworkPerfSeq?: number;
  __venomcoworkPerfLogs?: PerfLogRecord[];
  __venomcoworkPerfConsoleAt?: Record<string, number>;
  __venomcoworkPerfConsoleSuppressed?: Record<string, number>;
};
...
export const recordPerfLog = (
  ...
  const id = (root.__venomcoworkPerfSeq ?? 0) + 1;
  root.__venomcoworkPerfSeq = id;
  ...
  try {
    ...
    const now = Date.now();
    const lastByKey = root.__venomcoworkPerfConsoleAt ?? (root.__venomcoworkPerfConsoleAt = {});
    const suppressedByKey =
      root.__venomcoworkPerfConsoleSuppressed ?? (root.__venomcoworkPerfConsoleSuppressed = {});
...
export const clearPerfLogs = () => {
  const root = globalThis as PerfRoot;
  root.__venomcoworkPerfLogs = [];
  root.__venomcoworkPerfSeq = 0;
};
