...
type ModelSelectVenomCoworkItem = {
  kind: "venomcowork";
  id: string;
  title: string;
  subtitle: string;
...
type ModelSelectItem = ModelSelectModelItem | ModelSelectVenomCoworkItem;
...
function venomCoworkModelsGroup(): ModelSelectGroup {
  return {
    ...
    promo: true,
    items: VENOMCOWORK_MODEL_PREVIEWS.map((model) => ({
      kind: "venomcowork",
      id: model.id,
      title: model.title,
...
export function ModelSelect({
  ...
  const [promoHidden, setPromoHidden] = React.useState(isVenomCoworkModelsPromoHidden);
  ...
  const showVenomCoworkModelsPromo = React.useMemo(
    () => !promoHidden && !hasVenomCoworkModelsProvider(modelOptions.map((option) => option.providerID)),
  ...
  const handleVenomCoworkModels = React.useCallback(() => {
    onOpenChange(false);
    setSearch("");
  ...
  const handleHideVenomCoworkModels = React.useCallback(() => {
    hideVenomCoworkModelsPromo();
    setPromoHidden(true);
  }, []);
              ...
              >
                ...
                <CommandCollection>
                  {(item: ModelSelectItem) => {
                    if (item.kind === "venomcowork") {
                      return (
                        <CommandItem
                          className="gap-2 border border-blue-6/50 bg-blue-2/40 data-highlighted:bg-blue-3"
                          key={item.id}
                          value={`${VENOMCOWORK_MODELS_PROVIDER_NAME} ${item.title} ${item.id} sign in subscribe`}
                          onClick={handleVenomCoworkModels}
                        >
                          <ProviderIcon
                            providerId={VENOMCOWORK_MODELS_PROVIDER_ID}
                            providerName={VENOMCOWORK_MODELS_PROVIDER_NAME}
            ...
            <div className="flex items-center gap-1">
              ...
              {showVenomCoworkModelsPromo ? (
                <button
                  ...
                  onClick={handleHideVenomCoworkModels}
                >
                  Hide
                </button>
              ) : null}
