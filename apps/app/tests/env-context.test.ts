...
import {
  buildVenomcoworkEnvSystemContext,
  clearVenomcoworkEnvSystemContextCache,
} from "../src/react-app/domains/session/sync/env-context";

function client(keys: string[], calls: { count: number }): VenomcoworkServerClient {
  return {
...
describe("buildVenomcoworkEnvSystemContext", () => {
  test("lists configured key names without inventing secret values", async () => {
    clearVenomcoworkEnvSystemContextCache();
    const calls = { count: 0 };
    const context = await buildVenomcoworkEnvSystemContext(
  ...
  test("caches key context per session", async () => {
    ...
    await buildVenomcoworkEnvSystemContext(server, {
      cacheKey: "session-a",
      readPendingChanges: () => false,
    });
    await buildVenomcoworkEnvSystemContext(server, {
  ...
  test("does not truncate long key lists", async () => {
    ...
    const context = await buildVenomcoworkEnvSystemContext(client(keys, calls), {
      cacheKey: "session-a",
      readPendingChanges: () => false,
    });
  ...
  test("skips context while environment changes are pending", async () => {
    clearVenomcoworkEnvSystemContextCache();
    const calls = { count: 0 };
    const context = await buildVenomcoworkEnvSystemContext(client(["ANTHROPIC_API_KEY"], calls), {
      cacheKey: "session-a",
