...
addHost(process.env.VENOMCOWORK_PUBLIC_HOST ?? null);
...
const buildAppVersion =
  process.env.VITE_VENOMCOWORK_APP_VERSION?.trim() ||
  readPackageVersion(desktopPackagePath) ||
  readPackageVersion(appPackagePath) ||
  "0.0.0";

// Load the Tauri â†’ Electron migration-release fragment if present. Written
...
const isElectronPackagedBuild = process.env.VENOMCOWORK_ELECTRON_BUILD === "1";
...
export default defineConfig({
  ...
  define: {
    ...
    "import.meta.env.VITE_VENOMCOWORK_APP_VERSION": JSON.stringify(buildAppVersion),
  },
  plugins: [
    {
      name: "venomcowork-dev-server-id",
      configureServer(server) {
        server.middlewares.use("/__venomcowork_dev_server_id", (_req, res) => {
