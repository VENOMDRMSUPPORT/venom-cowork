...
try {
  ...
  const root = ".venomcowork/test-engine";
  const nestedDir = path.join(root, "nested");
  const filePath = path.join(root, "hello.txt");

  await mkdir(path.join(directory, nestedDir), { recursive: true });
  await writeFile(path.join(directory, filePath), "venomcowork engine test\n", "utf8");
  ...
  assert.ok(read.content.includes("venomcowork engine test"));
