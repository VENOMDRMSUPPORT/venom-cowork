...
async function main() {
  ...
  try {
    await waitFor(client, "Boolean(window.__venomcoworkControl)", 15000);
...
async function runPreflight(client) {
  ...
  const controlReady = await evaluate(client, "Boolean(window.__venomcoworkControl)");
  const actions = controlReady
    ? await evaluate(client, "window.__venomcoworkControl.listActions().map((action) => action.id)")
    : [];
  ...
  if (!controlReady) throw new Error("VenomCowork control API is not available.");
...
async function ensureVoicePanel(client) {
  await evaluate(client, "window.__venomcoworkControl.setEnabled(true)");
  await evaluate(client, "window.localStorage.setItem('venomcowork.extension.enabled.venomcowork-voice', '1'); window.dispatchEvent(new CustomEvent('venomcowork:extension-state-changed', { detail: { id: 'venomcowork-voice', enabled: true } }))");
  let actions = await evaluate(client, "window.__venomcoworkControl.listActions().map((action) => action.id)");
  ...
  if (actions.includes("voice.panel.open")) {
    ...
    await waitFor(client, "window.__venomcoworkControl.listActions().some((action) => action.id === 'voice.inject_audio')", 8000);
    return;
  }
...
async function readProof(client) {
  return evaluate(client, `({
    href: location.href,
    narration: window.__venomcoworkControl?.snapshot?.().narration ?? "",
    route: window.__venomcoworkControl?.snapshot?.().route ?? "",
    bodyText: document.body.innerText.slice(-2400),
  })`);
}

async function executeControl(client, actionId, actionArgs = undefined) {
  const expression = `window.__venomcoworkControl.execute(${JSON.stringify(actionId)}, ${JSON.stringify(actionArgs)})`;
...
async function synthesizePcm16Base64(input) {
  ...
  const dir = await mkdtemp(join(tmpdir(), "venomcowork-voice-cdp-"));
...
async function pickTarget(baseUrl) {
  ...
  const target = pages.find((page) => page.title === "VenomCowork") ??
