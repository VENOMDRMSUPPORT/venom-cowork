...
function workspace(overrides: Partial<WorkspaceInfo> = {}): WorkspaceInfo {
  return {
    ...
    venomcoworkHostUrl: "https://worker.example.com/w/ws_remote",
    venomcoworkToken: "ow-token",
    ...overrides,
  };
}
...
describe("resolveRemoteWorkspaceConnectionTarget", () => {
  test("builds a host-scoped VenomCowork target from saved worker credentials", () => {
    const target = resolveRemoteWorkspaceConnectionTarget(
      workspace({
        venomcoworkHostUrl: "https://worker.example.com",
        venomcoworkWorkspaceId: "ws_remote",
      }),
    );
  ...
  test("does not run VenomCowork probes against non-VenomCowork remote workspaces", () => {
    const target = resolveRemoteWorkspaceConnectionTarget(
      workspace({
        remoteType: "opencode",
        venomcoworkHostUrl: "",
    ...
    if (target.ok) return;
    expect(target.state.status).toBe("error");
    expect(target.state.message).toContain("VenomCowork remote workers");
  });

  test("does not run VenomCowork probes against stale VenomCowork fields on non-VenomCowork remotes", () => {
    const target = resolveRemoteWorkspaceConnectionTarget(
      workspace({
        remoteType: "opencode",
        venomcoworkHostUrl: "https://worker.example.com/w/ws_remote",
        venomcoworkToken: "owt_secret",
        baseUrl: "https://opencode.example.com",
      }),
    );
...
describe("testRemoteWorkspaceConnection", () => {
  ...
  test("reports a missing token after proving the worker endpoint is reachable", async () => {
    const result = await testRemoteWorkspaceConnection(workspace({ venomcoworkToken: "" }), {
      createClient: () => client(),
    });
  ...
  test("uses fallback VenomCowork tokens saved on older workspace records", async () => {
    const result = await testRemoteWorkspaceConnection(
      workspace({
        venomcoworkToken: "",
        venomcoworkClientToken: "legacy-client-token",
      }),
      {
        createClient: (target) => {
...
describe("remote diagnostic identity", () => {
  ...
  test("changes when connection credentials change", () => {
    const before = getRemoteWorkspaceConnectionKey(workspace({ venomcoworkToken: "old-token" }));
    const after = getRemoteWorkspaceConnectionKey(workspace({ venomcoworkToken: "new-token" }));
