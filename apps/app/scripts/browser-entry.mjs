...
function hasBuiltInBrowserPrompt(haystack) {
  return (
    haystack.includes("built-in venomcowork browser") ||
    haystack.includes("venomcowork browser") ||
...
try {
  tmpdir = await mkdtemp(path.join(os.tmpdir(), "venomcowork-browser-entry-"));
      ...
      if (req.method === "POST" && url.pathname.endsWith("/chat/completions")) {
        ...
        if (triesBuiltInBrowser) {
          ...
          writeSse(
            res,
            createTextStream(
              "Using the built-in VenomCowork Browser to open example.com and read the page title.",
            ),
          );
        } else {
  ...
  await step("session.create", async () => {
    const session = await client.session.create({ title: "VenomCowork browser-entry test" });
  ...
  await step("assert.built-in-browser-quickstart", async () => {
    assert.equal(sawBuiltInBrowserPrompt, true, "Expected browser quickstart prompt to use the built-in VenomCowork Browser");
