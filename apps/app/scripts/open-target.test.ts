...
describe("deriveOpenTargets", () => {
  ...
  it("extracts PowerPoint decks from assistant artifact summaries", () => {
    const targets = deriveOpenTargets([
      message("msg_1", "assistant", "Updated file: decks/venomcowork-vertebrae-deck.pptx"),
    ]);
    const deck = targets.find((target) => target.value === "decks/venomcowork-vertebrae-deck.pptx");
  ...
  it("extracts artifact paths from VenomCowork extension call metadata", () => {
    const targets = deriveOpenTargets([
      toolMessage("msg_tool", "venomcowork_extension_call", {
