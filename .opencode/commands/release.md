---
description: Run the VenomCowork release flow
---

You are running the VenomCowork release flow in this repo.
...
7. If releasing venomcowork-orchestrator sidecars, build deterministically with `SOURCE_DATE_EPOCH`, upload assets to `venomcowork-orchestrator-vX.Y.Z`, and publish `venomcowork-orchestrator`.
8. If `venomcowork-server` or `opencode-router` versions changed, publish those packages.
