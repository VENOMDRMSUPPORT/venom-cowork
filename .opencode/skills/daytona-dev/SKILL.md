...
Launch the VenomCowork Electron app in a Daytona cloud sandbox. The real desktop
...
- `venomcowork-eval-secrets:/daytona-secrets` for provider keys and eval-only secrets.
- `venomcowork-eval-artifacts:/daytona-artifacts` for screenshots, validation notes, and recordings.
...
```bash
daytona preview-url venomcowork-dev -p 6080
```

Open that URL in your browser. You'll see the real Electron VenomCowork app.

### 4. Get other URLs

```bash
# Den Web dashboard (if Den stack is running)
daytona preview-url venomcowork-dev -p 3005

# CDP debugging endpoint
daytona preview-url venomcowork-dev -p 9825
```
...
| **CDP** | 9825 | Chrome DevTools Protocol â€” for automation |
...
2. Sign in normally (uses production `app.venomcowork.local`)
...
# SSH into sandbox
daytona ssh venomcowork-dev

# Check logs
daytona exec venomcowork-dev 'tail -50 /tmp/electron.log'
daytona exec venomcowork-dev 'tail -50 /tmp/vite.log'
daytona exec venomcowork-dev 'tail -50 /tmp/start-vnc.log'

# Inspect Electron CDP targets
daytona exec venomcowork-dev 'curl -s http://127.0.0.1:9825/json/list'

# Capture a persistent screenshot artifact
daytona exec venomcowork-dev 'bash .devcontainer/capture-daytona-screenshot.sh'

# Restart just the Electron app
daytona exec venomcowork-dev 'bash -lc "pkill -f electron || true; pkill -f electron-dev || true"'
daytona exec venomcowork-dev 'bash -lc "cd /workspace && bash /opt/venomcowork-daytona/start-daytona-electron.sh --detach"'

# Stop the sandbox (preserves state)
daytona stop venomcowork-dev

# Start it again
daytona start venomcowork-dev

# Delete (destroys everything)
daytona delete venomcowork-dev
```
...
```bash
cd /workspace/apps/app && VENOMCOWORK_DEV_MODE=1 nohup npx vite --host 0.0.0.0 --port 5173 > /tmp/vite.log 2>&1 &
```

**noVNC shows black screen:**
Xvfb/XFCE may have crashed. Restart the desktop stack:
```bash
bash /opt/venomcowork-daytona/start-daytona-vnc.sh
```

**"no space left on device" when creating sandbox:**
Use `--disk 10`. The default Daytona disk can be 3 GB, which is not enough for
VenomCowork dependencies and sidecar prep. Also don't use `--context .` â€” it
