...
description: Local VenomCowork Electron browser automation with CDP. Use when driving a local Electron dev app, browser_list, browser_snapshot, browser_eval, composer automation, or local UI smoke tests.
---
...
- Attach OpenCode browser tools to the VenomCowork Electron app during local development.
...
```sh
VENOMCOWORK_ELECTRON_REMOTE_DEBUG_PORT=${VENOMCOWORK_ELECTRON_REMOTE_DEBUG_PORT:-9823}
```
...
```sh
VENOMCOWORK_ELECTRON_REMOTE_DEBUG_PORT=9830 pnpm dev
```

To disable Electron CDP for a run:

```sh
VENOMCOWORK_ELECTRON_REMOTE_DEBUG_PORT=0 pnpm dev
```
...
```sh
nohup pnpm dev > /var/folders/d9/xqhkvsp94rg0n0n523snqztm0000gn/T/opencode/venomcowork-dev.log 2>&1 &
```
...
2. Select the `VenomCowork` target ID.
...
Use this `browser_eval` pattern after selecting the VenomCowork target:
...
- Electron CDP is used for development test tooling. User browser tasks should use the built-in VenomCowork Browser target.
- A successful local attach should show an `VenomCowork` target at `http://127.0.0.1:9823`.
