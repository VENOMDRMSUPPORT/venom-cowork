...
Drive the real VenomCowork Electron app inside a Daytona sandbox via CDP browser
...
sandbox from the reusable `venomcowork-eval-vnc` snapshot when present, checks out
...
Refresh the snapshot with `bash .devcontainer/create-daytona-venomcowork-snapshot.sh`
when dependencies or base setup change. The snapshot excludes `node_modules`;
dependency installs reuse the `venomcowork-eval-pnpm-store` volume.
...
sandboxes mount `venomcowork-eval-secrets:/daytona-secrets` automatically and
...
- `daytona-secrets-volume`: provider keys and eval-only secrets in
  `venomcowork-eval-secrets:/daytona-secrets`.
...
- **Secrets volume:** use `venomcowork-eval-secrets:/daytona-secrets` for provider
...
- **Artifacts volume:** use `venomcowork-eval-artifacts:/daytona-artifacts` for
...
frame-by-frame HTML â€” named PNGs in a browseable index served on port 8090.
...
Should show: `[target_id] VenomCowork  http://localhost:5173/#/welcome`
...
5. **Wait 10-12s.** Verify:
   ...
   - Status bar shows "VenomCowork Ready"
...
```bash
# Minimize
daytona exec "$SANDBOX" -- "bash -lc 'DISPLAY=:99 xdotool search --name VenomCowork windowminimize'"

# Restore
daytona exec "$SANDBOX" -- "bash -lc 'DISPLAY=:99 xdotool search --name VenomCowork windowactivate'"
```
...
Every Daytona eval sandbox mounts `venomcowork-eval-secrets:/daytona-secrets` and
`/opt/venomcowork-daytona/start-daytona-electron.sh` sources every
...
sleep 3
daytona exec "$SANDBOX" -- "bash -lc 'cd /workspace && bash /opt/venomcowork-daytona/start-daytona-electron.sh --detach'"
```
...
`@venom-cowork/email` must be built before the seed imports Den email helpers:
```bash
daytona exec <server-sandbox> -- 'cd /workspace && pnpm --filter @venom-cowork/email build && cd /workspace/ee/apps/den-api && VENOMCOWORK_DEV_MODE=1 DATABASE_URL=mysql://root:password@127.0.0.1:3306/venomcowork_den DEN_DB_ENCRYPTION_KEY=daytona-den-db-encryption-key-please-change-1234567890 BETTER_AUTH_SECRET=local-dev-secret-not-for-production-use!! BETTER_AUTH_URL=http://localhost:3005 pnpm exec tsx scripts/seed-demo-org.ts --reset'
```
...
grant from the Den API, paste the `venomcowork://den-auth?...` URL into Cloud
Account -> `Paste sign-in code`, and choose `Acme Robotics`:
```bash
TOKEN=$(curl -s -X POST '<DEN_API_URL>/api/auth/sign-in/email' -H 'content-type: application/json' --data '{"email":"alex@acme.test","password":"VenomCoworkDemo123!"}' | node -e 'let s="";process.stdin.on("data",c=>s+=c);process.stdin.on("end",()=>process.stdout.write(JSON.parse(s).token))')
curl -s -X POST '<DEN_API_URL>/v1/auth/desktop-handoff' -H "authorization: Bearer $TOKEN" -H 'content-type: application/json' --data '{"desktopScheme":"venomcowork"}'
```
...
you also see `DevTools listening on ws://127.0.0.1:9825/...` and an VenomCowork
window in noVNC.
...
The app log line `[venomcowork] Electron CDP exposed at http://127.0.0.1:9825`
means VenomCowork requested CDP. The real success marker is Chromium's own line:
...
recordings are saved to the persistent `venomcowork-eval-artifacts` volume and
...
Save the sandbox name from the output (e.g. `SANDBOX=venomcowork-test-20260601-165424`).
...
```js
// In browser_eval:
const raw = localStorage.getItem("venomcowork.preferences");
const prefs = raw ? JSON.parse(raw) : {};
prefs.hasCompletedOnboarding = false;
localStorage.setItem("venomcowork.preferences", JSON.stringify(prefs));
location.reload();
```
...
- Recordings are stored on the `venomcowork-eval-artifacts` Daytona volume (5 GB,
...
- The `start-daytona-recording.sh` script records to a temp file first, then
  copies to the artifacts volume on stop â€” this avoids NFS write issues.
