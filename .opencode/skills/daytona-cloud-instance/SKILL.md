...
description: Daytona cloud instance, Den server, VenomCowork Cloud, Marketplace onboarding. Use when the user asks to run, launch, start, validate, or record a Daytona cloud/Den instance for VenomCowork Cloud flows.
---

# Daytona Cloud Instance

Use this skill to launch or validate an VenomCowork Cloud/Den Daytona server sandbox and collect useful URLs for desktop sign-in and Marketplace onboarding demos.
...
curl -fsS -X POST "$DEN_API_URL/api/auth/sign-in/email" \
  ...
  --data '{"email":"alex@acme.test","password":"VenomCoworkDemo123!"}'
```
...
```bash
daytona exec "$SERVER_SANDBOX" -- 'bash -lc '\''cd /workspace && pnpm --filter @venom-cowork-ee/den-web build'\'''
daytona exec "$SERVER_SANDBOX" -- 'bash -lc '\''pkill -f "next dev --hostname 0.0.0.0 --port 3005" || true; pkill -f "next-server" || true'\'''
daytona exec "$SERVER_SANDBOX" -- 'bash -lc '\''cd /workspace && nohup pnpm --filter @venom-cowork-ee/den-web exec next start --hostname 0.0.0.0 --port 3005 > /tmp/den-web-prod.log 2>&1 &'\'''
```
...
```js
localStorage.setItem('venomcowork.den.baseUrl', DEN_WEB_URL)
localStorage.setItem('venomcowork.den.apiBaseUrl', DEN_API_URL)
localStorage.setItem('venomcowork.den.authToken', TOKEN)
localStorage.setItem('venomcowork.den.activeOrgId', ORG_ID)
localStorage.setItem('venomcowork.den.activeOrgSlug', ORG_SLUG)
localStorage.setItem('venomcowork.den.activeOrgName', ORG_NAME)
window.dispatchEvent(new CustomEvent('venomcowork-den-settings-changed'))
window.dispatchEvent(new CustomEvent('venomcowork-den-session-updated', { detail: { token: TOKEN } }))
```
...
5. Desktop signed-out Marketplace nudge says VenomCowork works without an account.
6. Desktop signs in to VenomCowork Cloud.
7. Marketplace refresh shows `VenomCowork Marketplace` and org marketplaces.
