...
Prefer synthetic paste for the VenomCowork composer:
...
- The target VenomCowork/Chrome window is visible and not covered by a native file
...
right window first with `wmctrl -a "VenomCowork - Dev"` or
`wmctrl -a "VenomCowork Cloud - Chromium"` so the screenshot shows the intended
journey step.
