---
name: run-evals
description: Run VenomCowork UI evals on a Daytona sandbox or local Electron instance. Handles sandbox creation, service startup, and eval execution via CDP browser tools.
---

# Skill: Run Evals

Run the VenomCowork UI evaluation flows against a real Electron app. Prefer a fresh Daytona sandbox for each run, with a local test fallback when Daytona is unavailable.
...
- Optional provider coverage: reusable Daytona volume `venomcowork-eval-secrets`
...
`venomcowork-eval-vnc` snapshot, mounts the reusable
`venomcowork-eval-secrets:/daytona-secrets` volume, mounts the reusable
`venomcowork-eval-pnpm-store` pnpm cache volume, starts XFCE/noVNC, Vite, and
...
â†’ should show "VenomCowork" page target
```
...
VenomCowork's `Electron CDP exposed` line.
...
2. Follow the workspace creation flow from `evals/daytona-flows.md` Flow 1:
   - Click "Get started" â†’ "Local workspace"
...
Use the `__reactFiber$` â†’ `CreateWorkspaceModal` reducer dispatch with `{ key: "selectedFolder", value: "/path" }`. Full code in `evals/daytona-flows.md` Flow 1 Step 5.
...
```bash
pnpm install
pnpm --filter @venom-cowork/app typecheck
pnpm --filter @venom-cowork/app build
```
