# Default VenomCowork Marketplace onboarding flow
...
Marketplace containing built-in VenomCowork extension capabilities.

## Acceptance target

- Den download/sign-in copy does not claim that Den creates or loads a desktop workspace.
- Den introduces the built-in VenomCowork Marketplace and explains it appears after desktop sign-in.
- A signed-in desktop user can open Settings -> Marketplace and see `VenomCowork Marketplace` from Den.
- `VenomCowork Marketplace` includes built-in extension entries such as `VenomCowork Browser`, `Computer Use`, `OpenAI Image Gen`, `Google Workspace`, and `Ollama`.
- Built-in VenomCowork entries are rendered as cloud Marketplace entries with a `Built-in` status, not as locally injected desktop Marketplace rows.
- Signed-out desktop users can still use VenomCowork, but Marketplace nudges sign-in for built-in extensions and organization marketplaces.
...
8. Validate dev auth before UI recording: direct Den API sign-in with the seeded user should return `200` when `VENOMCOWORK_DEV_MODE=1`.
...
- Page includes `built-in VenomCowork Marketplace` or `built-in Marketplace`.
...
**Goal:** VenomCowork remains usable without account, while Marketplace clearly asks
for Cloud sign-in.
...
- The notice says the user can use VenomCowork without an account.
...
**Goal:** Desktop sign-in causes Den to provision and return the default VenomCowork
Marketplace for any user/org.

Steps:

1. Sign Electron into VenomCowork Cloud using the Daytona Den handoff flow.
2. Create or open a local workspace.
3. Open Settings -> Marketplace.
4. Click `Refresh` if the Marketplace list has not loaded yet.
5. Open `Filters` and inspect marketplace options.
6. Search for `VenomCowork Browser`.
7. Open the `VenomCowork Browser` card.

Expected outcome:

- `VenomCowork Marketplace` appears as a marketplace option.
- `VenomCowork Browser` appears as a Marketplace card from Den.
- The card shows `Built-in` or an equivalent built-in/ready status.
- The detail modal shows VenomCowork Browser setup/resource details from the Den extension manifest.
- The detail modal does not offer `Add` or `Remove` for built-in VenomCowork entries.
...
2. Find `VenomCowork Marketplace`.
3. Call `GET /v1/marketplaces/:id/resolved`.

Expected outcome:

- The API returns `VenomCowork Marketplace`.
- Resolved plugins include the built-in VenomCowork entries.
- Each built-in plugin has `extension.sourceFormat = venomcowork-builtin`.
- `VenomCowork Browser` has an extension manifest with the `opencode-chrome-devtools` resource.
...
2. Den explains in plain language: Marketplaces contain plugins; VenomCowork Marketplace is included; org marketplaces appear after desktop sign-in.
...
6. Marketplace is pre-populated without manual refresh, with `VenomCowork Marketplace` grouped separately from org marketplaces.
...
stores and returns VenomCowork extension manifests; the desktop still owns local
