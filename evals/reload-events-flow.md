...
VenomCowork only asks the user to reload when reload-relevant files change while the
app is already running.

## Why

VenomCowork bootstraps workspace files such as `opencode.jsonc`, `.opencode/agents`,
and legacy command frontmatter. Those internal startup writes should refresh the
VenomCowork/OpenCode baseline silently. The user-facing **Reload required** toast is
reserved for real runtime edits to OpenCode config, MCP, skills, agents,
commands, or plugins.

## Flow 1 â€” App boot does not show stale reload toast
...
## Flow 2 â€” Internal bootstrap writes are silent

Steps:
1. Create a temporary local workspace with no `.opencode` directory.
2. Add it through the VenomCowork workspace picker.
3. Wait until the session route is ready.
4. Wait 6 seconds for reload-event polling to run.

Pass criteria:
- VenomCowork creates the default project `opencode.jsonc` and `.opencode/agents/venomcowork.md`.
...
## Flow 3 â€” No-op rewrites do not prompt reload
...
## Flow 4 â€” Runtime config edits still prompt reload
...
## Flow 5 â€” Hidden project config is watched

Steps:
1. Use a workspace whose active project config lives at `.opencode/opencode.jsonc`.
2. Start VenomCowork and wait until the session route is ready.
...
- Raw filesystem notifications show a reload toast even when file content did
  not change.
- Startup-created VenomCowork agent/config files leak into the user-facing reload
  event stream.
