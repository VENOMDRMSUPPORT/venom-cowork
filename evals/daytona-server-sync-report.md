...
Date: 2026-05-21
PR: https://github.com/venom-cowork/workspace/pull/1887

## Topology

- Server sandbox: `venomcowork-server-20260521-195504`
- Electron sandbox: `venomcowork-test-20260521-200726`
...
- Pasted the full `venomcowork://den-auth?...` link into the desktop Cloud Account manual sign-in flow.
- Electron showed `VenomCowork Cloud Connected` and `Signed in as daytona-proof-1779419514@example.com`.
- Den API logs showed:
...
- Not fixed in this PR: raw manual sign-in grant paste does not reliably target the Den API in split Den Web/Den API deployments. Use the full `venomcowork://den-auth?...&denBaseUrl=...` handoff link for now.
...
- `bash -n .devcontainer/create-daytona-venomcowork-server-snapshot.sh`
