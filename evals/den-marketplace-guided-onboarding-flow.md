...
handoff explicit: install VenomCowork Desktop, sign in, then access organization
marketplaces from the app.

## Preconditions

- Run Den Web and Den API from a fresh Daytona server sandbox or a clean local DB.
- Use `VENOMCOWORK_DEV_MODE=1` for local/Daytona email-password sign-up without email delivery.
...
- `VenomCowork Marketplace`
  - Description explains built-in VenomCowork AI capabilities available in desktop after sign-in.
...
6. Confirm the onboarding screen includes:
   ...
   - Desktop guidance: download VenomCowork Desktop, sign in with the same account, then open Marketplace.
   - A visible `VenomCowork Marketplace` card.
   - A visible `Anthropic-Compatible Plugins` card.
   - A link or visible reference to `anthropics/knowledge-work-plugins`.
   - VenomCowork MCP install config using `npx -y venomcowork-ui-mcp`.
...
1. Start VenomCowork Desktop in dev mode pointed at the fresh Den Web/API URLs.
2. Open Settings -> Extensions -> Marketplace while signed out.
3. Confirm signed-out copy says VenomCowork is usable without an account and sign-in unlocks Marketplace/built-ins/org marketplaces.
4. Sign in to VenomCowork Cloud with the same Den account.
5. Return to Marketplace and refresh if needed.
6. Confirm:
   - `VenomCowork Marketplace` appears as a marketplace source/filter.
...
- Den sandbox: `venomcowork-server-onboarding-20260603-2108`.
- Electron sandbox: `venomcowork-test-20260603-211949`.
...
- Desktop Marketplace showed `VenomCowork Marketplace` and `Anthropic-Compatible Plugins`.
...
- Marketplace list shows only `VenomCowork Marketplace`.
