...
End-to-end user flows for signing the desktop app into VenomCowork Cloud through a
...
3. From Den Web, start a desktop handoff or create one through Den API:
   ...
   curl -H "authorization: Bearer $TOKEN" \
     ...
     --data '{"desktopScheme":"venomcowork"}' \
     "$DEN_API_URL/v1/auth/desktop-handoff"
   ```
4. Paste the full `venomcowork://den-auth?...&denBaseUrl=...` link into `Paste sign-in code`.
...
- `localStorage.venomcowork.den.authToken` is present.
...
- `localStorage.venomcowork.den.authToken` remains empty.
