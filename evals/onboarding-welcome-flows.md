...
- Friction notes written in product language, for example: â€œuser leaves chat for settings,â€ â€œreload required before value,â€ or â€œpayment/sign-in appears before value is demonstrated.â€
...
```text
Create a short welcome checklist for this VenomCowork workspace. Use exactly three bullets and mention one thing I can do next.
```
...
- The task is submitted from the composer with the normal â€œRun taskâ€ action.
...
## Flow 20 â€” Welcome screen renders on first launch

**Why**: When a user opens VenomCowork for the first time with zero
workspaces and `hasCompletedOnboarding === false`, they must see the
full-screen welcome page â€” not the session empty state.
...
4. Expect: full-screen page renders with:
   - "Welcome to VenomCowork" heading.
...
Pass criteria:
- URL is `/welcome`.
- Heading "Welcome to VenomCowork" is visible.
...
## Flow 21 â€” Welcome CTA starts workspace creation
...
## Flow 22 â€” Local workspace creation from welcome flow
...
3. Expect: the local panel shows:
   ...
   - Explanation text: "This folder becomes your workspace. VenomCowork
     will be able to:"
...
## Flow 23 â€” Folder explanation visible in local workspace panel

**Why**: The folder explanation must appear every time the local
workspace panel opens â€” not just from the welcome flow. Users creating
...
## Flow 24 â€” Remote workspace creation from welcome flow

**Why**: Users connecting to a remote VenomCowork server from the welcome
flow should also have onboarding marked complete.

Steps:
1. From `/welcome`, click "Get started".
2. Click "Connect custom remote".
3. Enter a valid VenomCowork server URL.
...
## Flow 25 â€” Welcome screen skipped when workspaces exist

**Why**: If a user already has workspaces, the welcome screen must
never appear â€” even if `hasCompletedOnboarding` is false (e.g., after
...
## Flow 26 â€” Reset onboarding restores welcome screen
...
## Flow 27 â€” Bring your own API key reaches first task value
...
3. On â€œPower your first task,â€ choose â€œBring your own API key.â€
...
## Flow 28 â€” VenomCowork Models path explains payment before value

**Why**: VenomCowork Models is the preferred business path, but it must make the tradeoff clear: pay/sign in through VenomCowork Cloud to skip API keys, then return to task execution.

Steps:
1. Start from a clean first-run app at `/welcome` with no workspaces.
2. Create a local workspace through the welcome CTA.
3. On â€œPower your first task,â€ choose â€œUse VenomCowork Models.â€
4. Expect: the user sees the VenomCowork Models value proposition and sign-in/payment CTA.
5. If a paid/signed-in test account is available, complete sign-in, select an VenomCowork model, and run the recommended first task prompt.
6. If no paid/signed-in test account is available, record the exact stop point and mark the run as â€œfunnel blocked before task value.â€
...
## Flow 29 â€” Ollama local model reaches first task value
...
1. Start from a clean first-run app at `/welcome` with no workspaces.
2. Create a local workspace through the welcome CTA.
3. Choose â€œSkip and use the free modelâ€ or enter the workspace, then open Extensions.
4. Open the Ollama setup card.
5. Expect: the UI explains whether Ollama is running and lists available local models.
6. Select an available model and click â€œAdd to workspace.â€
7. If a reload is required, click â€œReload nowâ€ and include that in the friction metrics.
8. Confirm AI Providers shows â€œOllama (local).â€
...
- 2026-04-29 â€” initial doc for the onboarding welcome feature
  (Flows 20-26).
- 2026-06-03 â€” added founder/designer value-flow criteria and first task
