...
1. Start the dev app (CDP auto-exposes on port 9223):
   ```bash
   pnpm --filter @venom-cowork/desktop dev
   ```
2. Wait ~20s, then verify CDP:
   ```
   browser_list({ browser_url: "http://127.0.0.1:9223" })
   ```
   You should see the VenomCowork target.
3. Enable control mode:
   ```
   browser_eval({ browser_url: CDP_URL, target_id: APP_TARGET,
     expression: "window.__venomcoworkControl.setEnabled(true); 'ok'" })
   ```

## Flow 1 â€” Plugin is loaded and browser tools exist
...
- No stale MCP keys (`venomcowork-browser`, `chrome`, `chrome-devtools`,
  `control-chrome`) exist in the `mcp` section.

Known regressions this catches:
- Plugin not declared in workspace config â†’ browser tools unavailable.
- Stale MCP entries from pre-extension architecture â†’ noisy connection errors.

## Flow 2 â€” Built-in browser navigates via correct tools
...
1. Create a new session:
   ```
   browser_eval({ browser_url: CDP_URL, target_id: APP_TARGET,
     expression: "window.__venomcoworkControl.execute('session.create_task')" })
   ```
2. Type and send the prompt:
   ```
   browser_eval({ browser_url: CDP_URL, target_id: APP_TARGET,
     expression: `(async () => {
       const ctrl = window.__venomcoworkControl;
       await ctrl.execute('composer.set_text', { text: 'Use the VenomCowork Browser extension to navigate to https://example.com and tell me the page title' });
...
## Flow 3 â€” Browser interaction (navigate + fill + snapshot)
...
Steps:
1. Create a new session.
2. Send: "Use the VenomCowork Browser extension to go to https://www.google.com,
...
## Flow 4 â€” Composer Extensions menu shows all extensions
...
Pass criteria:
- "VenomCowork Browser" is visible with a name and description.
- "Chrome" is not visible as an VenomCowork extension.

Known regressions this catches:
- Extension catalog not loaded.
- `isVenomCoworkExtensionEnabled` filtering incorrectly.

## Flow 5 â€” Extension chip inserts composerPrompt

**Why**: Clicking an extension in the menu must inject its `composerPrompt`
into the composer text.

Steps:
1. With the Extensions menu open, click the "VenomCowork Browser" entry.
2. Check the composer text.

Pass criteria:
- Composer contains "Use the VenomCowork Browser extension to".
...
## Flow 6 â€” Extension toggle hides from composer

**Why**: Disabling an extension via the Extensions panel must remove it from
the composer menu.

Steps:
1. Disable VenomCowork Browser:
   ```
   browser_eval({ browser_url: CDP_URL, target_id: APP_TARGET,
     expression: `(() => {
       localStorage.setItem('venomcowork.extension.disabled.venomcowork-browser', '1');
       window.dispatchEvent(new CustomEvent('venomcowork:extension-state-changed', {
         detail: { id: 'venomcowork-browser', enabled: false }
       }));
       return 'disabled';
     })()` })
   ```
...
3. Re-enable:
   ...
   browser_eval({ browser_url: CDP_URL, target_id: APP_TARGET,
     expression: `(() => {
       localStorage.removeItem('venomcowork.extension.disabled.venomcowork-browser');
       window.dispatchEvent(new CustomEvent('venomcowork:extension-state-changed', {
         detail: { id: 'venomcowork-browser', enabled: true }
       }));
       return 'enabled';
     })()` })
   ```
...
Pass criteria:
- After disabling: VenomCowork Browser is NOT in the Extensions menu.
- After re-enabling: VenomCowork Browser IS in the Extensions menu.
- `localStorage` key `venomcowork.extension.disabled.venomcowork-browser` is `"1"`
...
## Flow 7 â€” Stale MCP migration

**Why**: Workspaces from the pre-extension architecture have dead MCP entries
(`venomcowork-browser`, `chrome`) that must be cleaned up on activation.
...
1. Write a stale config to the workspace `opencode.jsonc`:
   ```json
   {
     ...
     "default_agent": "venomcowork",
     "mcp": {
       "venomcowork-browser": { "type": "remote", "url": "http://127.0.0.1:59674/mcp" },
       "chrome": { "type": "remote", "url": "http://127.0.0.1:59675/mcp" },
       "venomcowork-ui": { "type": "remote", "url": "http://127.0.0.1:59673/mcp" }
     }
   }
   ```
...
Pass criteria:
- `venomcowork-browser` MCP entry removed.
- `chrome` MCP entry removed.
- `venomcowork-ui` MCP entry preserved (not a legacy browser MCP).
- `default_agent: "venomcowork"` preserved.
...
## Flow 8 â€” Hidden extensions stay out of normal UI and composer
...
2. Open the "VenomCowork Browser" detail modal and click `Hide`.
3. Confirm "VenomCowork Browser" disappears from the normal Extensions catalog.
4. Open the composer tool menu -> Extensions.
5. Confirm "VenomCowork Browser" is not listed.
6. Return to Settings -> Extensions and click `Show hidden`.
7. Confirm "VenomCowork Browser" reappears with a hidden badge.
8. Open its detail modal and click `Show`.

Pass criteria:
- Hidden state is persisted in localStorage under
  `venomcowork.extension.hidden.venomcowork-browser`.
...
## Flow 9 â€” Cloud marketplace appears in Extensions Marketplace
...
Steps:
1. Sign in to VenomCowork Cloud with an org that has a marketplace plugin.
