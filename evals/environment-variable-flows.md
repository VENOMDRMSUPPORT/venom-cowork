...
3. If possible, point the environment store at an eval-only file before launch,
   for example `/tmp/venomcowork-env-eval.json`, so file verification cannot touch
...
3. Try to add `VENOMCOWORK_TOKEN=value`.
...
- Reserved `VENOMCOWORK_`/`OPENCODE_` names are blocked.
