...
1. Start the Electron dev app from the repo root:
   ```bash
   VENOMCOWORK_ELECTRON_REMOTE_DEBUG_PORT=9823 pnpm dev:electron
   ```
2. Attach browser tools to `http://127.0.0.1:9823` and select the VenomCowork target.
...
## Flow 1 â€” Inline Chips And Command Palette
...
## Flow 2 â€” Open Browser Target

Steps:

1. In Cmd/Ctrl+K â†’ `Accessible items`, choose the app-root `http://localhost:<port>` row.
...
## Flow 3 â€” Open Artifact Targets

Steps:

1. In Cmd/Ctrl+K â†’ `Accessible items`, choose `openable-items-eval.csv`.
2. Capture a screenshot of the artifact pane.
3. Choose `openable-items-eval.md` from Cmd/Ctrl+K â†’ `Accessible items`.
...
## Flow 4 â€” Stop Tracking

Steps:

1. Open Cmd/Ctrl+K â†’ `Accessible items`.
2. Choose `Stop tracking openable-items-eval.md`.
3. Open Cmd/Ctrl+K â†’ `Accessible items` again and capture a screenshot.
4. Reload the app/session and open Cmd/Ctrl+K â†’ `Accessible items` again.
