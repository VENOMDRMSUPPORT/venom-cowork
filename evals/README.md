# VenomCowork UI evals

Human-readable scenarios that an LLM (or a person) can replay against a live
VenomCowork instance to verify end-to-end behavior of the UI.
...
(OpenCode + VenomCowork server + React UI) so regressions in wiring â€” not just
types â€” get caught.
...
- Use `browser_eval` for button clicks and text input â€” it's more reliable
...
- [`cloud-admin-to-member-assignment-flows.md`](./cloud-admin-to-member-assignment-flows.md)
  â€” admin assigns providers/policies to a member, member desktop receives and
...
- [`cloud-signin-client-provisioning-funnel.md`](./cloud-signin-client-provisioning-funnel.md)
  â€” founder funnel from website sign-in to provisioning skills/plugins/providers
  and validating the capability appears and produces value in the desktop client.
- [`workspace-layout-state-flows.md`](./workspace-layout-state-flows.md) â€”
  persisted sidebar/browser layout, legacy layout migration, and workspace-safe
  layout state.
- [`environment-variable-flows.md`](./environment-variable-flows.md) â€” local
