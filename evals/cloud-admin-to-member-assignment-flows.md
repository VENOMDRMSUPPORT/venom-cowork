...
- If Electron was already started with stale Den preview URLs, restart Electron
  ...
  daytona exec <electron-sandbox> -- \
    "bash -lc 'cd /workspace && \
      VENOMCOWORK_DEN_BASE_URL=DEN_WEB_URL \
      VENOMCOWORK_DEN_API_BASE_URL=DEN_API_URL \
...
- The desktop member state can be bootstrapped for CDP-driven evals by setting:
  `venomcowork.den.baseUrl`, `venomcowork.den.apiBaseUrl`, `venomcowork.den.authToken`,
  `venomcowork.den.activeOrgId`, `venomcowork.den.activeOrgSlug`, and
  `venomcowork.den.activeOrgName` in localStorage, then reloading the renderer.
...
- Before assignment removal, Settings -> AI Providers showed:
  ...
  - `Cloud`.
  - `Imported`.
  - `openai Â· Credential ready Â· 1 models Â· managed provider`.
