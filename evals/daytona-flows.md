...
End-to-end scenarios that run against a real Electron VenomCowork instance in a
...
Use the helper. It creates from the reusable `venomcowork-eval-vnc` snapshot,
...
`bash .devcontainer/create-daytona-venomcowork-snapshot.sh`.

The reusable `venomcowork-eval-secrets` volume is mounted at `/daytona-secrets`.
...
the reusable `venomcowork-eval-artifacts` volume at `/daytona-artifacts`, starts a
...
Should return the VenomCowork page target.

### 4. Verify opencode sidecar

```bash
daytona exec venomcowork-test 'ps aux | grep opencode | grep -v grep'
```
...
1. Create the workspace directory on the sandbox:
   ```bash
   daytona exec venomcowork-test 'mkdir -p /workspace/hello'
   ```
...
9. Verify navigation to session page:
   ```
   browser_eval({ browser_url: CDP_URL, expression: "window.location.hash" })
   â†’ should contain "/session"
   ```

10. Verify opencode sidecar started:
    ```bash
    daytona exec venomcowork-test 'ps aux | grep opencode | grep -v grep'
    â†’ should show opencode serve process
    ```
...
7. Verify agent response appeared:
   ...
   â†’ should contain the agent's response about directory contents
   ```
...
1. Start the reusable mock OAuth MCP server in the Daytona sandbox:
   ```bash
   daytona exec venomcowork-test 'bash -lc "cd /workspace && nohup env PORT=3978 HOST=127.0.0.1 AUTO_APPROVE=1 node scripts/mock-oauth-mcp-server.mjs > /tmp/mock-mcp.log 2>&1 &"'
   ```
...
2. Verify the mock server is healthy:
   ```bash
   daytona exec venomcowork-test 'bash -lc "curl -s http://127.0.0.1:3978/health"'
   ```
...
8. Wait up to 30s, then verify the mock OAuth server saw an authorization
   ...
   ```bash
   daytona exec venomcowork-test 'bash -lc "curl -s http://127.0.0.1:3978/requests"'
   ```
...
5. Create or sign in to a Den Web account, then use the desktop handoff code or
   full `venomcowork://den-auth?...` link in `Paste sign-in code`.
...
- Server sandbox: `venomcowork-server-20260602-154721`
- Electron sandbox: `venomcowork-test-20260602-155000`
...
3. Restart Electron without local AI-provider secrets so the baseline has no
   ...
   ```bash
   DAYTONA_SECRETS_ENV=/tmp/no-daytona-secrets bash /opt/venomcowork-daytona/start-daytona-electron.sh --detach
   ```
...
6. Local file checks:
   - Confirm VenomCowork-owned cloud import metadata is stored in the VenomCowork
     runtime DB, not `.opencode/venomcowork.json`.
...
```bash
daytona stop venomcowork-test    # preserves state
daytona delete venomcowork-test  # destroys everything
```
...
```bash
daytona exec venomcowork-test 'bash -lc "pkill -f electron || true; pkill -f electron-dev || true"'
sleep 3
daytona exec venomcowork-test 'bash -lc "cd /workspace && bash /opt/venomcowork-daytona/start-daytona-electron.sh --detach"'
```

`[venomcowork] Electron CDP exposed...` only means VenomCowork requested CDP. The real
