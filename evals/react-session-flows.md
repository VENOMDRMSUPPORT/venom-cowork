...
- VenomCowork server proxy endpoints for `/w/:workspaceId/opencode/session/**`

## Preflight

Before running any eval:

1. Install dependencies in the VenomCowork repo:
   ```bash
   pnpm install --frozen-lockfile
   ```
2. Start the Electron dev app:
   ```bash
   nohup pnpm dev:electron > .venomcowork-run/electron.log 2>&1 &
   ```
   Wait ~15s for Vite + Electron to boot. The log will show
   `[venomcowork] Electron CDP exposed at http://127.0.0.1:9823` when ready.
3. Chrome MCP automatically connects to the Electron renderer via CDP on
   port 9823. Verify by taking a snapshot:
   ```
   chrome-devtools_take_snapshot
   ```
   You should see the full VenomCowork UI tree (sidebar, composer, footer).
4. Confirm the footer shows **"VenomCowork Ready"**.
...
inspection, and console messages â€” without needing Docker or a separate
...
## Flow 1 â€” Send a message and observe streaming
...
Steps:
1. Hover the workspace header in the sidebar â†’ click **New task**.
...
5. Expect: within ~1s, the user bubble appears in the transcript; within
   ~3â€“5s the assistant bubble appears and text progressively fills in.
...
- Status bar transitions `Running...` â†’ `Ready`.
...
## Flow 1A â€” Artifact pane opens generated spreadsheet/markdown targets
...
1. Run this once in an existing workspace, then create a new workspace and run
   it again there. The default VenomCowork agent should include artifact guidance
   in both cases.
2. Hover the workspace header in the sidebar â†’ click **New task**.
3. Fill the composer:
...
- The artifact pane only auto-opens a file after the VenomCowork server confirms
...
- Markdown artifacts open directly in the text editor and can save through the
  VenomCowork server write API without switching into a separate edit view.
- CSV/XLSX/Markdown artifacts expose a **Download artifact** action backed by
  the VenomCowork server, so it works for local and remote workspaces.
...
## Flow 1B â€” Local preview URLs and socket hints open through browser mode
...
## Flow 3 â€” Remove / Flow 8 â€” Delete a session

Steps:
1. Select the session you want to delete in the sidebar.
2. Click the **Session actions** (overflow `â€¦`) button on the selected row.
...
Known regressions this catches:
- `onDeleteSession` not wired â†’ menu has no Delete entry.
- Missing server-side `client.deleteSession(workspaceId, sessionId)` call.
- Sidebar not refreshed after delete.

---

## Flow 4 â€” Rename a session

Steps:
1. Select a session.
2. Click **Session actions** â†’ **Rename session**.
...
Known regressions this catches:
- `onRenameSession` not wired â†’ menu has no Rename entry.
...
## Flow 5 â€” Open Connect Providers modal
...
Known regressions this catches:
- Provider auth store never initialized â†’ modal empty or stuck on spinner.
- Infinite render loop making the modal immediately close itself.

---

## Flow 6 â€” Select a new default model
...
- Infinite loop caused by `refreshProviders()` inside a `useEffect` whose
  ...
  `84286ebe fix(react-app/settings): break infinite loopâ€¦`).

---

## Flow 7 â€” Toggle thinking mode (Show model reasoning)
...
Pass criteria:
- Button state flips (Off â†’ On or On â†’ Off) instantly without a spinner.
...
## Flow 9 â€” Navigate every settings tab, close, then create a session
...
2. Click each workspace tab in order:
   `Settings â†’ Skills â†’ Extensions â†’ Messaging â†’ Advanced`.
3. Click each global tab in order:
   `Cloud â†’ Appearance â†’ Updates â†’ Recovery`.
4. Click the **X Close settings** button in the header.
5. Back on `/session`, hover workspace â†’ click **New task**.
...
Known regressions this catches:
- Infinite render loop â€” the URL updates but body stays on General.
...
## Flow 10 â€” Keyboard: Cmd+N creates a new session
...
## Flow 11 â€” Command palette (Cmd+K)

**Why**: The palette is the cross-surface quick switcher â€” create sessions,
...
2. Expect: overlay appears with "Create new session", "Search sessions",
   ...
   "Arrow keys to navigate Â· Enter to run Â· Esc to close".
...
- "Create new session" â†’ creates a session in the selected workspace and
  closes the palette.
- "Search sessions" row â†’ swaps to sessions mode.
- "Settings" row â†’ navigates to `/settings/general` and closes.
...
## Flow 12 â€” Workspace options menu (Edit name / Share / Reveal / Remove)

**Why**: The workspace row in the sidebar has an overflow menu (`â€¦`) for
renaming, sharing, revealing the folder, and removing the workspace.
Post-cutover, all four handlers were stubs.

Steps:
1. In the sidebar, click the `â€¦` button on the selected workspace row.
2. Expect: popover opens with **Edit name**, **Shareâ€¦**, **Reveal in Finder**
   (desktop only), **Remove workspace**.
3. Click **Edit name** â†’ modal appears with current name selected.
4. Type a new name and click **Save**.
   - Pass: sidebar label and main header update immediately.
5. Open the menu again â†’ **Reveal in Finder**.
   - Pass: Finder becomes frontmost with the workspace folder highlighted
     (desktop only; no-op on web).
6. Open the menu again â†’ **Shareâ€¦**.
   - Pass: workspace path is copied to the clipboard. (Full ShareWorkspaceModal
     flow is still pending a second pass â€” see change log.)
7. Open the menu again â†’ **Remove workspace**.
...
- Focus-within reveals the `+` and `â€¦` buttons on every workspace row, not just
...
- Always start with `chrome-devtools_take_snapshot` after each interaction.
  Never trust that a click "worked" â€” re-snapshot and verify the new `uid`s.
...
- If a single flow fails, immediately run
  ...
  session â€” reload and reproduce with a narrower repro before continuing.
...
## Flow 13 â€” Slash command selection and backspace
...
## Flow 14 â€” Slash command error with model recovery

**Why**: When a slash command fails (e.g. model not found), the error must
show inline in the session area with a recovery action â€” not as raw
...
4. Expect: an amber "Pasted Â· N lines" chip appears inline in the composer
...
## Flow 16 â€” User message display
...
Pass criteria:
- No `CollapsibleUserText` or "N lines Â· expand" button on hand-typed text.
- The full message is visible immediately without clicking.

---

## Flow 17 â€” Stop / Run task single button
...
## Flow 18 â€” Skill lifecycle (create, verify, use)
...
## Flow 19 â€” Add workspace button placement
...
## Flow 20 â€” Local workspace creation opens a new session
...
## Flow 21 â€” Streaming survives session and route interruptions
...
### Subflow A â€” Switch to a different session while streaming
...
4. Wait 2â€“5 seconds on Session B.
...
### Subflow B â€” Navigate to Settings while streaming
...
4. Wait 2â€“5 seconds on Settings.
5. Click **Back to app**.
6. Poll the transcript until status is ready/idle or 90 seconds elapse.
...
the VenomCowork inspector and React Query cache so the eval can assert transcript
...
(function() {
  ...
  btn.click();
  return JSON.stringify({ ok: true, sessionId: window.__venomcowork.snapshot().route.selectedSessionId });
})()
```
...
(async function() {
  ...
  const route = window.__venomcowork.snapshot().route;
...
- 2026-04-16 â€” initial doc after the React port cutover fixed streaming,
  session CRUD, the model picker, and the settings tab infinite loop.
- 2026-04-17 â€” added Flow 10 (Cmd+N new session), Flow 11 (Cmd+K command
...
- 2026-04-28 â€” added Flows 13-19: slash command stability, error recovery
...
- 2026-05-06 â€” added Flow 20 to ensure local workspace creation opens the
  new workspace session surface instead of settings.
- 2026-05-18 â€” added Flow 21 to verify long streams survive switching to
