...
- Starts the **VenomCowork Cloud web app** on port 3005 inside Docker
...
- `DEN_API_PORT` â€” host port to map to the Den control plane :8788
- `DEN_WEB_PORT` â€” host port to map to the cloud web app :3005
- `DEN_BETTER_AUTH_SECRET` â€” Better Auth secret (auto-generated if unset)
- `DEN_PUBLIC_HOST` â€” host name/IP used for default auth URL + printed LAN/public URLs (defaults to your machine hostname)
- `DEN_BETTER_AUTH_URL` â€” browser-facing auth base URL (defaults to `http://$DEN_PUBLIC_HOST:<DEN_WEB_PORT>`)
- `DEN_MCP_RESOURCE_URL` â€” API-facing MCP resource URL (defaults to `http://localhost:<DEN_API_PORT>/mcp`)
- `DEN_BETTER_AUTH_TRUSTED_ORIGINS` â€” trusted origins for Better Auth (defaults to `DEN_CORS_ORIGINS`)
- `DEN_CORS_ORIGINS` â€” trusted origins for Express CORS (defaults include hostname, localhost, `127.0.0.1`, `0.0.0.0`, and detected LAN IPv4)
- `DEN_PROVISIONER_MODE` â€” `stub` or `render` (defaults to `stub`)
- `DEN_WORKER_URL_TEMPLATE` â€” stub worker URL template with `{workerId}` placeholder
...
From the VenomCowork repo root:

```bash
pnpm dev:den
```

Or from the VenomCowork enterprise root:

```bash
pnpm --dir _repos/venomcowork dev:den
```

What it does:
- Starts only **MySQL** in Docker
- Runs **Den controller** locally in watch mode
- Runs **VenomCowork Cloud web app** locally in Next.js dev mode
...
For micro-sandbox work, use the pre-baked image that compiles `venomcowork` and `venomcowork-server` from source and downloads the pinned `opencode` binary during `docker build`.

Build it from the repo root:

```bash
./scripts/build-microsandbox-venomcowork-image.sh
```
...
docker run --rm -p 8787:8787 \
  -e VENOMCOWORK_CONNECT_HOST=127.0.0.1 \
  venomcowork-microsandbox:dev
```

Defaults:
- `VENOMCOWORK_TOKEN=microsandbox-token`
- `VENOMCOWORK_HOST_TOKEN=microsandbox-host-token`
- `VENOMCOWORK_APPROVAL_MODE=auto`
...
Useful overrides:
- `VENOMCOWORK_TOKEN` â€” set your own client bearer token
- `VENOMCOWORK_HOST_TOKEN` â€” set your own host/admin token
- `VENOMCOWORK_CONNECT_HOST` â€” host name embedded in the printed connect URL
- `DOCKER_PLATFORM` â€” optional platform passed to `docker build`

---

## Production container

This is a minimal packaging template to run the VenomCowork Host contract in a single container.

It runs:

- `opencode serve` (engine) bound to `127.0.0.1:4096` inside the container
- `venomcowork-server` published on `0.0.0.0:8787` via an explicit `--remote-access` launch path (the only published surface)
...
- `VENOMCOWORK_TOKEN` (client token)
- `VENOMCOWORK_HOST_TOKEN` (host/owner token)

Optional:

- `VENOMCOWORK_APPROVAL_MODE=auto|manual`
- `VENOMCOWORK_APPROVAL_TIMEOUT_MS=30000`

Persistence:

- Workspace is mounted at `/workspace`
- Host data dir is mounted at `/data` (OpenCode caches + VenomCowork server config/tokens)

### Notes

- OpenCode is not exposed directly; access it via the VenomCowork proxy (`/opencode/*`).
