...
Use this directory to build and install the VenomCowork package locally on Arch Linux without Docker.
...
- Installs the Electron bundle under `/opt/venomcowork`.
- Adds `/usr/bin/venomcowork`, a desktop entry, and the VenomCowork icon.
...
2. build an Arch package such as `venomcowork-<version>-1-x86_64.pkg.tar.zst`
3. install it locally with `pacman`

After install, `venomcowork` is available as the desktop launcher. The bundled sidecars remain inside `/opt/venomcowork`; the package does not claim the standalone `opencode` command.
...
```bash
sudo pacman -U venomcowork-<version>-1-x86_64.pkg.tar.zst
```
