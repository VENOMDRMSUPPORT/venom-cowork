...
///
/// Three modes:
///   mcp       â€” run the MCP server over stdio
///   --check   â€” print permission status as JSON to stdout and exit
///   (default) â€” open the permission setup GUI
...
switch subcommand {
case "mcp":
    if ProcessInfo.processInfo.environment["VENOMCOWORK_COMPUTER_USE_CURSOR_OVERLAY"] == "0" {
...
case "--check":
    // Fresh process â†’ fresh TCC read â†’ always accurate.
