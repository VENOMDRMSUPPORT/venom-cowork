...
enum ComputerUsePermissions {
    /// Non-prompting check â€” safe to call on a timer / from HTTP polling.
    ...
    /// Prompting request â€” only call when the user explicitly clicks "Grant".
...
private final class PermissionSetupWindow: NSWindow {
    init() {
        ...
        title = "VenomCowork Computer Use"
...
final class PermissionSetupViewController: NSViewController {
    ...
    private func buildLayout() {
        ...
        let doneBtn = NSButton(title: "Done â€” Return to VenomCowork", target: self, action: #selector(done))
