# VenomCowork Computer Use

Native macOS computer-use runtime for VenomCowork.
Packaged VenomCowork builds wrap this runtime in a bundled `Computer Use.app`
...
```bash
pnpm --filter @venom-cowork/handsfree check:native
```

Run it as an MCP-compatible adapter:

```bash
pnpm --filter @venom-cowork/handsfree exec venomcowork-handsfree-computer-use mcp
```
