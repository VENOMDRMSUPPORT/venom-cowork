---
title: "Get started with VenomCowork Cloud"
description: "Connect the desktop app to your VenomCowork Cloud org and unlock shared workspaces, skill hubs, and shared providers"
---

VenomCowork Cloud requires an active subscription at [app.venomcowork.local](https://app.venomcowork.local/).
It is intended for teams what want to share `SKILLs.md`, `MCPs` and `configs`. 


First, you should connect your desktop app with Venomcowork Cloud.

<Frame>
  ![VenomCowork Cloud org settings with organization identity, allowed email domains, and desktop restrictions](/images/cloud-org-settings.png)
</Frame>

1. Open [app.venomcowork.local](https://app.venomcowork.local/) in your browser.
2. Click `Create organization`.
3. Enter your org name.
4. Finish checkout if VenomCowork Cloud asks for billing.
...
1. Open the `dashboard/org_settings` [page](https://app.venomcowork.local/dashboard/org-settings) in the cloud app.
...
4. If the browser does not return to VenomCowork automatically, click `Paste sign-in code`, paste the `venomcowork://den-auth?...` link or one-time code from VenomCowork Cloud, then click `Finish sign-in`.
...
- Use [Shared workspaces](/cloud/run-in-the-cloud/shared-workspace) to deploy hosted VenomCowork runtimes that teammates can open from the desktop app.
