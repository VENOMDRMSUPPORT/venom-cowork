...
In order to connect `slack` with your venomcowork `workspace`, you will need the following:

1. Valid slack admin permissions to add an app
2. Create a simple slack app
3. Connect said slack app with your workspace

This is one of the most relevant and meaningful applications of Venomcowork, and we use it daily ourselves.
...
For team-owned Slack or other long-running connectors, you can host the runtime in an VenomCowork Cloud [shared workspace](/cloud/run-in-the-cloud/shared-workspace) instead of keeping it on a laptop or self-managed server.
...
Go to [Create new app](https://api.slack.com/apps?new_app=1) in Slack. What you need to obtain are the `xoxb` (bot) and `xapp` (Websocket) tokens to paste into Venomcowork. For more guidelines, questions and instructions regarding slack, please follow [own slack's own documentation.](https://docs.slack.dev/tools/bolt-python/building-an-app)

1. Create the App Manifest: Go to `api.slack.com/apps` â†’ `Create New App` â†’ `From a manifest` â†’ pick your workspace.
2. Install to Workspace & Grab Tokens After creating the app:

- Install App â€” `sidebar` â†’ `Install App` â†’ `Install to Workspace` â†’ authorize.
- Copy the Bot token (`xoxb-...`) â€” shown right on the "Install App" page after authorization.

3. Generate an App-Level Token (for Socket Mode)

- Sidebar â†’ `Basic Information` â†’ scroll to `App-Level Tokens`.
- Click Generate Token and Scopes â†’ add scope `connections:write` â†’ name it anything â†’ Generate.
