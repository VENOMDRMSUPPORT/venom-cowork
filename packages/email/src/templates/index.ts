...
export const emailSubjects: { [Template in EmailTemplate]: (props: EmailTemplateProps[Template]) => string } = {
  verification: ({ verificationCode }) => `Your VenomCowork verification code is ${verificationCode}`,
  passwordReset: () => "Reset your VenomCowork password",
  organizationInvite: ({ organizationName }) => `You're invited to join ${organizationName} on VenomCowork`,
  feedback: ({ name, source }) => `VenomCowork feedback from ${name}${source ? ` (${source})` : ""}`,
}
