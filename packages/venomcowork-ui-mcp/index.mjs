 ...
 * venomcowork-ui-mcp
 *
 * MCP server that exposes VenomCowork's UI control surface as MCP tools.
 * Speaks MCP stdio and proxies to the VenomCowork desktop bridge HTTP API.
 *
 * Requires VenomCowork desktop running with the local UI control bridge active.
 *
 * Usage:
...
// â”€â”€ Bridge discovery â”€â”€

const DISCOVERY_FILE = "venomcowork-ui-control.json";
...
function discoveryPaths() {
  return [
    ...
    join(userAppDataDir(), "com.venom.cowork", DISCOVERY_FILE),
    join(userAppDataDir(), "com.venom.cowork.dev", DISCOVERY_FILE),
  ].filter(Boolean);
}
...
async function bridgeRequest(path, options = {}) {
  ...
  if (!bridge) {
    return {
      ok: false,
      error: "VenomCowork is not running. Launch the VenomCowork desktop app first.",
      hint: "The MCP server connects to a running VenomCowork instance via its local bridge.",
    };
  }
...
function formatActionLine(action) {
  ...
  return `${action.id}${disabled}${busy}\n    ${action.label || ""}${action.description ? ` â€” ${action.description}` : ""}${args ? `\n${args}` : ""}`;
}
...
server.tool(
  "ui_snapshot",
  "Get a snapshot of the current VenomCowork UI state: active route, narration, visible actions, and status. Use this before taking action to understand what the user sees.",
  {},
  async () => {
    ...
    if (Array.isArray(snapshot.actions)) {
      ...
      for (const action of snapshot.actions) {
        ...
        lines.push(`  ${action.id} â€” ${action.label || action.description || ""}${args}`);
      }
    }
    return { content: [{ type: "text", text: lines.join("\n") || "VenomCowork is reachable, but it did not return visible UI state." }] };
  }
);

// â”€â”€ ui.list_actions â”€â”€
server.tool(
  "ui_list_actions",
  "List all UI control actions currently available in VenomCowork: session navigation, composer control, transcript access, and more. Each action has an id you can pass to ui_execute_action.",
  {},
  async () => {
    ...
    if (!Array.isArray(result.actions) || result.actions.length === 0) {
      return { content: [{ type: "text", text: "No actions available. Is VenomCowork on the main screen?" }] };
    }
...
// â”€â”€ ui.execute_action â”€â”€
server.tool(
  "ui_execute_action",
  "Execute an VenomCowork UI action by its id. Use ui_list_actions first to see available actions and their required arguments.",
  {
...
server.tool(
  "ui_status",
  "Check if VenomCowork is running and the bridge is reachable. Returns connection status and app info.",
  {},
  async () => {
    const bridge = await discoverBridge();
    if (!bridge) {
      return { content: [{ type: "text", text: "VenomCowork is not running.\nLaunch the VenomCowork desktop app to enable UI control." }], isError: true };
    }
    try {
      const response = await fetch(`${bridge.baseUrl}/health`, { signal: AbortSignal.timeout(3000) });
      const data = await response.json();
      return { content: [{ type: "text", text: `Connected to ${data.app || "VenomCowork"}\nBridge: ${bridge.baseUrl}\nVersion: ${data.version ?? "?"}` }] };
    } catch (error) {
      clearBridgeCache();
      return { content: [{ type: "text", text: `Bridge file found but not reachable: ${error.message}\nVenomCowork may have quit. Relaunch it.` }], isError: true };
    }
  }
);
