...
Add self-serve enterprise SSO to Den so each VenomCowork organization can configure its own SAML or OIDC provider, expose an organization-specific sign-in URL, and keep SSO identity state aligned with SCIM provisioning state.
...
- SSO: `venomcowork-sso-${organizationId}`
- SCIM: `venomcowork-scim-${organizationId}`

If multiple SSO providers per org become necessary, use:

- `venomcowork-sso-${organizationId}-${connectionId}`
...
```text
https://app.venomcowork.local/sso/:orgSlug
```
